# Asi Extension Architecture & Development Guide

## Project Overview

Asi is a VSCode extension that provides AI assistance through a combination of a core extension backend and a React-based webview frontend. The extension is built with TypeScript and follows a modular architecture pattern.
...
The WebviewProvider maintains a reference to the Controller and delegates message handling to it. It also handles the creation of both sidebar and tab panel webviews, allowing Asi to be used in different contexts within VSCode.
...
Asi supports multiple AI providers through a modular API provider system. Each provider is implemented as a separate module in the `src/api/providers/` directory and follows a common interface.
...
Asi supports separate model configurations for Plan and Act modes:
...
class Task {
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    const previousRequest = this.AsiMessages[previousApiReqIndex]
...
class Task {
  async resumeTaskFromHistory() {
    // 1. Load saved state
    this.AsiMessages = await getSavedAsiMessages(this.getContext(), this.taskId)
  ...
  private async saveTaskState() {
    ...
    await saveAsiMessages(this.getContext(), this.taskId, this.AsiMessages)
...
Asi implements a dual-mode system that separates planning from execution:
...
Asi supports two types of MCP server connections:
...
class Controller {
  ...
  async downloadMcp(mcpId: string) {
    ...
    const response = await axios.post<McpDownloadResponse>(
      "https://api.Asi.bot/v1/mcp/download",
      { mcpId },
      {
    ...
    await this.initAsiWithTask(task)
  }
}
```
...
This guide provides a comprehensive overview of the Asi extension architecture, with special focus on state management, data persistence, and code organization. Following these patterns ensures robust feature implementation with proper state handling across the extension's components.
...
Contributions to the Asi extension are welcome! Please follow these guidelines:
...
The `.Asiignore` file allows users to specify files and directories that Asi should not access. When implementing new features, respect the `.Asiignore` rules and ensure that your code does not attempt to read or modify ignored files.
