...
**Proto files live in `proto/`** (e.g., `proto/asi/task.proto`, `proto/asi/ui.proto`)
- Each feature domain has its own `.proto` file
- For simple data, use shared types in `proto/asi/common.proto` (`StringRequest`, `Empty`, `Int64Request`)
...
**Adding new enum values** (like a new `AsiSay` type) requires updating conversion mappings in `src/shared/proto-conversions/Asi-message.ts`
...
- `proto/asi/task.proto` - Added `ExplainChangesRequest` message and `explainChanges` RPC
- `proto/asi/ui.proto` - Added `GENERATE_EXPLANATION = 29` to `AsiSay` enum
- `src/shared/ExtensionMessage.ts` - Added `AsiSayGenerateExplanation` type
- `src/shared/proto-conversions/Asi-message.ts` - Added mapping for new say type
...
1. `proto/asi/models.proto` - Add to the `ApiProvider` enum (e.g., `OPENAI_CODEX = 40;`)
...
1. **Add to `AsiDefaultTool` enum** in `src/shared/tools.ts`
2. **Tool definition** in `src/core/prompts/system-prompt/tools/` (create file like `generate_explanation.ts`)
   - Define variants for each `ModelFamily` (generic, next-gen, xs, etc.)
   - Export variants array (e.g., `export const my_tool_variants = [GENERIC, NATIVE_NEXT_GEN, XS]`)
   - **Fallback behavior**: If a variant isn't defined for a model family, `AsiToolSet.getToolByNameWithFallback()` automatically falls back to GENERIC. So you only need to export `[GENERIC]` unless the tool needs model-specific behavior.
...
8. **If tool has UI feedback**: add `AsiSay` enum in proto, update `src/shared/ExtensionMessage.ts`, update `src/shared/proto-conversions/Asi-message.ts`, update `webview-ui/src/components/chat/ChatRow.tsx`
...
- Add the field to `UpdateSettingsRequest` in `proto/asi/state.proto` (for webview update requests), then run `npm run protos`
