# Asi Hooks Documentation

## Overview

Asi hooks allow you to execute custom scripts at specific points in the agentic workflow. Hooks can be placed in either:
- **Global hooks directory**: `~/Documents/Asi/Hooks/` (applies to all workspaces)
- **Workspace hooks directory**: `.Asirules/hooks/` (applies to the workspace the repo is part of)

Hooks run automatically when enabled.

## Enabling Hooks

1. Open Asi settings in VSCode
...
- **Global Location**: `~/Documents/Asi/Hooks/TaskStart`
- **Workspace Location**: `.Asirules/hooks/TaskStart`
...
- **Global Location**: `~/Documents/Asi/Hooks/TaskResume`
- **Workspace Location**: `.Asirules/hooks/TaskResume`
...
- **Global Location**: `~/Documents/Asi/Hooks/TaskCancel`
- **Workspace Location**: `.Asirules/hooks/TaskCancel`
...
- **Global Location**: `~/Documents/Asi/Hooks/TaskComplete`
- **Workspace Location**: `.Asirules/hooks/TaskComplete`
...
- **Global Location**: `~/Documents/Asi/Hooks/UserPromptSubmit`
- **Workspace Location**: `.Asirules/hooks/UserPromptSubmit`

### PreToolUse Hook
- **When**: Runs BEFORE a tool is executed
- **Purpose**: Validate parameters, block execution, or add context
- **Global Location**: `~/Documents/Asi/Hooks/PreToolUse`
- **Workspace Location**: `.Asirules/hooks/PreToolUse`

### PostToolUse Hook
- **When**: Runs AFTER a tool completes
- **Purpose**: Observe results, track patterns, or add context
- **Global Location**: `~/Documents/Asi/Hooks/PostToolUse`
- **Workspace Location**: `.Asirules/hooks/PostToolUse`
...
- **Global Location**: `~/Documents/Asi/Hooks/PreCompact`
- **Workspace Location**: `.Asirules/hooks/PreCompact`

## Cross-Platform Hook Format

Asi uses a git-style approach for hooks that works consistently across all platforms:
...
Like git hooks, Asi executes hook files through a shell that interprets the shebang line:
...
```bash
# Create hook file
nano ~/Documents/Asi/Hooks/PreToolUse

# Make executable
chmod +x ~/Documents/Asi/Hooks/PreToolUse
```
...
# Log to file
echo "$input" >> ~/.Asi/hook-logs/tool-usage.jsonl
...
Asi supports two levels of hooks:

### Global Hooks
- **Location**: `~/Documents/Asi/Hooks/` (macOS/Linux)
...
### Workspace Hooks
- **Location**: `.Asirules/hooks/` in each workspace root
...
1. The global hooks directory is automatically created at:
   - macOS/Linux: `~/Documents/Asi/Hooks/`
...
3. Enable hooks in Asi settings
...
```bash
#!/usr/bin/env bash
# ~/Documents/Asi/Hooks/PreToolUse
...
If you have multiple workspace roots, you can place hooks in each root's `.Asirules/hooks/` directory. All hooks (global and workspace) may execute concurrently. Their results will be combined:
...
- Look for errors in VSCode's Output panel (Asi channel)
