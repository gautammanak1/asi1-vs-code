...
Global settings, secrets and workspace state are stored in **file-backed JSON stores** under `~/.Asi/data/`. This is the shared storage layer used by VSCode, CLI, and JetBrains.

## Key Abstractions

### `StorageContext` (src/shared/storage/storage-context.ts)
The entry point. Created via `createStorageContext()` and passed to `StateManager.initialize()`. Contains three `AsiFileStorage` instances:
- `globalState` → `~/.Asi/data/globalState.json`
- `secrets` → `~/.Asi/data/secrets.json` (mode 0o600)
- `workspaceState` → `~/.Asi/data/workspaces/<hash>/workspaceState.json`

### `AsiFileStorage` (src/shared/storage/AsiFileStorage.ts)
...
Remember that your data may be read by a different client than the one that wrote it. For example, a value written by Asi in JetBrains may be read by Asi CLI.
