...
if [ "$#" -ne 3 ]; then
    ...
    echo ""
    echo "Example:"
    echo "  $0 Asi-jetbrains-3.55.0.zip Asi-jetbrains-3.55.0-enterprise.zip endpoints.json"
    exit 1
fi
...
# JetBrains plugins typically have a structure like: Asi/lib/
# We need to add endpoints.json to the root of the plugin directory
PLUGIN_DIR="$TEMP_DIR/Asi"
...
echo "When installed in JetBrains IDEs, Asi will automatically use the bundled configuration."
