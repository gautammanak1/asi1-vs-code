 ...
 * Returns array of:
 * { serviceName: string, serviceKey: string, protoPackage: "Asi"|"host", moduleBase: string }
 */
async function parseServicesWithFiles(protoDir, protoFiles) {
	...
	for (const relPath of protoFiles) {
		...
		const pkg = relPath.startsWith("host/") ? "host" : "Asi"
...
async function generateAsiClientPy(outDir, services) {
...
class AsiClient:
    """
    Unified Python client analogous to src/generated/grpc-go/client/AsiClient.

    Usage:
        client = AsiClient("localhost:17611")
        client.connect()
...
`
	...
	await fs.writeFile(path.join(clientDir, "Asi_client.py"), content)
}
...
async function generatePythonClient(protoDir, pyOutDir, clientDir, protoFiles) {
	...
	// Asi_client.py (unified that composes service wrappers)
	await generateAsiClientPy(clientDir, services)
}
...
[project]
name = "Asi-grpc-python"
version = "0.1.0"
description = "Generated Python gRPC stubs and client wrappers for Asi protos"
