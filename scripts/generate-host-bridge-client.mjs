...
/** When the proto method name does not match the implementation file/export, map methodName -> module file base and optional export symbol. */
const HOST_BRIDGE_WORKSPACE_METHOD_IMPORT_OVERRIDES = {
	openAsiSidebarPanel: { module: "openClineSidebarPanel", exportAs: "openClineSidebarPanel" },
}

function getHostBridgeVscodeImportLine(serviceFolderName, methodName) {
	const override = HOST_BRIDGE_WORKSPACE_METHOD_IMPORT_OVERRIDES[methodName]
	const moduleBase = override?.module ?? methodName
	const exportAs = override?.exportAs ?? methodName
	if (exportAs !== methodName) {
		return `import { ${exportAs} as ${methodName} } from "@/hosts/vscode/hostbridge/${serviceFolderName}/${moduleBase}"`
	}
	return `import { ${methodName} } from "@/hosts/vscode/hostbridge/${serviceFolderName}/${moduleBase}"`
}
...
async function generateVscodeClientFile(hostServices) {
	...
	for (const [serviceName, serviceDefinition] of Object.entries(hostServices)) {
		...
		for (const [methodName, _methodDef] of Object.entries(serviceDefinition.service)) {
			imports.push(getHostBridgeVscodeImportLine(name, methodName))
		}
		imports.push("")
