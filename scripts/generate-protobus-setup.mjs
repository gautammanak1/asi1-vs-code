...
/**
 * Proto RPC method names do not always match the on-disk module basename (e.g. *Asi RPCs implemented in *Cline files).
 * Maps RPC name -> { module: file base without .ts, exportAs?: symbol exported from that file when it differs from rpcName }.
 */
const PROTOBUS_RPC_MODULE_OVERRIDES = {
	addToAsi: { module: "addToCline" },
	fixWithAsi: { module: "fixWithCline" },
	explainWithAsi: { module: "explainWithCline" },
	improveWithAsi: { module: "improveWithCline" },
	toggleAsiRule: { module: "toggleClineRule" },
	refreshAsiRecommendedModelsRpc: {
		module: "refreshClineRecommendedModelsRpc",
		exportAs: "refreshClineRecommendedModelsRpc",
	},
	refreshAsiModelsRpc: { module: "refreshClineModelsRpc", exportAs: "refreshClineModelsRpc" },
	installAsiCli: { module: "installClineCli" },
}

function getProtobusHandlerImportLine(dir, rpcName) {
	const override = PROTOBUS_RPC_MODULE_OVERRIDES[rpcName]
	const moduleBase = override?.module ?? rpcName
	const exportAs = override?.exportAs
	if (exportAs && exportAs !== rpcName) {
		return `import { ${exportAs} as ${rpcName} } from "@core/controller/${dir}/${moduleBase}"`
	}
	return `import { ${rpcName} } from "@core/controller/${dir}/${moduleBase}"`
}
...
async function generateWebviewProtobusClients(protobusServices) {
	...
	static override serviceName: string = "Asi.${serviceName}"
${rpcs.join("\n")}
}`)
	}
...
async function generateVscodeProtobusServers(protobusServices) {
	...
	for (const [serviceName, def] of Object.entries(protobusServices)) {
		...
		for (const [rpcName, _rpc] of Object.entries(def.service)) {
			imports.push(getProtobusHandlerImportLine(dir, rpcName))
			servers.push(`    ${rpcName}: ${rpcName},`)
		}
		servers.push(`} \n`)
		serviceMap.push(`    "Asi.${serviceName}": ${serviceName}Handlers,`)
		imports.push("")
	}
...
async function generateStandaloneProtobusServiceSetup(protobusServices) {
	...
	for (const [name, def] of Object.entries(protobusServices)) {
		...
		handlerSetup.push(`    server.addService(Asi.${name}Service, {`)
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			imports.push(getProtobusHandlerImportLine(dir, rpcName))
			const requestType = "Asi." + rpc.requestType.type.name
			const responseType = "Asi." + rpc.responseType.type.name
			if (rpc.requestStream) {
				throw new Error("Request streaming is not supported")
			}
			if (rpc.responseStream) {
...
import { Asi } from "@generated/grpc-js"
import { Controller } from "@core/controller"
