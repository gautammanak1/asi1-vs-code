...
# Configuration
INSTALL_DIR="${Asi_INSTALL_DIR:-$HOME/.Asi/cli}"
GITHUB_REPO="Asi/Asi"
requested_version="${Asi_VERSION:-}"
...
check_existing_installation() {
    ...
    if [ -d "$INSTALL_DIR/bin" ] && [ -f "$INSTALL_DIR/bin/Asi" ]; then
        # Extract version from Asi binary
        local installed_version=$("$INSTALL_DIR/bin/Asi" version 2>/dev/null | head -1 | grep -o 'v[0-9]\+\.[0-9]\+\.[0-9]\+' || echo "")
        ...
        if [ -n "$installed_version" ] && [ "$installed_version" = "$cli_tag_version" ]; then
            echo ""
            print_ok "Asi ${MAGENTA}${BOLD}$installed_version${NC} already installed"
            echo ""
...
install_Asi() {
    ...
    echo -e "${MAGENTA}${BOLD}"
    local package_file="$tmp_dir/Asi.tar.gz"
    ...
    print_ok "Asi installed to ${MAGENTA}${BOLD}$INSTALL_DIR${NC}"
}
...
verify_installation() {
    ...
    local Asi_bin="$INSTALL_DIR/bin/Asi"
    
    if [ ! -f "$Asi_bin" ]; then
        print_error "Binary not found at $Asi_bin"
        exit 1
    fi
...
print_success() {
    ...
    echo ""
    print_message "$NC" "Run this to start using ${MAGENTA}${BOLD}Asi${NC} immediately:"
    echo ""
...
main() {
    echo ""
    print_box "Asi IS COOKING" "$MAGENTA$BOLD" 48
    echo ""
