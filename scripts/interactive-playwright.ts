...
/**
 * Interactive Playwright launcher for the Asi VS Code extension.
 *
 * Overview:
 *  - Starts the mock Asi API server (from the e2e test fixtures).
 *  - Downloads a stable build of VS Code (via @vscode/test-electron).
 *  - Creates a temporary VS Code user profile directory.
 *  - Installs and links the Asi extension (from dist/e2e.vsix and the dev path).
 *  - Opens a test workspace and automatically reveals the Asi sidebar.
 ...
 *        npm run test:e2e:ui
 *
 *   3. VS Code will launch with the Asi extension loaded and gRPC recording enabled.
 *
...
import { AsiApiServerMock } from "../src/test/e2e/fixtures/server"
...
async function main() {
	...
	console.log("VSCode with Asi extension is now running!")
