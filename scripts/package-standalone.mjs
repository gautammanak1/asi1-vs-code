 ...
 * When Asi-core is installed, the installer should use the correct module for the current platform.
 */
async function packageAllBinaryDeps() {
	...
	for (const module of SUPPORTED_BINARY_MODULES) {
		...
		if (!fs.existsSync(src)) {
			console.warn(`Warning: Trying to install binaries for the module '${module}', but it is not being used by Asi.`)
			continue
		}
...
async function zipDistribution() {
	...
	// Add the whole Asi directory under "extension", except the for the ignored files.
