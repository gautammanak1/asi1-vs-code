...
// Get the fully qualified name for a proto type, e.g. getFqn('StringRequest') returns 'Asi.StringRequest'
...
export async function loadServicesFromProtoDescriptor() {
	...
	for (const [name, def] of Object.entries(proto.Asi)) {
		if (def && "service" in def) {
			protobusServices[name] = def
		} else {
			addTypeNameToFqn(name, `proto.Asi.${name}`)
		}
	}
