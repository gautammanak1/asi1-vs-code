 ...
 *    - Changes name to "Asi-nightly"
 *    - Changes displayName to "Asi (Nightly)"
...
const config = {
	...
	nightlyName: "Asi-nightly",
	originalName: "claude-dev",
	nightlyDisplayName: "Asi (Nightly)",
	...
	get vsixPath() {
		return path.join(this.distDir, "Asi-nightly.vsix")
	},
	get nodeModulesPath() {
...
class NightlyPublisher {
	 ...
	 * changes package.json name from "claude-dev" to "Asi-nightly". If we don't
	 * align this link, vsce's dependency detection (`npm list --production`) fails
	 * with ELSPROBLEMS (missing Asi-nightly + extraneous claude-dev).
	 */
	 ...
	 */
	updatePackageJson() {
		// Replace any occurrences Asi. or claude-dev with nightly name
		const rawContent = fs.readFileSync(config.packageJsonPath, "utf-8")
		const content = rawContent.replaceAll("claude-dev", config.nightlyName).replaceAll('"Asi.', `"${config.nightlyName}.`)
