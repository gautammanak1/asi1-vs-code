...
const getAsiVersion = () => {
	try {
		...
		const AsiMatch = extensions.match(/claude-dev@(\d+\.\d+\.\d+)/);
		return AsiMatch ? AsiMatch[1] : "Not installed";
	} catch (_err) {
...
const collectSystemInfo = () => {
	...
	try {
		if (process.platform === "darwin") {
			cpuInfo = execSync("sysctl -n machdep.cpu.brand_string")
				.toString()
				.trim();
...
async function main() {
	const consent = await ask(
		"Do you consent to collect system data and submit a GitHub issue? (y/n): ",
	);
