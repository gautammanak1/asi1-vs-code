...
SESSION="Asi-dev"
WORKSPACE="${Asi_WORKSPACE:-$(cd "$(dirname "$0")/.." && pwd)}"
ENVIRONMENT="${Asi_ENVIRONMENT:-production}"
...
export Asi_ENVIRONMENT="$ENVIRONMENT"
if [ -f .env ]; then
  set -a
  source .env
  set +a
...
tmux send-keys -t "$SESSION:0.3" "while [ ! -f '$WORKSPACE/dist/extension.js' ]; do sleep 0.5; done && echo 'Launching Extension Host...' && code --extensionDevelopmentPath='$WORKSPACE' --disable-workspace-trust --disable-extension saoudrizwan.claude-dev --disable-extension saoudrizwan.Asi-nightly '$WORKSPACE' && echo 'Extension Host launched.'" Enter
