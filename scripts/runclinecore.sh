...
# This installs the Asi-core app to the user's home directory,
...
LOG_FILE=~/.Asi/Asi-core-service.log
...
pkill -f Asi-core.js || true

# Detect platform name using the same logic as AsiDirs.kt in the plugin.
...
echo pwd: $(pwd)
set -x
NODE_PATH=$BINARY_MODULES_DIR:./node_modules DEV_WORKSPACE_FOLDER=/tmp/ node Asi-core.js 2>&1 | tee $LOG_FILE
