...
import {
	...
	DiffServiceService,
} from "../src/generated/grpc-js/host/diff";
import {
	type EnvServiceServer,
	EnvServiceService,
} from "../src/generated/grpc-js/host/env";
import {
	type TestingServiceServer,
	TestingServiceService,
} from "../src/generated/grpc-js/host/testing";
import {
	type WindowServiceServer,
	WindowServiceService,
} from "../src/generated/grpc-js/host/window";
import {
	type WorkspaceServiceServer,
	WorkspaceServiceService,
} from "../src/generated/grpc-js/host/workspace";
...
export async function startTestHostBridgeServer() {
	...
	server.addService(
		WorkspaceServiceService,
		createMockService<WorkspaceServiceServer>("WorkspaceService"),
	);
	server.addService(
		WindowServiceService,
		createMockService<WindowServiceServer>("WindowService"),
	);
	server.addService(
		EnvServiceService,
		createMockService<EnvServiceServer>("EnvService"),
	);
	server.addService(
		DiffServiceService,
		createMockService<DiffServiceServer>("DiffService"),
	);
	server.addService(
		TestingServiceService,
		createMockService<TestingServiceServer>("TestingService"),
	);
	...
	server.bindAsync(
		...
		(err) => {
			if (err) {
				console.error(
					`Failed to bind test host bridge server to ${bindAddress}:`,
					err,
				);
				process.exit(1);
			}
 ...
 */
function createMockService<T extends grpc.UntypedServiceImplementation>(
	serviceName: string,
): T {
	const handler: ProxyHandler<T> = {
		get(_target, prop) {
			...
			return (call: any, callback: any) => {
				console.log(
					`Hostbridge: ${serviceName}.${String(prop)} called with:`,
					call.request,
				);
				...
				switch (prop) {
					case "getWorkspacePaths": {
						const workspaceDir =
							process.env.TEST_HOSTBRIDGE_WORKSPACE_DIR || "/test-workspace";
						callback(null, {
							paths: [workspaceDir],
						});
						return;
