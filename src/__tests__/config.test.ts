...
import { AsiConfigurationError, AsiEndpoint, AsiEnv, Environment } from "../config"

describe("AsiEndpoint configuration", () => {
	...
	beforeEach(async () => {
		...
		// Create .Asi directory
		await fs.mkdir(path.join(tempDir, ".Asi"), { recursive: true })
		...
		;(AsiEndpoint as any)._instance = null
		;(AsiEndpoint as any)._initialized = false
		;(AsiEndpoint as any)._extensionFsPath = undefined
	})
	...
	describe("valid config parsing", () => {
		it("should parse valid endpoints.json with all required fields", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), JSON.stringify(validConfig), "utf8")

			await AsiEndpoint.initialize(tempDir)

			const config = AsiEndpoint.config
		...
		it("should work without endpoints.json (standard mode)", async () => {
			...
			config.appBaseUrl.should.equal("https://app.Asi.bot")
			config.apiBaseUrl.should.equal("https://api.Asi.bot")
		})
		...
		it("should accept URLs with paths", async () => {
			const validConfig = {
				appBaseUrl: "https://proxy.enterprise.com/asi/app",
				apiBaseUrl: "https://proxy.enterprise.com/asi/api",
				mcpBaseUrl: "https://proxy.enterprise.com/asi/mcp",
			}
			...
			const config = AsiEndpoint.config
			config.appBaseUrl.should.equal("https://proxy.enterprise.com/asi/app")
		})
	})

	describe("invalid JSON handling", () => {
		it("should throw AsiConfigurationError for invalid JSON syntax", async () => {
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), "{ invalid json }", "utf8")

			try {
				await AsiEndpoint.initialize(tempDir)
				throw new Error("Should have thrown")
			} catch (error: any) {
				error.should.be.instanceof(AsiConfigurationError)
				error.message.should.containEql("Invalid JSON")
			}
		})

		it("should throw AsiConfigurationError for truncated JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), '{"appBaseUrl": "https://test.com"', "utf8")
		...
		it("should throw AsiConfigurationError for empty file", async () => {
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), "", "utf8")
		...
		it("should throw AsiConfigurationError for non-object JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), '"just a string"', "utf8")
		...
		it("should throw AsiConfigurationError for array JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), "[]", "utf8")
		...
		it("should throw AsiConfigurationError for null JSON", async () => {
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), "null", "utf8")
	...
	describe("missing required fields", () => {
		it("should throw AsiConfigurationError when appBaseUrl is missing", async () => {
			const config = {
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}

			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), JSON.stringify(config), "utf8")
		...
		it("should throw AsiConfigurationError when apiBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
			}
		...
		it("should throw AsiConfigurationError when mcpBaseUrl is missing", async () => {
			const config = {
				appBaseUrl: "https://app.enterprise.com",
				apiBaseUrl: "https://api.enterprise.com",
			}
		...
		it("should throw AsiConfigurationError when all fields are missing", async () => {
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), "{}", "utf8")
		...
		it("should throw AsiConfigurationError when field is null", async () => {
			const config = {
				appBaseUrl: null,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw AsiConfigurationError when field is empty string", async () => {
			const config = {
				appBaseUrl: "",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw AsiConfigurationError when field is whitespace only", async () => {
			const config = {
				appBaseUrl: "   ",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
		...
		it("should throw AsiConfigurationError when field is non-string", async () => {
			const config = {
				appBaseUrl: 12345,
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("invalid URL detection", () => {
		it("should throw AsiConfigurationError for invalid URL format", async () => {
			const config = {
		...
		it("should throw AsiConfigurationError for URL without protocol", async () => {
			const config = {
		...
		it("should throw AsiConfigurationError for malformed URL", async () => {
			const config = {
				appBaseUrl: "https://",
				apiBaseUrl: "https://api.enterprise.com",
				mcpBaseUrl: "https://mcp.enterprise.com",
	...
	describe("environment switching blocked in self-hosted mode", () => {
		it("should throw error when trying to change environment in self-hosted mode", async () => {
			...
			AsiEndpoint.config.environment.should.equal(Environment.selfHosted)

			// Try to change environment - should throw
			try {
				AsiEnv.setEnvironment("staging")
				throw new Error("Should have thrown")
			} catch (error: any) {
		...
		it("should allow environment switching in standard mode", async () => {
			...
			AsiEndpoint.config.environment.should.not.equal(Environment.selfHosted)

			// Should be able to change environment
			AsiEnv.setEnvironment("staging")
			AsiEnv.getEnvironment().environment.should.equal("staging")

			AsiEnv.setEnvironment("local")
			AsiEnv.getEnvironment().environment.should.equal("local")

			AsiEnv.setEnvironment("production")
			AsiEnv.getEnvironment().environment.should.equal("production")
		})
	})
	...
	describe("self-hosted mode behavior", () => {
		it("should report selfHosted environment in self-hosted mode", async () => {
			...
			const envConfig = AsiEndpoint.config
		...
		it("should use custom endpoints from file", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), JSON.stringify(customConfig), "utf8")
	...
	describe("initialization behavior", () => {
		it("should only initialize once", async () => {
			await AsiEndpoint.initialize(tempDir)
			AsiEndpoint.isInitialized().should.be.true()
	...
	describe("isSelfHosted() method", () => {
		it("should return true when not initialized (safety fallback)", async () => {
			...
			AsiEndpoint.isInitialized().should.be.false()
			AsiEndpoint.isSelfHosted().should.be.true()
		})
		...
		it("should return false when in normal mode (no endpoints.json)", async () => {
			...
			AsiEndpoint.isSelfHosted().should.be.false()
		})
	})
	...
	describe("bundled endpoints.json behavior", () => {
		...
		it("should use bundled endpoints.json when available", async () => {
			...
			await AsiEndpoint.initialize(bundledDir)
		...
		it("should prefer bundled endpoints.json over user file", async () => {
			...
			// Set up both configs
			await fs.writeFile(path.join(bundledDir, "endpoints.json"), JSON.stringify(bundledConfig), "utf8")
			await fs.writeFile(path.join(tempDir, ".Asi", "endpoints.json"), JSON.stringify(userConfig), "utf8")
		...
		it("should throw AsiConfigurationError for invalid bundled file", async () => {
			const invalidConfig = {
		...
		it("should throw AsiConfigurationError for invalid JSON in bundled file", async () => {
