 ...
 *
 * @param AsiMessages - Array of messages containing tool calls to transform
...
export function transformToolCallMessages(
	AsiMessages: AsiStorageMessage[],
	nativeTools?: AsiDefaultTool[],
): AsiStorageMessage[] {
	// Early return if no messages or native tools provided
	if (!AsiMessages?.length || !nativeTools?.length) {
		return AsiMessages
	}
	...
	const hasApplyPatchNative = nativeToolSet.has(AsiDefaultTool.APPLY_PATCH)
	const hasFileEditNative = nativeToolSet.has(AsiDefaultTool.FILE_EDIT) || nativeToolSet.has(AsiDefaultTool.FILE_NEW)

	const hasApplyPatchUsed = usedToolSet.has(AsiDefaultTool.APPLY_PATCH)
	const hasFileEditUsed = usedToolSet.has(AsiDefaultTool.FILE_EDIT) || usedToolSet.has(AsiDefaultTool.FILE_NEW)

	// Convert write_to_file/replace_in_file → apply_patch
	if (hasApplyPatchNative && hasFileEditUsed) {
		return convertWriteToFileToolCalls(AsiMessages)
	}

	// Convert apply_patch → write_to_file/replace_in_file
	if (hasFileEditNative && hasApplyPatchUsed) {
		return convertApplyPatchToolCalls(AsiMessages)
	}
