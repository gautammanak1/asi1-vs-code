...
describe("AsiHandler", () => {
	...
	const createHandler = (options: ConstructorParameters<typeof AsiHandler>[0]) => {
		sinon.stub(AsiAccountService, "getInstance").returns({} as any)
		sinon.stub(AuthService, "getInstance").returns({} as any)
		return new AsiHandler(options)
	}
