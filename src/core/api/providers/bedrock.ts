...
import type { AsiTool } from "@/shared/tools"
...
export class AwsBedrockHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: AsiStorageMessage[], tools?: AsiTool[]): ApiStream {
	...
	private async getAwsCredentials(): Promise<{
		...
		const providerOptions: ProviderChainOptions = {
			clientConfig: {
				...
				userAgentAppId: `Asi#${ExtensionRegistryInfo.version}`,
			},
		}
		const useProfile =
	 ...
	 */
	private formatDeepseekR1Prompt(systemPrompt: string, messages: AsiStorageMessage[]): string {
	 ...
	 */
	private estimateInputTokens(systemPrompt: string, messages: AsiStorageMessage[]): number {
	...
	/**
	 * Converts Asi's tool definitions (Anthropic format with `input_schema`) to the
	 ...
	 */
	private mapAsiToolsToBedrockToolConfig(tools?: AsiTool[]): ToolConfiguration | undefined {
		if (!tools || tools.length === 0) {
			return undefined
		}

		const isAnthropicTool = (tool: AsiTool): tool is AnthropicTool => "input_schema" in tool
	...
	private async *createAnthropicMessage(
		...
		const toolConfig = this.mapAsiToolsToBedrockToolConfig(tools)
	 ...
	 */
	private formatMessagesForConverseAPI(messages: AsiStorageMessage[]): Message[] {
