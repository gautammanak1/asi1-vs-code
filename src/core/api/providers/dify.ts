...
export class DifyHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: AsiStorageMessage[]): ApiStream {
		...
		const requestBody = {
			...
			user: "Asi-user", // A unique user identifier
			files: [],
		}
	...
	private convertMessagesToQuery(systemPrompt: string, messages: AsiStorageMessage[]): string {
	 ...
	 * @param user User identifier (defaults to "Asi-user")
	 * @returns Promise with file upload response
	 */
	async uploadFile(file: Buffer, filename: string, user: string = "Asi-user"): Promise<DifyFileResponse> {
	 ...
	 */
	async stopGeneration(taskId: string, user: string = "Asi-user"): Promise<void> {
	 ...
	 */
	async deleteConversation(conversationId: string, user: string = "Asi-user"): Promise<void> {
