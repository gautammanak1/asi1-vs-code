...
import { ApiFormat } from "@/shared/proto/index.Asi"
...
export class OcaHandler implements ApiHandler {
	...
	async *createMessageChatApi(systemPrompt: string, messages: AsiStorageMessage[], tools?: OpenAITool[]): ApiStream {
		const client = this.ensureOpenAIClient()
		...
		const chatCompletionsParams: OpenAI.Chat.Completions.ChatCompletionCreateParamsStreaming = {
			...
			...(this.options.taskId && {
				litellm_session_id: `Asi-${this.options.taskId}`,
	...
	async *createMessageResponsesApi(systemPrompt: string, messages: AsiStorageMessage[], tools?: OpenAITool[]): ApiStream {
		const client = this.ensureOpenAIClient()
	...
	async *createMessageMessagesApi(systemPrompt: string, messages: AsiStorageMessage[], tools?: OpenAITool[]): ApiStream {
