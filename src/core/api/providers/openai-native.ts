...
export class OpenAiNativeHandler implements ApiHandler {
	...
	private ensureClient(): OpenAI {
		if (!this.client) {
			...
			try {
				...
				throw new Error(
					`Error creating OpenAI client: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
		}
		return this.client;
	...
	async *createMessage(
		...
		if (
			...
			if (!tools?.length) {
				throw new Error(
					"Native Tool Call must be enabled in your setting for OpenAI Responses API",
				);
			}
	...
	private async *createCompletionStream(
			...
			const response = await client.chat.completions.create(
				{
					...
					messages: [
						...
						...convertToOpenAiMessages(messages, "openai-native"),
					],
				},
		...
		const reasoningEffort =
			includeReasoning && requestedEffort !== "none"
		...
		const stream = await client.chat.completions.create({
			...
			...(model.info.temperature !== undefined
				? { temperature: model.info.temperature }
				: {}),
			...(includeTools
				? getOpenAIToolParams(tools, isGPT5ModelFamily(model.id))
				: {}),
		});
		...
		for await (const chunk of stream) {
			...
			if (delta?.tool_calls) {
				try {
					...
					Logger.error(
						"Error processing tool call delta:",
						error,
						delta.tool_calls,
					);
				}
	...
	private async *createResponseStream(
		...
		const { input, previousResponseId } = convertToOpenAIResponsesInput(
			messages,
		...
		if (usePreviousResponseId && previousResponseId) {
			try {
				...
				Logger.error(
					"OpenAI websocket mode failed, falling back to HTTP Responses API:",
					error,
				);
	...
	private useWebsocketMode(apiFormat?: ApiFormat): boolean {
		if (
			featureFlagsService.getBooleanFlagEnabled(
				FeatureFlag.OPENAI_RESPONSES_WEBSOCKET_MODE,
			)
		) {
	...
	private buildResponseCreateParams(args: {
		...
		const reasoning:
			| { effort: ChatCompletionReasoningEffort; summary: "auto" }
			| undefined =
	...
	private async *createResponseStreamHttp(
		...
		const client = this.ensureClient();
		Logger.debug(
			`OpenAI Responses Input (HTTP): ${JSON.stringify(params.input)}`,
		);
	...
	private async *createResponseStreamWebsocket(
		...
		Logger.debug(
			`OpenAI Responses Input (WebSocket): ${JSON.stringify(primaryParams.input)}`,
		);
		try {
			yield* this.processResponsesEvents(
				this.createResponseEventsViaWebsocket(primaryParams),
				modelInfo,
			);
		} catch (error) {
			if (
				this.shouldRetryWebsocketWithFullContext(
					error,
					!!primaryParams.previous_response_id,
				)
			) {
				Logger.log(
				...
				this.closeResponsesWebsocket();
				yield* this.processResponsesEvents(
					this.createResponseEventsViaWebsocket(fallbackParams),
					modelInfo,
				);
				return;
			}
	...
	private shouldRetryWebsocketWithFullContext(
		error: unknown,
	...
	private async *createResponseEventsViaWebsocket(
		...
		if (this.websocketRequestInFlight) {
			const error: Error & { code?: string } = new Error(
				"Websocket response.create is already in progress",
			);
		...
		const handleError = () => {
			const error: Error & { code?: string } = new Error(
				"Responses websocket emitted an error event",
			);
		...
		const handleClose = () => {
			if (!completed) {
				const error: Error & { code?: string } = new Error(
					"Responses websocket closed during response stream",
				);
	...
	private async *processResponsesEvents(
		...
		const functionCallByItemId = new Map<
			string,
			{ call_id?: string; name?: string; id?: string }
		>();
