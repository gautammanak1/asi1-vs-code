...
namespace Bedrock {
	 ...
	 */
	export function formatMessagesForConverseAPI(messages: AsiStorageMessage[]): BedrockMessage[] {
...
export class SapAiCoreHandler implements ApiHandler {
	...
	private async *createMessageWithOrchestration(systemPrompt: string, messages: AsiStorageMessage[]): ApiStream {
		try {
			...
			// sent directly to the LLM with the Asi system prompt without validation errors.
	...
	private async *createMessageWithDeployments(systemPrompt: string, messages: AsiStorageMessage[]): ApiStream {
		const token = await this.getToken()
		const externalHeaders = buildExternalBasicHeaders()
		const headers = {
			...externalHeaders,
	...
	private convertMessageParamToSAPMessages(messages: AsiStorageMessage[]): ChatMessage[] {
