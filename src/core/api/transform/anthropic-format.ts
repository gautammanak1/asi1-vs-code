...
import { AsiStorageMessage, convertAsiStorageToAnthropicMessage } from "@/shared/messages/content"

/**
 * Converts Asi storage messages to Anthropic API format with optional cache control.
 * Adds ephemeral cache control to the last two user messages to prevent them from being
 * stored in Anthropic's cache.
 *
 * @param AsiMessages - Array of Asi storage messages to convert
...
export function sanitizeAnthropicMessages(
	AsiMessages: Array<AsiStorageMessage | Anthropic.MessageParam>,
	...
	const userMsgIndices = AsiMessages.reduce((acc, msg, index) => {
		if (msg.role === "user") {
			acc.push(index)
		}
		return acc
	...
	return AsiMessages.map((msg, index) => {
		const anthropicMsg = convertAsiStorageToAnthropicMessage(msg)
