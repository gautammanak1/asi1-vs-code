 ...
 *
 * @param toolId - The original tool ID from Asi/Anthropic format
...
/**
 * Converts an array of AsiStorageMessage objects to OpenAI's Completions API format.
 *
 * Handles conversion of Asi-specific content types (tool uses, tool results, images, reasoning details)
 * into OpenAI's expected message structure, including tool_calls and tool_call_id fields.
 *
 * @param anthropicMessages - Array of AsiStorageMessage objects to be converted
...
export function convertToOpenAiMessages(
	anthropicMessages: Omit<AsiStorageMessage, "modelInfo">[],
         ...
         */
			if (anthropicMessage.role === "user") {
				const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
					nonToolMessages: (AsiTextContentBlock | AsiImageContentBlock)[]
					toolMessages: AsiUserToolResultContentBlock[]
				}>(
					(acc, part) => {
				...
				const toolResultImages: AsiImageContentBlock[] = []
				...
				const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
					...
					toolMessages: AsiAssistantToolUseBlock[]
				}>(
					(acc, part) => {
				...
				const tool_calls: OpenAI.Chat.ChatCompletionMessageToolCall[] = toolMessages.map((toolMessage) => {
					...
					if (toolDetails) {
						if (Array.isArray(toolDetails)) {
							...
							// See: https://github.com/gautammanak1/asi1-vs-code/issues/8214
...
const UNIQUE_ERROR_TOOL_NAME = "_Asi_error_unknown_function_"
