...
import { AsiAssistantThinkingBlock, AsiStorageMessage } from "@/shared/messages/content"
...
export function addReasoningContent(
	...
	originalMessages: AsiStorageMessage[],
		...
		if (msg.role === "assistant") {
			if (Array.isArray(msg.content)) {
				const thinking = msg.content
					.filter((p): p is AsiAssistantThinkingBlock => p.type === "thinking")
					.map((p) => p.thinking)
					.join("\n")
				if (thinking) {
