import { getSavedAsiMessages, getTaskMetadata, readTaskHistoryFromState, writeTaskHistoryToState } from "@core/storage/disk"
...
async function reconstructTaskHistoryItem(taskId: string): Promise<HistoryItem | null> {
	try {
		...
		const AsiMessages = await getSavedAsiMessages(taskId)
		if (AsiMessages.length === 0) {
		...
		const taskInfo = extractTaskInformation(AsiMessages, metadata)
...
function extractTaskInformation(AsiMessages: AsiMessage[], metadata: any): TaskInfo {
	// Find the first user message (task description)
	const firstUserMessage = AsiMessages.find((msg) => msg.type === "say" && msg.say === "text" && msg.text)

	// Extract timestamp from first message or use task ID as fallback
	const timestamp = AsiMessages.length > 0 ? AsiMessages[0].ts : Date.now()
	...
	const apiReqMessages = AsiMessages.filter(
	...
	const messageSize = JSON.stringify(AsiMessages).length
