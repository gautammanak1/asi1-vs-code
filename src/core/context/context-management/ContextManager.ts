...
import { AsiApiReqInfo, AsiMessage } from "@shared/ExtensionMessage"
...
export class ContextManager {
	...
	shouldCompactContextWindow(
		...
		if (previousApiReqIndex >= 0) {
			const previousRequestText = AsiMessages[previousApiReqIndex]?.text
			if (previousRequestText) {
				try {
					const { tokensIn, tokensOut, cacheWrites, cacheReads }: AsiApiReqInfo = JSON.parse(previousRequestText)
	...
	getContextTelemetryData(
		...
		if (triggerIndex !== undefined) {
			...
			const apiReqIndices = AsiMessages
		...
		if (targetIndex >= 0) {
			const targetRequestText = AsiMessages[targetIndex]?.text
			if (targetRequestText) {
				try {
					const { tokensIn, tokensOut, cacheWrites, cacheReads }: AsiApiReqInfo = JSON.parse(targetRequestText)
	...
	async getNewContextMessagesAndMetadata(
		...
		if (!useAutoCondense) {
			...
			if (previousApiReqIndex >= 0) {
				const previousRequestText = AsiMessages[previousApiReqIndex]?.text
				if (previousRequestText) {
					const timestamp = AsiMessages[previousApiReqIndex].ts
					const { tokensIn, tokensOut, cacheWrites, cacheReads }: AsiApiReqInfo = JSON.parse(previousRequestText)
					const totalTokens = (tokensIn || 0) + (tokensOut || 0) + (cacheWrites || 0) + (cacheReads || 0)
	...
	public getNextTruncationRange(
		...
		// NOTE: anthropic format messages are always user-assistant-user-assistant, while openai format messages can have multiple user messages in a row (we use anthropic format throughout Asi)
	...
	async attemptFileReadOptimization(
		...
		const previousRequest = AsiMessages[previousApiReqIndex]
