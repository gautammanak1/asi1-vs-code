...
describe("ContextManager", () => {
					...
					content: [
						{
							...
							content: [
								{
									type: "text",
									text: "[write_to_file for '/Users/toshi/Desktop/Asi_testing_repo/test2.txt'] Result:\nThe content was successfully saved.\n\n<final_file_content path=\"/Users/toshi/Desktop/Asi_testing_repo/test2.txt\">\ntest\n\n</final_file_content>",
								},
							],
						},
					...
					content: [
						{
							...
							content: [
								{
									type: "text",
									text: "[replace_in_file for '/Users/toshi/Desktop/Asi_testing_repo/test2.txt'] Result:\nThe content was successfully saved.\n\n<final_file_content path=\"/Users/toshi/Desktop/Asi_testing_repo/test2.txt\">\ntest2\n\n</final_file_content>",
								},
							],
						},
						{ type: "text", text: "<environment_details>\n# Current Mode\nACT MODE\n</environment_details>" },
	...
	describe("shouldCompactContextWindow", () => {
		...
		it("does not compact at 33K tokens with default 0.75 threshold on 200K context", () => {
			...
			const AsiMessages: AsiMessage[] = [createApiReqMessage({ tokensIn: 30_000, tokensOut: 3_000 })]

			const result = contextManager.shouldCompactContextWindow(AsiMessages, api, 0, 0.75)
			expect(result).to.equal(false)
		})

		it("compacts when tokens exceed 0.75 threshold on 200K context", () => {
			const api = createMockApi(200_000)
			const AsiMessages: AsiMessage[] = [createApiReqMessage({ tokensIn: 140_000, tokensOut: 15_000 })]
		...
		it("compacts at only 10K tokens when threshold is accidentally set to 0.05", () => {
			...
			const AsiMessages: AsiMessage[] = [createApiReqMessage({ tokensIn, tokensOut })]

			const result = contextManager.shouldCompactContextWindow(AsiMessages, api, 0, accidentalThreshold)
			expect(result).to.equal(true)
		})
		...
		it("falls back to maxAllowedSize when threshold is undefined", () => {
			...
			const AsiMessages: AsiMessage[] = [createApiReqMessage({ tokensIn: 150_000, tokensOut: 5_000 })]

			const result = contextManager.shouldCompactContextWindow(AsiMessages, api, 0, undefined)
		...
		it("falls back to maxAllowedSize when threshold is 0", () => {
			...
			const result = contextManager.shouldCompactContextWindow(AsiMessages, api, 0, 0)
			expect(result).to.equal(false)
		})
		...
		it("returns false when previousApiReqIndex is negative", () => {
			const api = createMockApi(200_000)
			const AsiMessages: AsiMessage[] = [createApiReqMessage({ tokensIn: 200_000 })]

			const result = contextManager.shouldCompactContextWindow(AsiMessages, api, -1, 0.75)
			expect(result).to.equal(false)
		})
		...
		it("threshold is capped at maxAllowedSize even when percentage is very high", () => {
			...
			const AsiMessages: AsiMessage[] = [createApiReqMessage({ tokensIn: 165_000 })]

			const result = contextManager.shouldCompactContextWindow(AsiMessages, api, 0, 1.0)
			expect(result).to.equal(true)
		})
	})
})
