...
/**
 * Builds the evaluation context used for conditional Asi Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in AsiMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const AsiMessages = deps.messageStateHandler.getAsiMessages()
		...
		// (3) Files edited by Asi during this task (completed operations):
