...
describe("Skills Utility Functions", () => {
	...
	const GLOBAL_SKILLS_DIR = path.join("/home", "user", ".Asi", "skills")
	...
	beforeEach(() => {
		...
		sandbox.stub(disk, "getSkillsDirectoriesForScan").returns([
			{ path: path.join(TEST_CWD, ".Asirules", "skills"), source: "project" },
			{ path: path.join(TEST_CWD, ".Asi", "skills"), source: "project" },
...
Instructions here`)
		...
		it("should discover skills from project .Asirules/skills directory", async () => {
			const projectSkillsDir = path.join(TEST_CWD, ".Asirules", "skills")
...
Use analogies and ASCII diagrams when explaining code.`)
		...
		it("should discover skills from project .Asi/skills directory", async () => {
			const AsiSkillsDir = path.join(TEST_CWD, ".Asi", "skills")
			const skillDir = path.join(AsiSkillsDir, "debugging")
			const skillMdPath = path.join(skillDir, "SKILL.md")

			fileExistsStub.withArgs(AsiSkillsDir).resolves(true)
			fileExistsStub.withArgs(skillMdPath).resolves(true)
			isDirectoryStub.withArgs(AsiSkillsDir).resolves(true)
			readdirStub.withArgs(AsiSkillsDir).resolves(["debugging"])
			statStub.withArgs(skillDir).resolves({ isDirectory: () => true })
			readFileStub.withArgs(skillMdPath, "utf-8").resolves(`---
name: debugging
