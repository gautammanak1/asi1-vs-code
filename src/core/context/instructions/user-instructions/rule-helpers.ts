...
export async function synchronizeRuleToggles(
	...
	currentToggles: AsiRulesToggles,
	allowedFileExtension: string = "",
	excludedPaths: string[][] = [],
): Promise<AsiRulesToggles> {
...
export function synchronizeRemoteRuleToggles(
	...
	const updatedToggles: AsiRulesToggles = {}
 ...
 */
export function combineRuleToggles(toggles1: AsiRulesToggles, toggles2: AsiRulesToggles): AsiRulesToggles {
 ...
 */
export const getRuleFilesTotalContent = async (rulesFilePaths: string[], basePath: string, toggles: AsiRulesToggles) => {
 ...
 * Used by getGlobalAsiRules and getLocalAsiRules.
 */
...
/**
 * Handles converting any directory into a file (specifically used for .Asirules and .Asirules/workflows)
 * The old .Asirules file or .Asirules/workflows file will be renamed to a default filename
 * Doesn't do anything if the dir already exists or doesn't exist
 * Returns whether there are any uncaught errors
 */
export async function ensureLocalAsiDirExists(AsirulePath: string, defaultRuleFilename: string): Promise<boolean> {
	try {
		const exists = await fileExistsAtPath(AsirulePath)

		if (exists && !(await isDirectory(AsirulePath))) {
			// logic to convert .Asirules file into directory, and rename the rules file to {defaultRuleFilename}
			const content = await fs.readFile(AsirulePath, "utf8")
			const tempPath = AsirulePath + ".bak"
			await fs.rename(AsirulePath, tempPath) // create backup
			try {
				await fs.mkdir(AsirulePath, { recursive: true })
				await fs.writeFile(path.join(AsirulePath, defaultRuleFilename), content, "utf8")
				...
				try {
					await fs.rm(AsirulePath, { recursive: true, force: true }).catch(() => {})
					await fs.rename(tempPath, AsirulePath) // restore backup
				} catch (_restoreError) {}
...
export const createRuleFile = async (isGlobal: boolean, filename: string, cwd: string, type: string) => {
	try {
		...
		if (isGlobal) {
			if (type === "workflow") {
				const globalAsiWorkflowFilePath = await ensureWorkflowsDirectoryExists()
				filePath = path.join(globalAsiWorkflowFilePath, filename)
			} else {
				const globalAsiRulesFilePath = await ensureRulesDirectoryExists()
				filePath = path.join(globalAsiRulesFilePath, filename)
			}
		} else {
			const localAsiRulesFilePath = path.resolve(cwd, GlobalFileNames.AsiRules)

			const hasError = await ensureLocalAsiDirExists(localAsiRulesFilePath, "default-rules.md")
			if (hasError === true) {
				return { filePath: null, fileExists: false }
			}

			await fs.mkdir(localAsiRulesFilePath, { recursive: true })

			if (type === "workflow") {
				const localWorkflowsFilePath = path.resolve(cwd, GlobalFileNames.workflows)

				const hasError = await ensureLocalAsiDirExists(localWorkflowsFilePath, "default-workflows.md")
				if (hasError === true) {
					return { filePath: null, fileExists: false }
				}
				...
				// Asirules file creation
				filePath = path.join(localAsiRulesFilePath, filename)
			}
		}
	...
	try {
		...
		if (isGlobal) {
			if (type === "workflow") {
				...
				const toggles = controller.stateManager.getGlobalSettingsKey("globalAsiRulesToggles")
				delete toggles[rulePath]
			...
			if (type === "workflow") {
				...
				const toggles = controller.stateManager.getWorkspaceStateKey("localAsiRulesToggles")
				delete toggles[rulePath]
				controller.stateManager.setWorkspaceState("localAsiRulesToggles", toggles)
			}
		}
