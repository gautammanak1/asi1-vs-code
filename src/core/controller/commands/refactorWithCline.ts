...
export async function refactorWithAsi(
	...
	if (!request.selectedText?.trim() && !notebookContext) {
		Logger.log("❌ No text selected and no notebook context (refactor)");
		HostProvider.window.showMessage({
			type: ShowMessageType.INFORMATION,
			message: "Please select some code to refactor.",
		});
		return {};
	}
	...
	let prompt: string;
	if (hasSelectedText) {
		prompt = resolveQuickActionPrompt("refactor", vars);
		if (editorSnapshot) {
		...
		prompt = resolveQuickActionPrompt("refactorNotebook", vars);
	}

	if (notebookContext) {
		Logger.log("Adding notebook context to refactorWithAsi task");
		prompt += `\n${notebookContext}`;
	}
...
export { refactorWithAsi as refactorWithCline };
