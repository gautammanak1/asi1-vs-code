import { HookInfo, HooksToggles, WorkspaceHooks } from "@shared/proto/Asi/file";
...
export async function refreshHooks(
	...
	const globalHooksDir =
		...
		path.join(os.homedir(), "Documents", "Asi", "Hooks");
	...
	for (const workspacePath of workspacePaths.paths) {
		const workspaceHooksDir = path.join(workspacePath, ".Asirules", "hooks");
...
async function isExecutable(filePath: string): Promise<boolean> {
	if (process.platform === "win32") {
		// Windows: treat existing hook file as enabled until JSON-backed state exists.
		return true;
	}
