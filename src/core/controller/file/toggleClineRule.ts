...
import type { ToggleAsiRuleRequest } from "@shared/proto/asi/file"
import { RuleScope, ToggleAsiRules } from "@shared/proto/asi/file"
 ...
 */
export async function toggleAsiRule(controller: Controller, request: ToggleAsiRuleRequest): Promise<ToggleAsiRules> {
	...
	if (controller.task?.ulid) {
		...
		const ruleFileName = getWorkspaceBasename(rulePath, "Controller.toggleAsiRule")
		const isGlobal = scope === RuleScope.GLOBAL
		telemetryService.captureAsiRuleToggled(controller.task.ulid, ruleFileName, enabled, isGlobal)
	}

	// Get the current state to return in the response
	const globalToggles = controller.stateManager.getGlobalSettingsKey("globalAsiRulesToggles")
	const localToggles = controller.stateManager.getWorkspaceStateKey("localAsiRulesToggles")
	const remoteToggles = controller.stateManager.getGlobalStateKey("remoteRulesToggles")

	return ToggleAsiRules.create({
