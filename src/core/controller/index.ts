...
import { AsiExtensionContext } from "@/shared/asi"
...
export class Controller {
	...
	constructor(readonly context: AsiExtensionContext) {
		...
		Logger.log("[Controller] AsiProvider instantiated")
	}
	...
	*/
	...
	async handleSignOut() {
		try {
			...
			const updatedConfig = {
				...apiConfiguration,
				planModeApiProvider: "openai" as ApiProvider,
				actModeApiProvider: "openai" as ApiProvider,
			}
			...
			HostProvider.window.showMessage({
				...
				message: "Successfully logged out of Asi",
			})
		} catch (_error) {
			HostProvider.window.showMessage({
				type: ShowMessageType.INFORMATION,
	...
	async cancelTask() {
		...
		try {
			...
			if (this.task) {
				// 'abandoned' will prevent this Asi instance from affecting future Asi instance gui. this may happen if its hanging on a streaming request
	...
	async handleAuthCallback(customToken: string, provider: string | null = null) {
		try {
			...
			if (planActSeparateModelsSetting) {
				...
				if (currentMode === "plan") {
					updatedConfig.planModeApiProvider = AsiProvider
				} else {
					updatedConfig.actModeApiProvider = AsiProvider
				}
			} else {
	...
	private async fetchMcpMarketplaceFromApi(): Promise<McpMarketplaceCatalog> {
		const response = await axios.get(`${AsiEnv.config().mcpBaseUrl}/marketplace`, {
			headers: {
	...
	async readOpenRouterModels(): Promise<Record<string, ModelInfo> | undefined> {
		...
		try {
			if (await fileExistsAtPath(openRouterModelsFilePath)) {
				...
				return appendAsiStealthModels(models)
			}
		} catch (error) {
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		const globalAsiRulesToggles = this.stateManager.getGlobalSettingsKey("globalAsiRulesToggles")
		...
		const localAsiRulesToggles = this.stateManager.getWorkspaceStateKey("localAsiRulesToggles")
		...
		const AsiMessages = [...(this.task?.messageStateHandler.getAsiMessages() || [])]
		...
		const AsiConfig = AsiEnv.config()
		const environment = AsiConfig.environment
		...
		return {
			...
			environment,
			distinctId,
			globalAsiRulesToggles: globalAsiRulesToggles || {},
			localAsiRulesToggles: localAsiRulesToggles || {},
			...
			AsiWebToolsEnabled: {
				user: this.stateManager.getGlobalSettingsKey("AsiWebToolsEnabled"),
	...
	/*
	Now that we use retainContextWhenHidden, we don't have to store a cache of Asi messages in the user's state, but we could to reduce memory footprint in long conversations.

	- We have to be careful of what state is shared between AsiProvider instances since there could be multiple instances of the extension running at once. For example when we cached Asi messages using the same key, two instances of the extension could end up using the same key and overwriting each other's messages.
	- Some state does need to be shared between the instances, i.e. the API key--however there doesn't seem to be a good way to notify the other instances that the API key has changed.

	We need to use a unique identifier for each AsiProvider instance's message cache since we could be running several instances of the extension outside of just the sidebar i.e. in editor panels.
