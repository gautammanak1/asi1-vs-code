...
async function fetchRawAsiModels(): Promise<AsiRawModelInfo[]> {
	...
	const response = await axios.get(
		`${apiBaseUrl}/api/v1/ai/asi/models`,
		getAxiosSettings(),
	);
...
export async function refreshClineModels(
	...
	const shouldUseAsiEndpointSource = featureFlagsService.getBooleanFlagEnabled(
		FeatureFlag.EXTENSION_Asi_MODELS_ENDPOINT,
	);
...
async function fetchAndCacheAsiModels(): Promise<Record<string, ModelInfo>> {
	const AsiModelsFilePath = path.join(
		await ensureCacheDirectoryExists(),
		GlobalFileNames.AsiModels,
	);
 ...
 */
export async function readAsiModelsFromCache(): Promise<
