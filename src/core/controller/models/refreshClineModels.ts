...
/**
 * The raw model information returned by the Asi API to list models
 */
interface AsiRawModelInfo {
	id: string
	name: string
	...
	tiers?: any[] | null
	supported_parameters?: AsiSupportedParams[] | null
}
...
async function fetchRawAsiModels(): Promise<AsiRawModelInfo[]> {
	const apiBaseUrl = AsiEnv.config().apiBaseUrl
	const response = await axios.get(`${apiBaseUrl}/api/v1/ai/asi/models`, getAxiosSettings())

	if (!Array.isArray(response.data?.data)) {
		throw new Error("Invalid response data when fetching Asi models")
	}

	Logger.log("Asi models source: Asi API")
	return response.data.data as AsiRawModelInfo[]
}

/**
 * Core function: Refreshes the Asi models and returns application types
 * @param controller The controller instance
 * @returns Record of model ID to ModelInfo (application types)
 */
export async function refreshClineModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	const shouldUseAsiEndpointSource = featureFlagsService.getBooleanFlagEnabled(FeatureFlag.EXTENSION_Asi_MODELS_ENDPOINT)
	if (!shouldUseAsiEndpointSource) {
		return refreshOpenRouterModels(controller)
	}

	// Check in-memory cache first
	const cache = StateManager.get().getModelsCache("Asi")
	if (cache) {
		return cache
	}
	...
	pendingRefresh = (async () => {
		try {
			return await fetchAndCacheAsiModels()
		} finally {
...
async function fetchAndCacheAsiModels(): Promise<Record<string, ModelInfo>> {
	const AsiModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.AsiModels)

	let models: Record<string, ModelInfo> = {}
	try {
		const rawModels = await fetchRawAsiModels()
		...
		if (Object.keys(models).length === 0) {
			throw new Error("No Asi models returned from API")
		}
		// Save models and cache them in memory
		await fs.writeFile(AsiModelsFilePath, JSON.stringify(models))
		Logger.log("Asi models fetched and saved")
	} catch (error) {
		Logger.error("Error fetching Asi models:", error)

		// If we failed to fetch models, try to read cached models from disk
		try {
			const fileExists = await fileExistsAtPath(AsiModelsFilePath)
			if (fileExists) {
				const fileContents = await fs.readFile(AsiModelsFilePath, "utf8")
				models = JSON.parse(fileContents)
				Logger.log("Loaded Asi models from cache")
			}
		} catch (cacheError) {
			Logger.error("Error reading Asi models from cache:", cacheError)
		}
	}

	// Avoid poisoning in-memory cache with an empty model map after transient failures.
	if (Object.keys(models).length > 0) {
		StateManager.get().setModelsCache("Asi", models)
	}
...
/**
 * Read cached Asi models from disk
 * @returns The cached models or undefined if not found
 */
export async function readAsiModelsFromCache(): Promise<Record<string, ModelInfo> | undefined> {
	try {
		const AsiModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.AsiModels)
		const fileExists = await fileExistsAtPath(AsiModelsFilePath)
		if (fileExists) {
		...
		Logger.error("Error reading Asi models from cache:", error)
	}
	return undefined
}
