...
let inMemoryCache: { data: AsiRecommendedModelsData; timestamp: number } | null = null
...
function useUpstreamRecommendedModels(): boolean {
	return featureFlagsService.getBooleanFlagEnabled(FeatureFlag.Asi_RECOMMENDED_MODELS_UPSTREAM)
}

function normalizeRecommendedModel(raw: unknown): AsiRecommendedModelData | null {
...
function normalizeRecommendedModelsResponse(raw: unknown): AsiRecommendedModelsData | null {
	if (!raw || typeof raw !== "object") {
		return null
	}
...
async function fetchAndCacheAsiRecommendedModels(): Promise<AsiRecommendedModelsData> {
	const AsiRecommendedModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.AsiRecommendedModels)
	let result: AsiRecommendedModelsData = { recommended: [], free: [] }

	try {
		const apiBaseUrl = AsiEnv.config().apiBaseUrl
		const response = await axios.get(`${apiBaseUrl}/api/v1/ai/asi/recommended-models`, getAxiosSettings())
		const normalized = normalizeRecommendedModelsResponse(response.data)
		if (!normalized) {
			throw new Error("Invalid response data when fetching Asi recommended models")
		}
		...
		try {
			const fileExists = await fs
				...
				.catch(() => false)
			if (fileExists) {
				const fileContents = await fs.readFile(AsiRecommendedModelsFilePath, "utf8")
			...
			Logger.error("Error reading Asi recommended models from cache:", cacheError)
		}
	}
