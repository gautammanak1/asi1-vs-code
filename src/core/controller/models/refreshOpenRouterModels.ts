...
async function fetchAndCacheModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	...
	const finalModels = appendAsiStealthModels(models)
 ...
 */
const Asi_STEALTH_MODELS: Record<string, ModelInfo> = {
	"stealth/giga-potato": {
		name: "Giga Potato",
		maxTokens: 8192,
...
export function appendAsiStealthModels(currentModels: Record<string, ModelInfo>): Record<string, ModelInfo> {
	// Create a shallow clone of the current models to avoid mutating the original object
	const cloned = { ...currentModels }
	for (const [modelId, modelInfo] of Object.entries(Asi_STEALTH_MODELS)) {
		if (!cloned[modelId]) {
