...
import { PlanActMode, McpDisplayMode as ProtoMcpDisplayMode, UpdateSettingsRequest } from "@shared/proto/asi/state"
...
export async function updateSettings(controller: Controller, request: UpdateSettingsRequest): Promise<Empty> {
	try {
		if (request.AsiEnv !== undefined) {
			AsiEnv.setEnvironment(request.AsiEnv)
		...
		// Update Asi web tools setting
		if (request.AsiWebToolsEnabled !== undefined) {
			if (controller.task) {
				telemetryService.captureAsiWebToolsToggle(controller.task.ulid, request.AsiWebToolsEnabled)
			}
			controller.stateManager.setGlobalState("AsiWebToolsEnabled", request.AsiWebToolsEnabled)
		}
		...
		if (request.focusChainSettings !== undefined) {
			{
				...
				const focusChainSettings = {
					enabled: isEnabled,
					remindAsiInterval: request.focusChainSettings.remindAsiInterval,
				}
