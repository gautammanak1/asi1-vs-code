...
import { PlanActMode, UpdateSettingsRequestCli } from "@shared/proto/asi/state"
...
export async function updateSettingsCli(controller: Controller, request: UpdateSettingsRequestCli): Promise<Empty> {
	...
	try {
		if (request.environment !== undefined) {
			AsiEnv.setEnvironment(request.environment)
		...
		if (request.settings) {
			...
			// Update Asi web tools setting (requires telemetry)
			if (AsiWebToolsEnabled !== undefined) {
				if (controller.task) {
					telemetryService.captureAsiWebToolsToggle(controller.task.ulid, AsiWebToolsEnabled)
				}
				controller.stateManager.setGlobalState("AsiWebToolsEnabled", AsiWebToolsEnabled)
			}
			...
			if (focusChainSettings !== undefined) {
				...
				const newFocusChainSettings = {
					enabled: isEnabled,
					remindAsiInterval: focusChainSettings.remindAsiInterval,
				}
