...
import { ExplainChangesRequest } from "@shared/proto/asi/task"
...
export async function explainChanges(controller: Controller, request: ExplainChangesRequest): Promise<Empty> {
	...
	try {
		...
		const messageIndex = AsiMessages.findIndex((m: any) => m.ts === request.messageTs)
		...
		const lastTaskCompletedMessageCheckpointHash = findLast(
			AsiMessages.slice(0, messageIndex),
			(m: any) => m.say === "completion_result",
		)?.lastCheckpointHash

		const firstCheckpointMessageCheckpointHash = AsiMessages.find(
