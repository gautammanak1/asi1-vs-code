...
describe("TaskCancel Hook", () => {
	...
	describe("Hook Input Format", () => {
		it("should receive task metadata with completionStatus", async () => {
			const hookPath = path.join(tempDir, ".Asirules", "hooks", "TaskCancel");
...
console.log(JSON.stringify({
  ...
  errorMessage: ""
			...
			result1.contextModification?.should.equal(
				"This is a context modification that should be ignored",
			);
...
console.log(JSON.stringify({
  ...
  errorMessage: ""
			...
			result2.contextModification?.should.equal(
				"Different context that is also ignored",
			);
