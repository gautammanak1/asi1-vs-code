...
describe("TaskStart Hook", () => {
	...
	describe("Hook Input Format", () => {
		it("should receive task metadata from startTask", async () => {
			const hookPath = path.join(tempDir, ".Asirules", "hooks", "TaskStart");
...
console.log(JSON.stringify({
  ...
  errorMessage: ""
			...
			result.contextModification?.should.equal(
				"TaskStart hook executed successfully",
			);
		});
...
console.log(JSON.stringify({
  ...
  errorMessage: ""
			...
			result.cancel.should.be.false();
			result.contextModification?.should.equal(
				"TASK_START: Task 'Build a todo app' beginning",
			);
		});
	});
		...
		it("should validate representative fixtures end-to-end", async () => {
			...
			for (const scenario of scenarios) {
				await withFixtureRunner(
					"TaskStart",
					`hooks/taskstart/${scenario.fixtureName}`,
					hookTestEnv,
					async (runner) => {
						const result = await runner.run({
							taskId: "test-task-id",
