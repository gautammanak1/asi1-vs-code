 ...
 */
export function extractTokenUsageFromMessage(message: AsiMessage | undefined): TokenUsage {
...
export async function writePreCompactContextFiles(
	taskId: string,
	currentContext: AsiStorageMessage[],
...
export interface PreCompactHookParams {
	...
	/** Asi messages for extracting token usage */
...
export async function executePreCompactHookWithCleanup(params: PreCompactHookParams): Promise<PreCompactHookResult> {
	...
	try {
		...
		const previousApiReqIndex = findLastIndex(params.AsiMessages, (m) => m.say === "api_req_started")
		const previousRequest = previousApiReqIndex !== -1 ? params.AsiMessages[previousApiReqIndex] : undefined
		...
		if (preCompactResult.cancel === true) {
			...
			await params.messageStateHandler.saveAsiMessagesAndUpdateHistory()
