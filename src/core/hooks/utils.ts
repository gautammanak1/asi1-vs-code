...
/**
 * All valid hook types that can be created and executed by Asi.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Asi", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".Asirules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".Asirules", "hooks")
}
