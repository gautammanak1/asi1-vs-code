...
export const formatResponse = {
	...
	AsiIgnoreError: (path: string) =>
		`Access to ${path} is blocked by the .Asiignore file settings. You must try to continue in the task without using this file, or ask the user to update the .Asiignore file.`,

	permissionDeniedError: (reason: string) =>
		`Command execution blocked by Asi_COMMAND_PERMISSIONS: ${reason}. You must try a different approach or ask the user to update the permission settings.`,
...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	formatFilesList: (
		...
		const sorted = files
			...
			// Sort so files are listed under their respective directories to make it clear what files are children of what directories. Since we build file list top down, even if file list is truncated it will show directories that Asi can then explore further.
			.sort((a, b) => {
		...
		const AsiIgnoreParsed = ClineIgnoreController
			? sorted.map((filePath) => {
		...
		if (didHitLimit) {
			return `${AsiIgnoreParsed.join(
				"\n",
			)}\n\n(File list truncated. Use list_files on specific subdirectories if you need to explore further.)`
		}
		if (AsiIgnoreParsed.length === 0 || (AsiIgnoreParsed.length === 1 && AsiIgnoreParsed[0] === "")) {
...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
	...
	AsiIgnoreInstructions: (content: string) =>
		`# .Asiignore\n\n(The following is provided by a root-level .Asiignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${content}\n.Asiignore`,

	AsiRulesGlobalDirectoryInstructions: (globalAsiRulesFilePath: string, content: string) =>
		`# .Asirules/\n\nThe following is provided by a global .Asirules/ directory, located at ${globalAsiRulesFilePath.toPosix()}, where the user has specified instructions for all working directories:\n\n${content}`,

	AsiRulesLocalDirectoryInstructions: (cwd: string, content: string) =>
		`# .Asirules/\n\nThe following is provided by a root-level .Asirules/ directory where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,

	AsiRulesLocalFileInstructions: (cwd: string, content: string) =>
		`# .Asirules\n\nThe following is provided by a root-level .Asirules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,
