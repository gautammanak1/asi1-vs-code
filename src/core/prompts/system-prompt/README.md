...
├── registry/
│   ├── AsiToolSet.ts            # Tool set management & registry
...
interface PromptVariant {
  ...
  // Tool configuration
  tools?: AsiDefaultTool[];                    // Ordered list of tools to include
  toolOverrides?: { [K in AsiDefaultTool]?: ConfigOverride }; // Tool-specific customizations
}
...
```markdown
You are Asi, a highly skilled software engineer...
...
Tools are managed through the `AsiToolSet` and can be configured per variant:

```typescript
class AsiToolSet {
  private static variants: Map<ModelFamily, Set<AsiToolSet>> = new Map();

  static register(config: AsiToolSpec): AsiToolSet {
    return new AsiToolSet(config.id, config);
  }

  static getTools(variant: ModelFamily): AsiToolSet[] {
...
public static async getToolsPrompts(variant: PromptVariant, context: SystemPromptContext) {
  const tools = AsiToolSet.getTools(variant.family);
...
export const config = createVariant(ModelFamily.NEXT_GEN)
  ...
  .tools(
    ...
    AsiDefaultTool.LIST_CODE_DEF,
    AsiDefaultTool.BROWSER,
    AsiDefaultTool.WEB_FETCH,  // Additional tool for next-gen
    AsiDefaultTool.MCP_USE,
    AsiDefaultTool.MCP_ACCESS,
    AsiDefaultTool.ASK,
    AsiDefaultTool.ATTEMPT,
...
const config = createVariant(ModelFamily.GENERIC)
  ...
  .overrideTool(AsiDefaultTool.BASH, {          // Optional, tool overrides
    enabled: false,
  })
...
The system supports the following tools (mapped to `AsiDefaultTool` enum):
...
Each tool in Asi follows a specific structure with variants for different model families. Here's the anatomy of a tool:
...
const id = AsiDefaultTool.MY_NEW_TOOL // Add to enum first
...
const generic: AsiToolSpec = {
	...
	parameters: [
		...
		},
		{
			...
			dependencies: [AsiDefaultTool.SOME_OTHER_TOOL], // Only show if dependency exists
		},
	],
}
...
First, add your tool ID to the `AsiDefaultTool` enum:

```typescript
// src/shared/tools.ts
export enum AsiDefaultTool {
	// ... existing tools
...
const id = AsiDefaultTool.MY_NEW_TOOL
...
export function registerAsiToolSets(): void {
...
```typescript
// In your tool handler class (e.g., AsiProvider)
...
```typescript
const contextAwareTool: AsiToolSpec = {
	variant: ModelFamily.GENERIC,
	id: AsiDefaultTool.CONTEXT_TOOL,
	name: "context_tool",
...
```typescript
const claude: AsiToolSpec = {
	variant: ModelFamily.GENERIC,
	id: AsiDefaultTool.MODEL_SPECIFIC_TOOL,
...
```typescript
const dependentTool: AsiToolSpec = {
	variant: ModelFamily.GENERIC,
	id: AsiDefaultTool.DEPENDENT_TOOL,
	name: "dependent_tool",
...
const id = AsiDefaultTool.ANALYZE_FILE
...
This comprehensive guide should help developers understand both the architecture and practical steps needed to extend Asi with new tools.
