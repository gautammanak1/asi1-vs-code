...
export interface AsiToolSpec {
	...
	parameters?: Array<AsiToolSpecParameter>
}

interface AsiToolSpecParameter {
	name: string
	required: boolean
	instruction: string | ((context: SystemPromptContext) => string)
	usage?: string
	dependencies?: AsiDefaultTool[]
	description?: string
	contextRequirements?: (context: SystemPromptContext) => boolean
...
/**
 * Converts a AsiToolSpec into an OpenAI ChatCompletionTool definition
 * Docs: https://openrouter.ai/docs/features/tool-calling#step-1-inference-request-with-tools
 */
export function toolSpecFunctionDefinition(tool: AsiToolSpec, context: SystemPromptContext): OpenAITool {
...
/**
 * Converts a AsiToolSpec into an Anthropic Tool definition
 */
export function toolSpecInputSchema(tool: AsiToolSpec, context: SystemPromptContext): AnthropicTool {
	// Check if the tool should be included based on context requirements
	if (tool.contextRequirements && !tool.contextRequirements(context)) {
		throw new Error(`Tool ${tool.name} does not meet context requirements`)
	}
...
/**
 * Converts a AsiToolSpec into a Google Gemini function.
 * Docs: https://ai.google.dev/gemini-api/docs/function-calling
 */
export function toolSpecFunctionDeclarations(tool: AsiToolSpec, context: SystemPromptContext): GoogleTool {
	// Check if the tool should be included based on context requirements
	if (tool.contextRequirements && !tool.contextRequirements(context)) {
		throw new Error(`Tool ${tool.name} does not meet context requirements`)
	}
