...
This directory contains the tool registration system for Asi tools. The system automatically collects and registers all tool variants with the `AsiToolSet` provider.

## Overview

Each tool file in this directory exports a `{toolName}_variants` array containing tool specifications for different prompt variants (e.g., Claude, GPT). The registration system automatically imports all these variants and registers them with the `AsiToolSet` provider.
...
```typescript
import { AsiToolSet } from "../registry/AsiToolSet";
import { PromptVariant } from "@/shared/tools";

// Get all tools for a specific variant
const claudeTools = AsiToolSet.getTools(PromptVariant.CLAUDE);

// Get a specific tool by name
const writeToFileTool = AsiToolSet.getToolByName("write_to_file", PromptVariant.CLAUDE);
```
...
```typescript
import { AsiDefaultTool, PromptVariant, type AsiToolSpec } from "@/shared/tools";
