...
export async function parseSlashCommands(
	...
	for (const { regex } of tagPatterns) {
		...
		if (tagMatch) {
			...
			if (SUPPORTED_DEFAULT_COMMANDS.includes(commandName)) {
				...
				return { processedText: processedText, needsAsirulesFileCheck: commandName === "newrule" }
			}
				...
				if (parts.length >= 3) {
					...
					try {
						...
						if (promptResponse) {
							...
							return { processedText, needsAsirulesFileCheck: false }
						}
	...
	return { processedText: text, needsAsirulesFileCheck: false }
}
