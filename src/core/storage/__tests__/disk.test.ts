...
describe("disk - hooks functionality", () => {
	...
	describe("getWorkspaceHooksDirs", () => {
		...
		it("should return hooks directory when it exists", async () => {
			...
			const workspaceRoot = path.join(tempDir, "workspace1")
			const hooksDir = path.join(workspaceRoot, ".Asirules", "hooks")
		...
		it("should not return hooks directory if it's a file instead of directory", async () => {
			...
			const workspaceRoot = path.join(tempDir, "workspace1")
			const hooksPath = path.join(workspaceRoot, ".Asirules", "hooks")
		...
		it("should return multiple hooks directories for multi-root workspace", async () => {
			...
			const hooksDir1 = path.join(workspaceRoot1, ".Asirules", "hooks")
			const hooksDir2 = path.join(workspaceRoot2, ".Asirules", "hooks")
		...
		it("should return only existing hooks directories in multi-root workspace", async () => {
			...
			const hooksDir1 = path.join(workspaceRoot1, ".Asirules", "hooks")
			const hooksDir3 = path.join(workspaceRoot3, ".Asirules", "hooks")
			...
			result.should.not.containEql(path.join(workspaceRoot2, ".Asirules", "hooks"))
		})
		...
		it("should use correct path joining for hooks directory", async () => {
			const workspaceRoot = path.join(tempDir, "workspace1")
			const expectedHooksDir = path.join(workspaceRoot, ".Asirules", "hooks")
			...
			result[0].should.match(/\.Asirules[\\/]hooks$/)
		})
...
describe("disk - atomic writes", () => {
	...
	before(async () => {
		...
		testGlobalStorageDir = path.join(os.tmpdir(), `Asi-test-storage-${Date.now()}-${Math.random().toString(36).slice(2)}`)
