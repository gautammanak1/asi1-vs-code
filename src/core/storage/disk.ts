...
export const GlobalFileNames = {
	...
	AsiRecommendedModels: "Asi_recommended_models.json",
	AsiModels: "Asi_models.json",
	openRouterModels: "openrouter_models.json",
	vercelAiGatewayModels: "vercel_ai_gateway_models.json",
	groqModels: "groq_models.json",
	basetenModels: "baseten_models.json",
	hicapModels: "hicap_models.json",
	mcpSettings: "Asi_mcp_settings.json",
	AsiRules: ".Asirules",
	workflows: ".Asirules/workflows",
	hooksDir: ".Asirules/hooks",
	AsiruleSkillsDir: ".Asirules/skills",
	AsiSkillsDir: ".Asi/skills",
...
/**
 * Returns the cross-platform path to the Asi home directory (~/.Asi).
 * This works on macOS, Linux, and Windows:
 * - macOS: /Users/username/.Asi
 * - Linux: /home/username/.Asi
 * - Windows: C:\Users\username\.Asi
 *
 * This is intended to eventually replace ~/Documents/Asi as the global config location.
 */
export function getAsiHomePath(): string {
	return path.join(os.homedir(), ".Asi")
}
...
export async function ensureRulesDirectoryExists(): Promise<string> {
	...
	const AsiRulesDir = path.join(userDocumentsPath, "Asi", "Rules")
	try {
		await fs.mkdir(AsiRulesDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Asi", "Rules") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return AsiRulesDir
}
...
export async function ensureWorkflowsDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const AsiWorkflowsDir = path.join(userDocumentsPath, "Asi", "Workflows")
	try {
		await fs.mkdir(AsiWorkflowsDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Asi", "Workflows") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return AsiWorkflowsDir
}

export async function ensureMcpServersDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const mcpServersDir = path.join(userDocumentsPath, "Asi", "MCP")
	try {
		await fs.mkdir(mcpServersDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Asi", "MCP") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine since this path is only ever used in the system prompt
	}
...
export async function ensureHooksDirectoryExists(): Promise<string> {
	const userDocumentsPath = await getDocumentsPath()
	const AsiHooksDir = path.join(userDocumentsPath, "Asi", "Hooks")
	try {
		await fs.mkdir(AsiHooksDir, { recursive: true })
	} catch (_error) {
		return path.join(os.homedir(), "Documents", "Asi", "Hooks") // in case creating a directory in documents fails for whatever reason (e.g. permissions) - this is fine because we will fail gracefully with a path that does not exist
	}
	return AsiHooksDir
}

/**
 * Returns the global skills directory path (~/.Asi/skills) without creating it.
 */
function getAsiSkillsDirectoryPath(): string {
	return path.join(getAsiHomePath(), "skills")
}
...
export function getSkillsDirectoriesForScan(cwd: string): SkillsScanDirectory[] {
	return [
		{ path: path.join(cwd, GlobalFileNames.AsiruleSkillsDir), source: "project" },
		{ path: path.join(cwd, GlobalFileNames.AsiSkillsDir), source: "project" },
		...
		{ path: getAsiSkillsDirectoryPath(), source: "global" },
...
export async function getSavedAsiMessages(taskId: string): Promise<AsiMessage[]> {
...
export async function saveAsiMessages(taskId: string, uiMessages: AsiMessage[]) {
	try {
...
export async function collectEnvironmentMetadata(): Promise<Omit<EnvironmentMetadataEntry, "ts">> {
	try {
		...
		return {
			...
			Asi_version: ExtensionRegistryInfo.version,
		}
	} catch (error) {
 ...
 * 3. Each workspace root's .Asirules/hooks directory (if they exist)
 *
...
/**
 * Gets the paths to the workspace's .Asirules/hooks directories to search for
	...
	return (
		await Promise.all(
			workspaceRootPaths.map(async (workspaceRootPath) => {
				// Look for a .Asirules/hooks folder in this workspace root.
