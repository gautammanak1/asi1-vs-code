...
export async function migrateMcpMarketplaceEnableSetting(mcpMarketplaceEnabledRaw: boolean | undefined): Promise<boolean> {
	const config = vscode.workspace.getConfiguration("Asi")
...
export async function migrateCustomInstructionsToGlobalRules(context: vscode.ExtensionContext) {
	try {
		...
		if (customInstructions?.trim()) {
			Logger.log("Migrating custom instructions to global Asi rules...")

			// Create global .Asirules directory if it doesn't exist
			...
			Logger.log("Successfully migrated custom instructions to global Asi rules")
		}
	} catch (error) {
...
export async function migrateWelcomeViewCompleted(context: vscode.ExtensionContext) {
	try {
		...
		if (welcomeViewCompleted === undefined) {
			...
			const AsiAccountId = await context.secrets.get("AsiAccountId")
...
export async function cleanupOldApiKey(context: vscode.ExtensionContext) {
	try {
		...
		await context.secrets.delete("AsiApiKey")
	} catch (error) {
		Logger.error("Failed to cleanup old AsiApiKey", error)
	}
}
