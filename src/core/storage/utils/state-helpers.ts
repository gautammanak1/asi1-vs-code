...
import type { AsiFileStorage } from "@shared/storage/ClineFileStorage"
import {
    applyTransform,
...
import { AsiMemento } from "@/shared/storage"
...
/**
 * Read secrets from a AsiFileStorage instance.
 */
export function readSecretsFromStorage(store: AsiFileStorage<string>): Secrets {
...
/**
 * Read workspace state from a AsiFileStorage instance.
 */
export function readWorkspaceStateFromStorage(store: AsiFileStorage): LocalState {
...
/**
 * Read global state from a AsiFileStorage instance.
 */
export async function readGlobalStateFromStorage(store: AsiMemento): Promise<GlobalStateAndSettings> {
	try {
...
async function handleComputedProperties(result: any, stateValues: Map<string, any>): Promise<void> {
	...
	const defaultApiProvider: ApiProvider = "openai"
