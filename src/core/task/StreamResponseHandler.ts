...
export interface PendingReasoning {
	...
	content: string
	signature: string
	redactedThinking: AsiAssistantRedactedThinkingBlock[]
...
/**
 * Handles streaming native tool use blocks and converts them to AsiAssistantToolUseBlock format
 */
class ToolUseHandler {
	...
	getFinalizedToolUse(id: string): AsiAssistantToolUseBlock | undefined {
	...
	getAllFinalizedToolUses(summary?: AsiAssistantToolUseBlock["reasoning_details"]): AsiAssistantToolUseBlock[] {
		const results: AsiAssistantToolUseBlock[] = []
	...
	getPartialToolUsesAsContent(): ToolUse[] {
		...
		for (const pending of pendingToolUses) {
			...
			if (pending.name.includes(Asi_MCP_TOOL_IDENTIFIER)) {
				const [key, toolName] = pending.name.split(Asi_MCP_TOOL_IDENTIFIER)
				results.push({
					type: "tool_use",
					name: AsiDefaultTool.MCP_USE,
					params: {
				...
				results.push({
					type: "tool_use",
					name: pending.name as AsiDefaultTool,
...
class ReasoningHandler {
	...
	getCurrentReasoning(): AsiAssistantThinkingBlock | null {
		if (!this.pendingReasoning) {
			return null
		}
	...
	getRedactedThinking(): AsiAssistantRedactedThinkingBlock[] {
