...
import { AsiAsk, AsiSay } from "@shared/ExtensionMessage"
import { AsiContent } from "@shared/messages/content"
import { AsiDefaultTool, toolUseNames } from "@shared/tools"
...
export function canonicalizeAttemptCompletionParams(block: ToolUse): boolean {
	if (block.name === AsiDefaultTool.ATTEMPT && !block.params?.result && typeof block.params?.response === "string") {
...
export class ToolExecutor {
	...
	private shouldAutoApproveTool(toolName: AsiDefaultTool): boolean | [boolean, boolean] {
		return this.autoApprover.shouldAutoApproveTool(toolName)
	}

	private async shouldAutoApproveToolWithPath(
		blockname: AsiDefaultTool,
	...
	constructor(
		...
		private sayAndCreateMissingParamError: (toolName: AsiDefaultTool, paramName: string, relPath?: string) => Promise<any>,
		private removeLastPartialMessageIfExistsWithType: (type: "ask" | "say", askOrSay: AsiAsk | AsiSay) => Promise<void>,
	 ...
	 */
	private static readonly PLAN_MODE_RESTRICTED_TOOLS: AsiDefaultTool[] = [
		AsiDefaultTool.FILE_NEW,
		AsiDefaultTool.FILE_EDIT,
		AsiDefaultTool.NEW_RULE,
		AsiDefaultTool.APPLY_PATCH,
	]
	 ...
	 */
	private isPlanModeToolRestricted(toolName: AsiDefaultTool): boolean {
