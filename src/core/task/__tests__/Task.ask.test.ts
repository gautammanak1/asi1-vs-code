...
function createFakeTask(taskState: {
	...
	const fakeTask = {
		...
		taskId: "task-1",
		messageStateHandler: {
			addToAsiMessages: async (message: AsiMessage) => {
				AsiMessages.push(message)
			},
...
describe("Task.ask", () => {
	it("keeps resume asks waiting for a user response even when the task is aborted", async () => {
		...
		const { AsiMessages, fakeTask } = createFakeTask(taskState)
		...
		try {
			...
			assert.equal(AsiMessages.length, 1)
			assert.equal(AsiMessages[0].ask, "resume_task")
	...
	it("keeps resume-completed asks waiting for a user response even when the task is aborted", async () => {
		...
		try {
			...
			await flushMicrotasks()
			assert.equal(AsiMessages.length, 1)
			assert.equal(AsiMessages[0].ask, "resume_completed_task")
			assert.notEqual(taskState.lastMessageTs, undefined)
	...
	it("still wakes non-resume asks when abort is triggered after the ask is shown", async () => {
		...
		try {
			...
			await flushMicrotasks()
			assert.equal(AsiMessages.length, 1)
			assert.equal(AsiMessages[0].ask, "completion_result")

			const rejectionPromise = assert.rejects(askPromise, /Asi instance aborted/)
