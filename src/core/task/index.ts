...
import { AsiApiReqCancelReason, AsiApiReqInfo, AsiAsk, AsiMessage, AsiSay } from "@shared/ExtensionMessage"
...
export class Task {
	...
	constructor(params: TaskParams) {
		...
		const effectiveApiConfiguration: ApiConfiguration = {
			...
			onRetryAttempt: async (attempt: number, maxRetries: number, delay: number, error: any) => {
				...
				const lastApiReqStartedIndex = findLastIndex(AsiMessages, (m) => m.say === "api_req_started")
				if (lastApiReqStartedIndex !== -1) {
					try {
						const currentApiReqInfo: AsiApiReqInfo = JSON.parse(AsiMessages[lastApiReqStartedIndex].text || "{}")
						...
						await this.messageStateHandler.updateAsiMessage(lastApiReqStartedIndex, {
		...
		const commandExecutorCallbacks: CommandExecutorCallbacks = {
			...
			updateAsiMessage: async (index: number, updates: { commandCompleted?: boolean; text?: string }) => {
				await this.messageStateHandler.updateAsiMessage(index, updates)
			},
			getAsiMessages: () => this.messageStateHandler.getAsiMessages() as Array<{ ask?: string; say?: string }>,
			addToUserMessageContent: (content: { type: string; text: string }) => {
				// Cast to AsiTextContentBlock which is compatible with AsiContent
				this.taskState.userMessageContent.push({ type: "text", text: content.text } as AsiTextContentBlock)
			},
		}
	...
	async handleWebviewAskResponse(askResponse: AsiAskResponse, text?: string, images?: string[], files?: string[]) {
	...
	async sayAndCreateMissingParamError(toolName: AsiDefaultTool, paramName: string, relPath?: string) {
		await this.say(
			"error",
	...
	async removeLastPartialMessageIfExistsWithType(type: "ask" | "say", askOrSay: AsiAsk | AsiSay) {
		const AsiMessages = this.messageStateHandler.getAsiMessages()
		const lastMessage = AsiMessages.at(-1)
	 ...
	 */
	private calculatePreCompactDeletedRange(apiConversationHistory: AsiStorageMessage[]): [number, number] {
	...
	public async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		...
		const imageBlocks: AsiImageContentBlock[] = formatResponse.imageBlocks(images)
	...
	public async resumeTaskFromHistory() {
		...
		const lastApiReqStartedIndex = findLastIndex(savedAsiMessages, (m) => m.type === "say" && m.say === "api_req_started")
		if (lastApiReqStartedIndex !== -1) {
			const lastApiReqStarted = savedAsiMessages[lastApiReqStartedIndex]
			const { cost, cancelReason }: AsiApiReqInfo = JSON.parse(lastApiReqStarted.text || "{}")
		...
		this.messageStateHandler.setAsiMessages(await getSavedAsiMessages(this.taskId))
		...
		if (existingApiConversationHistory.length > 0) {
			...
			if (lastMessage.role === "assistant") {
				...
				const existingUserContent: AsiContent[] = Array.isArray(lastMessage.content)
					? lastMessage.content
		...
		const wasRecent = lastAsiMessage?.ts && Date.now() - lastAsiMessage.ts < 30_000
	...
	private async initiateTaskLoop(userContent: AsiContent[]): Promise<void> {
		...
		while (!this.taskState.abort) {
			const didEndLoop = await this.recursivelyMakeAsiRequests(nextUserContent, includeFileDetails)
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const { globalToggles, localToggles } = await refreshAsiRulesToggles(this.controller, this.cwd)
		...
		const globalRules = await getGlobalAsiRules(globalAsiRulesFilePath, globalToggles, { evaluationContext })
		...
		const localRules = await getLocalAsiRules(this.cwd, localToggles, { evaluationContext })
		...
		if (AsiIgnoreContent) {
			AsiIgnoreInstructions = formatResponse.AsiIgnoreInstructions(AsiIgnoreContent)
		}
		...
		const promptContext: SystemPromptContext = {
			...
			AsiWebToolsEnabled:
				this.stateManager.getGlobalSettingsKey("AsiWebToolsEnabled") && featureFlagsService.getWebtoolsEnabled(),
		...
		try {
			...
			const asiError = ErrorService.get().toAsiError(error, model.id, providerId)
			...
			if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
				...
				if (isContextWindowExceededError) {
					...
					if (truncatedConversationHistory.length > 3) {
						asiError.message = "Context window exceeded. Click retry to truncate the conversation and try again."
				...
				if (!isAsiProviderInsufficientCredits && !isAuthError && this.taskState.autoRetryAttempts < 3) {
					...
					if (autoRetryApiReqIndex !== -1) {
						const AsiMessages = this.messageStateHandler.getAsiMessages()
						const currentApiReqInfo: AsiApiReqInfo = JSON.parse(AsiMessages[autoRetryApiReqIndex].text || "{}")
						delete currentApiReqInfo.streamingFailedMessage
						await this.messageStateHandler.updateAsiMessage(autoRetryApiReqIndex, {
							text: JSON.stringify(currentApiReqInfo),
						})
					}
				...
				if (manualRetryApiReqIndex !== -1) {
					const AsiMessages = this.messageStateHandler.getAsiMessages()
					const currentApiReqInfo: AsiApiReqInfo = JSON.parse(AsiMessages[manualRetryApiReqIndex].text || "{}")
					delete currentApiReqInfo.streamingFailedMessage
					await this.messageStateHandler.updateAsiMessage(manualRetryApiReqIndex, {
						text: JSON.stringify(currentApiReqInfo),
					})
				}
	...
	async recursivelyMakeAsiRequests(userContent: AsiContent[], includeFileDetails = false): Promise<boolean> {
		...
		if (this.taskState.consecutiveMistakeCount >= this.stateManager.getGlobalSettingsKey("maxConsecutiveMistakes")) {
			...
			if (autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					subtitle: "Error",
					message: "Asi is having trouble. Would you like to continue the task?",
				})
			}
		...
		const previousApiReqIndex = findLastIndex(this.messageStateHandler.getAsiMessages(), (m) => m.say === "api_req_started")

		// Save checkpoint if this is the first API request
		const isFirstRequest = this.messageStateHandler.getAsiMessages().filter((m) => m.say === "api_req_started").length === 0
			...
			if (lastCheckpointMessageIndex !== -1) {
				...
				commitPromise
					?.then(async (commitHash) => {
						if (commitHash) {
							await this.messageStateHandler.updateAsiMessage(lastCheckpointMessageIndex, {
		...
		if (shouldCompact) {
			...
			;[parsedUserContent, environmentDetails, AsirulesError] = await this.loadContext(
				userContent,
				includeFileDetails,
				useCompactPrompt,
			)
		...
		const lastApiReqIndex = findLastIndex(this.messageStateHandler.getAsiMessages(), (m) => m.say === "api_req_started")
		...
		try {
			...
			*/
			...
			const finalizeApiReqMsg = async (cancelReason?: AsiApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			const abortStream = async (cancelReason: AsiApiReqCancelReason, streamingFailedMessage?: string) => {
			...
			const finalizePendingReasoningMessage = async (thinking: string): Promise<boolean> => {
				...
				const completedReasoning = this.messageStateHandler.getAsiMessages()[pendingReasoningIndex]
				if (completedReasoning) {
					await sendPartialMessageEvent(convertAsiMessageToProto(completedReasoning))
				}
				return true
			}
			...
			try {
				...
				if (!this.taskState.abandoned) {
					const asiError = ErrorService.get().toAsiError(error, this.api.getModel().id)
			...
			if (assistantHasContent) {
				...
				const recDidEndLoop = await this.recursivelyMakeAsiRequests(this.taskState.userMessageContent)
	...
	async loadContext(
		...
		const parseTextBlock = async (text: string): Promise<string> => {
			...
			const { processedText, needsAsirulesFileCheck: needsCheck } = await parseSlashCommands(
				parsedText,
		...
		const processTextContent = async (block: AsiTextContentBlock): Promise<AsiTextContentBlock> => {
		...
		const processContentBlock = async (block: AsiContent): Promise<AsiContent> => {
			if (block.type === "text") {
	...
	async getEnvironmentDetails(includeFileDetails = false) {
		...
		const AsiMessages = this.messageStateHandler.getAsiMessages()
		const modifiedMessages = combineApiRequests(combineCommandSequences(AsiMessages.slice(1)))
