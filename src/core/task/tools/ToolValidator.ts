...
export class ToolValidator {
	...
	/**
	 * Verifies access is allowed to a given path via .Asiignore rules.
	 * Callers should pass a repo-relative (workspace-relative) path.
	 */
	checkAsiIgnorePath(relPath: string): ValidationResult {
		const accessAllowed = this.ClineIgnoreController.validateAccess(relPath)
		if (!accessAllowed) {
			return {
				ok: false,
				error: `Access to path '${relPath}' is blocked by .Asiignore settings.`,
			}
		}
		return { ok: true }
	}
