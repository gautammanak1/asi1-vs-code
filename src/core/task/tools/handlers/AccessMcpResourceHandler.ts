...
import { AsiAsk, AsiAskUseMcpServer } from "@shared/ExtensionMessage"
...
export class AccessMcpResourceHandler implements IFullyManagedTool {
	readonly name = AsiDefaultTool.MCP_ACCESS
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		if (shouldAutoApprove) {
			...
			await uiHelpers.ask("use_mcp_server" as AsiAsk, partialMessage, block.partial).catch(() => {})
		}
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!server_name) {
			...
			return await config.callbacks.sayAndCreateMissingParamError(AsiDefaultTool.MCP_ACCESS, "server_name")
		}

		if (!uri) {
			config.taskState.consecutiveMistakeCount++
			return await config.callbacks.sayAndCreateMissingParamError(AsiDefaultTool.MCP_ACCESS, "uri")
		}
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `Asi wants to access ${uri || "unknown resource"} on ${server_name || "unknown server"}`
