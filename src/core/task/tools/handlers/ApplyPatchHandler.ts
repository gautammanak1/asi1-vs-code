...
export class ApplyPatchHandler implements IFullyManagedTool {
	readonly name = AsiDefaultTool.APPLY_PATCH
	...
	private async previewPatchStream(rawInput: string, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		await config.callbacks
			.ask(
				"tool",
				JSON.stringify({
					tool: PatchAsiSayMap[actionType],
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			for (const changedFilePath of changedFiles) {
				...
				config.services.fileContextTracker.markFileAsEditedByAsi(pathToTrack)
				await config.services.fileContextTracker.trackFileContext(pathToTrack, "Asi_edited")
	...
	private async generateChangeSummary(changes: Record<string, FileChange>): Promise<AsiSayTool[]> {
	...
	private async handleApproval(
		...
		showNotificationForApproval(`Asi wants to edit '${message.path}'`, config.autoApprovalSettings.enableNotifications)
