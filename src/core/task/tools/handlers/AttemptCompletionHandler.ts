...
export class AttemptCompletionHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = AsiDefaultTool.ATTEMPT
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const addNewChangesFlagToLastCompletionResultMessage = async () => {
			...
			const AsiMessages = config.messageState.getAsiMessages()

			const lastCompletionResultMessageIndex = findLastIndex(AsiMessages, (m: any) => m.say === "completion_result")
			const lastCompletionResultMessage =
				lastCompletionResultMessageIndex !== -1 ? AsiMessages[lastCompletionResultMessageIndex] : undefined
			if (
				...
				await config.messageState.updateAsiMessage(lastCompletionResultMessageIndex, {
		...
		if (partialCompletionIndex !== -1) {
			const updatedMessages = [
				...AsiMessages.slice(0, partialCompletionIndex),
				...AsiMessages.slice(partialCompletionIndex + 1),
			]
			config.messageState.setAsiMessages(updatedMessages)
			await config.messageState.saveAsiMessagesAndUpdateHistory()
		}

		let commandResult: any
		const lastMessage = config.messageState.getAsiMessages().at(-1)
		...
		if (command) {
			...
			const autoApproveResult = config.autoApprover?.shouldAutoApproveTool(AsiDefaultTool.BASH)
			...
			if (autoApproveSafe) {
				...
				showNotificationForApproval(
					`Asi wants to execute a command: ${command}`,
		...
		if (config.messageState.getAsiMessages().at(-1)?.ask === "command_output") {
