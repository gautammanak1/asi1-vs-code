...
import { AsiAsk } from "@shared/ExtensionMessage"
...
export class ExecuteCommandToolHandler implements IFullyManagedTool {
	readonly name = AsiDefaultTool.BASH
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		await uiHelpers
			.ask("command" as AsiAsk, uiHelpers.removeClosingTag(block, "command", command), block.partial)
			.catch(() => {})
	}
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (!command) {
			...
			await config.callbacks.say(
				"error",
				"Asi tried to use execute_command without value for required parameter 'command'. Retrying...",
			)
		...
		// Check command permission validation (Asi_COMMAND_PERMISSIONS env var)
		...
		if (!permissionResult.allowed) {
			...
			if (permissionResult.failedSegment) {
				errorMessage =
					`Command "${actualCommand}" was denied by Asi_COMMAND_PERMISSIONS. ` +
		...
		// Check Asiignore validation for command
		const ignoredFileAttemptedToAccess = config.services.ClineIgnoreController.validateCommand(actualCommand)
		if (ignoredFileAttemptedToAccess) {
			if (!config.isSubagentExecution) {
				await config.callbacks.say("Asiignore_error", ignoredFileAttemptedToAccess)
			}
			return formatResponse.toolError(formatResponse.AsiIgnoreError(ignoredFileAttemptedToAccess))
		}
