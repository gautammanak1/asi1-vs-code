...
import type { AsiSayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified AsiSayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: AsiSayGenerateExplanation["status"],
	error?: string,
): string {
	const message: AsiSayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = AsiDefaultTool.GENERATE_EXPLANATION
