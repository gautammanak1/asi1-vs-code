...
import { AsiPlanModeResponse } from "@/shared/ExtensionMessage"
...
export class PlanModeRespondHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = AsiDefaultTool.PLAN_MODE
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.mode === "plan" && config.yoloModeToggled && !needsMoreExploration) {
			...
			if (switchSuccessful) {
				...
				const lastPlanMessage = findLast(config.messageState.getAsiMessages(), (m: any) => m.ask === this.name)
				if (lastPlanMessage) {
