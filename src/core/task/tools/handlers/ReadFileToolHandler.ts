...
async function emitReadFileToolUiComplete(
	...
	const payload: AsiSayTool = { ...sharedMessageProps }
	if (range) {
...
export class ReadFileToolHandler implements IFullyManagedTool {
	readonly name = AsiDefaultTool.FILE_READ
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		// Check Asiignore access
		const accessValidation = this.validator.checkAsiIgnorePath(relPath!)
		if (!accessValidation.ok) {
			if (!config.isSubagentExecution) {
				await config.callbacks.say("Asiignore_error", relPath)
			}
			return formatResponse.toolError(formatResponse.AsiIgnoreError(relPath!))
		}
		...
		if (shouldAutoApprove) {
			...
			const notificationMessage = `Asi wants to read ${getWorkspaceBasename(absolutePath, "ReadFileToolHandler.notification")}`
