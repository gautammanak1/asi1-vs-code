...
export class ReportBugHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = AsiDefaultTool.REPORT_BUG
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Asi wants to create a github issue...",
				message: `Asi is suggesting to create a github issue with the title: ${title}`,
			})
		}
		...
		const AsiVersion = ExtensionRegistryInfo.version
		...
		if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
			...
			try {
				...
				params.set("Asi-version", AsiVersion)
				...
				await createAndOpenGitHubIssue("Asi", "Asi", "bug_report.yml", params)
			} catch (error) {
