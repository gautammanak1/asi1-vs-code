...
import {
    AsiAskUseSubagents,
    AsiSaySubagentStatus,
    AsiSubagentUsageInfo,
    SubagentStatusItem,
...
export class UseSubagentsToolHandler implements IFullyManagedTool {
	readonly name = AsiDefaultTool.USE_SUBAGENTS
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		const partialMessage = JSON.stringify({ prompts } satisfies AsiAskUseSubagents)
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		const approvalPayload: AsiAskUseSubagents = { prompts }
		...
		if (didAutoApprove) {
			...
			showNotificationForApproval(
				prompts.length === 1
					? `Asi wants to use ${configuredSubagentName ? `the '${configuredSubagentName}' subagent` : "a subagent"}`
					: `Asi wants to use ${prompts.length} subagents`,
		...
		const emitStatus = async (status: AsiSaySubagentStatus["status"], partial: boolean) => {
			...
			const payload: AsiSaySubagentStatus = {
				status,
		...
		const queueStatusUpdate = (status: AsiSaySubagentStatus["status"], partial: boolean): Promise<void> => {
		...
		const subagentUsagePayload: AsiSubagentUsageInfo = {
			source: "subagents",
