...
export class SummarizeTaskHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = AsiDefaultTool.SUMMARIZE_TASK
		...
		try {
			...
			if (hooksEnabled) {
				try {
					...
					const result = await executePreCompactHookWithCleanup({
						...
						AsiMessages: config.messageState.getAsiMessages(),
			...
			if (match) {
				...
				for (const relPath of filePaths) {
					...
					// Check .Asiignore first and skip ignored files
					...
					if (await config.callbacks.shouldAutoApproveToolWithPath(AsiDefaultTool.FILE_READ, relPath)) {
						try {
			...
			const telemetryData = config.services.contextManager.getContextTelemetryData(
				config.messageState.getAsiMessages(),
				config.api,
