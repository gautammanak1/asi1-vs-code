...
describe("ListCodeDefinitionNamesToolHandler.execute – error recovery", () => {
	...
	beforeEach(async () => {
		...
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "Asi-listdef-test-"))
	...
	function makeBlock(relPath?: string) {
		return {
			...
			name: AsiDefaultTool.LIST_CODE_DEF,
...
describe("ListFilesToolHandler.execute – error recovery", () => {
	...
	beforeEach(async () => {
		sandbox = sinon.createSandbox()
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "Asi-listfiles-test-"))
		sandbox.stub(pathUtils, "isLocatedInWorkspace").resolves(true)
	})
	...
	function makeBlock(relPath?: string, recursive?: string) {
		...
		return {
			type: "tool_use" as const,
			name: AsiDefaultTool.LIST_FILES,
			params,
			partial: false,
		}
	}
	...
	it("increments consecutiveMistakeCount on Asiignore denial", async () => {
		...
		assert.equal(typeof result, "string")
		assert.ok((result as string).includes("Asiignore"))
		assert.equal(taskState.consecutiveMistakeCount, 1)
	})

	it("accumulates Asiignore denials across repeated calls", async () => {
		const { config, taskState } = createConfig()
		const blockingValidator = new ToolValidator({ validateAccess: () => false } as any)
		const handler = new ListFilesToolHandler(blockingValidator)
...
describe("SearchFilesToolHandler.execute – error recovery", () => {
	...
	beforeEach(async () => {
		sandbox = sinon.createSandbox()
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "Asi-search-test-"))
		sandbox.stub(pathUtils, "isLocatedInWorkspace").resolves(true)
	})
