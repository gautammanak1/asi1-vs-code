...
import { AsiDefaultTool, setDynamicToolUseNames } from "@shared/tools"
...
/** Default Directory for agent configurations: ~/Documents/asi/Agents */
...
const AgentBaseConfigSchema = z.object({
	...
	tools: z.array(z.nativeEnum(AsiDefaultTool)).default([]),
...
function normalizeToolName(toolName: string): AsiDefaultTool {
	const trimmed = toolName.trim()
	if (!trimmed) {
		throw new Error("Tool name cannot be empty.")
	}

	const asDefaultTool = trimmed as AsiDefaultTool
	if (Object.values(AsiDefaultTool).includes(asDefaultTool)) {
		return asDefaultTool
	}

	throw new Error(
		`Unknown tool '${trimmed}'. Expected a AsiDefaultTool value (for example: read_file, list_files, search_files).`,
	)
}

function parseTools(tools: string | string[] | undefined): AsiDefaultTool[] {
	if (!tools) {
		return []
	}
...
export function getAgentsConfigPath(homeDir = os.homedir()): string {
	return path.join(homeDir, "Documents", "Asi", AGENTS_CONFIG_DIRECTORY_NAME)
}
