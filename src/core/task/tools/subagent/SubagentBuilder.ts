...
export const SUBAGENT_DEFAULT_ALLOWED_TOOLS: AsiDefaultTool[] = [
...
export class SubagentBuilder {
	...
	getAllowedTools(): AsiDefaultTool[] {
	...
	buildNativeTools(context: SystemPromptContext) {
		...
		const toolSets = AsiToolSet.getToolsForVariantWithFallback(family, this.allowedTools)
	...
	private resolveAllowedTools(configuredTools?: AsiDefaultTool[]): AsiDefaultTool[] {
		const sourceTools = configuredTools && configuredTools.length > 0 ? configuredTools : SUBAGENT_DEFAULT_ALLOWED_TOOLS
		return Array.from(new Set([...sourceTools, AsiDefaultTool.ATTEMPT]))
	}
