...
import { AsiDefaultTool, AsiTool } from "@shared/tools";
...
export class SubagentRunner {
	...
	private readonly allowedTools: AsiDefaultTool[];
	...
	async run(
		...
		try {
			...
			const conversation: AsiStorageMessage[] = [
				{
					role: "user",
					content: [
						{
			...
			while (true) {
				...
				if (useNativeToolCalls) {
					assistantContent.push(
						...finalizedToolCalls.map(toAssistantToolUseBlock),
					);
				}
				...
				const toolResultBlocks = [] as AsiUserContent[];
				for (const call of finalizedToolCalls) {
					const toolName = call.name as AsiDefaultTool;
					const toolCallParams = toToolUseParams(call.input);

					if (toolName === AsiDefaultTool.ATTEMPT) {
					...
					const latestToolCall = formatToolCallPreview(
						toolName,
						toolCallParams,
					);
					...
					const toolDescription =
						handler?.getDescription(toolCallBlock) || `[${toolName}]`;
					pushSubagentToolResultBlock(
						toolResultBlocks,
						call,
						toolDescription,
			...
			const errorText =
				(error as Error).message || "Subagent execution failed.";
	...
	private createSubagentTaskConfig(state: TaskState): TaskConfig {
		...
		const validator = new ToolValidator(
			this.baseConfig.services.ClineIgnoreController,
		);
	...
	private shouldRetryInitialStreamError(
		error: unknown,
		providerId: string,
		modelId: string,
	): boolean {
		// Mirror main loop behavior: do not auto-retry auth/balance failures.
		const parsedError = AsiError.transform(error, modelId, providerId);
		const isAuthError = parsedError.isErrorType(AsiErrorType.Auth);
		const isBalanceError = parsedError.isErrorType(AsiErrorType.Balance);
	...
	private compactConversationForContextWindow(
		...
		conversation: AsiStorageMessage[],
		conversationHistoryDeletedRange: [number, number] | undefined,
	): {
		didCompact: boolean;
		conversationHistoryDeletedRange: [number, number] | undefined;
	} {
		const optimizationResult = this.optimizeConversationForContextWindow(
			contextManager,
			conversation,
		);
	...
	private optimizeConversationForContextWindow(
		...
		const optimizedConversation =
			optimizationResult.optimizedConversationHistory.map(
				(message) => message as AsiStorageMessage,
			);
		conversation.splice(0, conversation.length, ...optimizedConversation);
		return {
			didOptimize: true,
			needToTruncate: optimizationResult.needToTruncate,
		};
	}
	...
	private async *createMessageWithInitialChunkRetry(
		api: ReturnType<typeof buildApiHandler>,
		systemPrompt: string,
		fullConversation: AsiStorageMessage[],
		nativeTools: AsiTool[] | undefined,
		providerId: string,
		modelId: string,
		contextManager: ContextManager,
		...
		for (
			...
			const truncatedConversation = contextManager
				...
				.map((message) => message as AsiStorageMessage);
			...
			try {
				...
				Logger.warn(
					`[SubagentRunner] Initial stream failed. Retrying attempt ${attempt + 1}.`,
					error,
				);
				await delay(delayMs);
			}
