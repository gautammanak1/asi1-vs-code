...
You are a code reviewer.`
		...
		assert.deepEqual(parsed.tools, [AsiDefaultTool.FILE_READ, AsiDefaultTool.LIST_FILES, AsiDefaultTool.SEARCH])
		assert.equal(parsed.systemPrompt, "You are a code reviewer.")
	})

	it("supports raw Asi tool ids in tools", () => {
		const content = `---
name: cli-agent
...
Prompt body`
		...
		const parsed = parseAgentConfigFromYaml(content)
		assert.deepEqual(parsed.tools, [AsiDefaultTool.FILE_READ, AsiDefaultTool.LIST_FILES])
	})
...
Prompt body`
	...
	it("loads all yaml/yml files from homeDir/.Asi/data/agents", async () => {
		const tempHome = await createTempHomeDir()
		tempDirs.push(tempHome)
...
Reviewer prompt`,
		...
		assert.deepEqual(localAgent?.tools, [AsiDefaultTool.FILE_READ])
		assert.equal(localAgent?.systemPrompt, "Prompt body")
		assert.equal(reviewer?.name, "reviewer")
		assert.deepEqual(reviewer?.tools, [AsiDefaultTool.LIST_FILES])
