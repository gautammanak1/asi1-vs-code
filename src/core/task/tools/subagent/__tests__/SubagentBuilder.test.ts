...
import { AsiToolSet } from "@core/prompts/system-prompt/registry/ClineToolSet"
...
describe("SubagentBuilder", () => {
	...
	it("uses cached config by subagent name and applies act-mode provider model override", () => {
			...
			getCachedConfig: (subagentName?: string) =>
				subagentName === "cached-agent"
					? {
							...
							tools: [AsiDefaultTool.LIST_FILES],
							modelId: "gpt-5",
		...
		assert.deepEqual(builder.getAllowedTools(), [AsiDefaultTool.LIST_FILES, AsiDefaultTool.ATTEMPT])
			...
			getCachedConfig: (subagentName?: string) =>
				subagentName === "openrouter-agent"
					? {
							...
							tools: [AsiDefaultTool.FILE_READ],
	...
	it("builds native tools by filtering allowed ids and context requirements then converting", () => {
		...
		const getToolsStub = sinon.stub(AsiToolSet, "getToolsForVariantWithFallback").returns([
			{
				config: {
					id: AsiDefaultTool.LIST_FILES,
					contextRequirements: () => true,
				},
			},
			{
				config: {
					id: AsiDefaultTool.SEARCH,
					contextRequirements: () => true,
				},
			},
			{
				config: {
					id: AsiDefaultTool.ATTEMPT,
					contextRequirements: () => false,
				},
			},
		] as never)
		const converter = sinon.stub().callsFake((tool: { id: string }) => ({ converted: tool.id }))
		const getConverterStub = sinon.stub(AsiToolSet, "getNativeConverter").returns(converter as never)
		...
		assert.deepEqual(result, [{ converted: AsiDefaultTool.LIST_FILES }])
	})
})
