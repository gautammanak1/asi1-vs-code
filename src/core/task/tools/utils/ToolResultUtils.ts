...
export class ToolResultUtils {
	...
	static pushToolResult(
		...
		if (typeof content === "string") {
			...
			// Get tool_use_id from map using call_id, or use "Asi" as fallback for backward compatibility
			const toolUseId = toolUseIdMap?.get(block.call_id || "") || "Asi"
			...
			// If using backward-compatible "Asi" ID and content is an array, spread it directly
			// instead of wrapping it (which would cause JSON.stringify in createToolResultBlock)
			if ((toolUseId === "Asi" || !toolUseId) && Array.isArray(content)) {
	...
	private static createToolResultBlock(content: ToolResponse, id?: string, call_id?: string) {
		// If id is "Asi", we treat it as a plain text result for backward compatibility
	 ...
	 */
	static async askApprovalAndPushFeedback(type: AsiAsk, completeMessage: string, config: TaskConfig) {
