...
export function registerTaskCommands(controller: Controller): vscode.Disposable[] {
	return [
		vscode.commands.registerCommand("Asi.dev.expireMcpOAuthTokens", async () => {
			try {
		...
		vscode.commands.registerCommand("Asi.dev.createTestTasks", async () => {
			const count = (
 ...
 */
function createRealisticMessageSequence(baseTimestamp: number, taskPrompt: string, taskIndex: number): AsiMessage[] {
	...
	const messages: AsiMessage[] = [
		// Initial task message - uses "say" with "text" which is the format used in Asi.ts
		{
			ts: baseTimestamp,
			type: "say",
			say: "text",
