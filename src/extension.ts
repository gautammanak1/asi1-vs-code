...
export async function activate(context: vscode.ExtensionContext) {
	...
	// After this, all platforms (VSCode, CLI, JetBrains) read from ~/.Asi/data/.
	...
	*/
	...
	const handleUri = async (uri: vscode.Uri) => {
		...
		if (isTaskUri) {
			await openAsiSidebarForTaskUri()
		}
	...
	if (IS_DEV) {
		vscode.commands.executeCommand("setContext", "Asi.isDevMode", IS_DEV)
		// Use dynamic import to avoid loading the module in production
		import("./dev/commands/tasks")
			.then((module) => {
				const devTaskCommands = module.registerTaskCommands(webview.controller)
				context.subscriptions.push(...devTaskCommands)
				Logger.log("[Asi Dev] Dev mode activated & dev commands registered")
			})
			.catch((error) => {
				Logger.log("[Asi Dev] Failed to register dev commands: " + error)
			})
	}
		...
		vscode.languages.registerCodeActionsProvider(
			...
			new (class implements vscode.CodeActionProvider {
				...
				provideCodeActions(
					...
					// Add to Asi (Always available)
					const addAction = new vscode.CodeAction("Add to Asi", vscode.CodeActionKind.QuickFix)
					addAction.command = {
					...
					// Explain with Asi (Always available)
					const explainAction = new vscode.CodeAction("Explain with Asi", vscode.CodeActionKind.RefactorExtract) // Using a refactor kind
					explainAction.command = {
						command: commands.ExplainCode,
						title: "Explain with Asi",
						arguments: [expandedRange],
					}
					actions.push(explainAction)

					// Improve with Asi (Always available)
					const improveAction = new vscode.CodeAction("Improve with Asi", vscode.CodeActionKind.RefactorRewrite) // Using a refactor kind
					improveAction.command = {
						command: commands.ImproveCode,
						title: "Improve with Asi",
						arguments: [expandedRange],
					}
					actions.push(improveAction)

					// Fix with Asi (Only if diagnostics exist)
					if (context.diagnostics.length > 0) {
						const fixAction = new vscode.CodeAction("Fix with Asi", vscode.CodeActionKind.QuickFix)
						fixAction.isPreferred = true
						fixAction.command = {
							command: commands.FixWithAsi,
							title: "Fix with Asi",
							arguments: [expandedRange, context.diagnostics],
						}
...
\`\`\``
				...
				await addToAsi(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(
			...
			async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
				...
				await explainWithAsi(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
...
\`\`\``
				...
				await improveWithAsi(ctx.controller, ctx.commandContext, notebookContext)
			},
		),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.Walkthrough, async () => {
			await vscode.commands.executeCommand("workbench.action.openWalkthrough", `${context.extension.id}#FetchCoderWalkthrough`)
...
function setupHostProvider(context: ExtensionContext) {
	const outputChannel = registerAsiOutputChannel(context)
	outputChannel.appendLine("[Fetch Coder] Setting up VS Code host...")
...
async function openAsiSidebarForTaskUri(): Promise<void> {
...
async function cleanupLegacyVSCodeStorage(context: ExtensionContext): Promise<void> {
	try {
		...
		// Migrate custom instructions to global Asi rules (one-time cleanup)
