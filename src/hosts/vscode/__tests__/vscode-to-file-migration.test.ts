...
import "should"
import { AsiFileStorage } from "@shared/storage/ClineFileStorage"
...
describe("vscode-to-file-migration", () => {
	...
	describe("sentinel behavior", () => {
		...
		it("should skip everything when both sentinels are current version", async () => {
			...
			mockCtx._workspaceStateStore.set("localAsiRulesToggles", { "rule-1": true })
		...
		it("should migrate workspace state when globals already migrated (new workspace)", async () => {
			...
			const stored = storageContext.workspaceState.get("localAsiRulesToggles") as any
		...
		it("should migrate globals when workspace already migrated", async () => {
			...
			const mockCtx = createMockVSCodeContext()
			mockCtx._globalStateStore.set("mode", "plan")
			mockCtx._workspaceStateStore.set("localAsiRulesToggles", { "rule-1": true }) // should be skipped
	...
	describe("workspace state migration", () => {
		it("should migrate workspace state keys", async () => {
			...
			const mockCtx = createMockVSCodeContext()
			mockCtx._workspaceStateStore.set("localAsiRulesToggles", toggles)
		...
		it("should NOT overwrite existing workspace state", async () => {
			...
			storageContext.workspaceState.set("localAsiRulesToggles", existingToggles)

			const mockCtx = createMockVSCodeContext()
			mockCtx._workspaceStateStore.set("localAsiRulesToggles", { "rule-vscode": true })
...
describe("createStorageContext", () => {
	...
	it("should create all three stores", () => {
		const ctx = createStorageContext({ AsiDir: tempDir, workspacePath: "/fake/workspace" })

		ctx.globalState.should.be.instanceOf(AsiFileStorage)
		ctx.secrets.should.be.instanceOf(AsiFileStorage)
		ctx.workspaceState.should.be.instanceOf(AsiFileStorage)
	})
	...
	it("should produce deterministic workspace hashes", () => {
		const ctx1 = createStorageContext({ AsiDir: tempDir, workspacePath: "/some/project" })
		const ctx2 = createStorageContext({ AsiDir: tempDir, workspacePath: "/some/project" })
	...
	it("should produce different hashes for different workspaces", () => {
		const ctx1 = createStorageContext({ AsiDir: tempDir, workspacePath: "/project-a" })
		const ctx2 = createStorageContext({ AsiDir: tempDir, workspacePath: "/project-b" })
	...
	it("should store and retrieve values correctly", () => {
		const ctx = createStorageContext({ AsiDir: tempDir, workspacePath: "/test" })
