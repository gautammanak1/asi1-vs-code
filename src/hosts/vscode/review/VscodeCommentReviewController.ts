 ...
 */
const Asi_AVATAR_URL = "https://avatars.githubusercontent.com/u/184127137"
...
export class VscodeCommentReviewController extends CommentReviewController implements vscode.Disposable {
	...
	constructor() {
		...
		this.commentController = vscode.comments.createCommentController("Asi-ai-review", "Asi AI Review")
		...
		this.disposables.push(
			vscode.commands.registerCommand("Asi.reviewComment.reply", async (reply: vscode.CommentReply) => {
				await this.handleReply(reply)
			}),
		)

		// Register add to chat command - sends the conversation to Asi's main chat
		this.disposables.push(
			vscode.commands.registerCommand("Asi.reviewComment.addToChat", async (thread: vscode.CommentThread) => {
	...
	addReviewComment(comment: ReviewComment): void {
		...
		const commentObj: vscode.Comment = {
			...
			author: {
				name: "Asi",
				iconPath: vscode.Uri.parse(Asi_AVATAR_URL),
			},
		}
...
Please continue helping the user with their question about this code.`
	...
	/**
	 * Close all tabs that use the Asi-diff URI scheme (both diff views and regular text documents)
	 */
	async closeDiffViews(): Promise<void> {
		const tabs = vscode.window.tabGroups.all
			...
			.filter((tab) => {
				...
				// Check for regular text document tabs with Asi-diff scheme (opened during comment reveal)
