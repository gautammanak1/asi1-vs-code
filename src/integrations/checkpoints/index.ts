...
import { AsiApiReqInfo, AsiMessage, AsiSay } from "@shared/ExtensionMessage"
...
export class TaskCheckpointManager implements ICheckpointManager {
	...
	async saveCheckpoint(isAttemptCompletionMessage: boolean = false, completionMessageTs?: number): Promise<void> {
		try {
			...
			const AsiMessages = this.services.messageStateHandler.getAsiMessages()
			AsiMessages.forEach((message) => {
			...
			if (!isAttemptCompletionMessage) {
				...
				if (messageTs) {
					const messages = this.services.messageStateHandler.getAsiMessages()
						...
						this.state.checkpointTracker
							...
							.then(async (commitHash) => {
								if (commitHash) {
									...
									await this.services.messageStateHandler.saveAsiMessagesAndUpdateHistory()
								}
							})
							.catch((error) => {
								Logger.error(
				...
				const lastFiveAsiMessages = this.services.messageStateHandler.getAsiMessages().slice(-3)
				const lastCompletionResultMessage = findLast(lastFiveAsiMessages, (m) => m.say === "completion_result")
	...
	async restoreCheckpoint(
		messageTs: number,
		restoreType: AsiCheckpointRestore,
		offset?: number,
	): Promise<CheckpointRestoreStateUpdate> {
		try {
			const AsiMessages = this.services.messageStateHandler.getAsiMessages()
			const messageIndex = AsiMessages.findIndex((m) => m.ts === messageTs) - (offset || 0)
			// Find the last message before messageIndex that has a lastCheckpointHash
			const lastHashIndex = findLastIndex(AsiMessages.slice(0, messageIndex), (m) => m.lastCheckpointHash !== undefined)
			const message = AsiMessages[messageIndex]
			const lastMessageWithHash = AsiMessages[lastHashIndex]
	...
	async presentMultifileDiff(messageTs: number, seeNewChangesSinceLastTaskCompletion: boolean): Promise<void> {
		...
		try {
			...
			const AsiMessages = this.services.messageStateHandler.getAsiMessages()
			const messageIndex = AsiMessages.findIndex((m) => m.ts === messageTs)
			const message = AsiMessages[messageIndex]
			if (!message) {
				Logger.error(`[TaskCheckpointManager] Message not found for timestamp ${messageTs} in task ${this.task.taskId}`)
				relinquishButton()
			...
			if (seeNewChangesSinceLastTaskCompletion) {
				...
				const lastTaskCompletedMessageCheckpointHash = findLast(
					this.services.messageStateHandler.getAsiMessages().slice(0, messageIndex),
	...
	async doesLatestTaskCompletionHaveNewChanges(): Promise<boolean> {
		try {
			...
			const AsiMessages = this.services.messageStateHandler.getAsiMessages()
			const messageIndex = findLastIndex(AsiMessages, (m) => m.say === "completion_result")
			const message = AsiMessages[messageIndex]
			if (!message) {
	...
	private async handleSuccessfulRestore(
		...
		switch (restoreType) {
			...
			case "taskAndWorkspace":
				...
				const AsiMessages = this.services.messageStateHandler.getAsiMessages()
				const deletedMessages = AsiMessages.slice(messageIndex + 1)
				...
				const newAsiMessages = AsiMessages.slice(0, messageIndex + 1)
				await this.services.messageStateHandler.overwriteAsiMessages(newAsiMessages) // calls saveAsiMessages which saves historyItem
		...
		try {
			checkpointsWarningTimer = setTimeout(async () => {
				if (!checkpointsWarningShown) {
					...
					await this.setcheckpointManagerErrorMessage(
						"Checkpoints are taking longer than expected to initialize. Working in a large repository? Consider re-opening Asi in a project that uses git, or disabling checkpoints.",
					)
				}
			}, 7_000)
			...
			const tracker = await pTimeout(
				CheckpointTracker.create(this.task.taskId, this.config.enableCheckpoints, workspacePath),
				{
					milliseconds: 15_000,
					message:
						"Checkpoints taking too long to initialize. Consider re-opening Asi in a project that uses git, or disabling checkpoints.",
				},
			)
			...
			if (errorMessage.includes("Checkpoints taking too long to initialize")) {
				await this.setcheckpointManagerErrorMessage(
					"Checkpoints initialization timed out. Consider re-opening Asi in a project that uses git, or disabling checkpoints.",
				)
			} else {
				await this.setcheckpointManagerErrorMessage(errorMessage)
			}
