...
import { AsiToolResponseContent } from "@shared/messages"
...
export class CommandExecutor {
	...
	async cancelBackgroundCommand(): Promise<boolean> {
		...
		if (cancelled) {
			...
			const messages = this.callbacks.getAsiMessages()
			...
			if (lastCommandOutputIndex !== -1) {
				...
				await this.callbacks.updateAsiMessage(lastCommandOutputIndex, {
