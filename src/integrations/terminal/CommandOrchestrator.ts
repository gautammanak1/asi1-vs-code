...
export async function orchestrateCommandExecution(
	...
	const clearCommandState = async () => {
		...
		const AsiMessages = callbacks.getAsiMessages()
		const lastCommandIndex = findLastIndex(AsiMessages, (m) => m.ask === "command" || m.say === "command")
		if (lastCommandIndex !== -1) {
			await callbacks.updateAsiMessage(lastCommandIndex, {
	...
	const switchToFileBased = async () => {
		...
		// Set up file logging using AsiTempManager for proper cleanup
		const outputLogPath = AsiTempManager.createTempFilePath("large-output")
		largeOutputLogPath = outputLogPath
		largeOutputLogStream = fs.createWriteStream(outputLogPath, { flags: "a" })
