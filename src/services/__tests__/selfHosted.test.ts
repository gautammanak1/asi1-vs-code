 ...
 * When AsiEndpoint.isSelfHosted() returns true, all PostHog functionality should be disabled.
 */
...
describe("SelfHosted Mode - PostHog Disabling", () => {
	...
	describe("FeatureFlagsProviderFactory", () => {
		it("should return no-op config when in selfHosted mode", () => {
			isSelfHostedStub = sinon.stub(AsiEndpoint, "isSelfHosted").returns(true)
		...
		it("should return posthog config when NOT in selfHosted mode (if PostHog config is valid)", () => {
			isSelfHostedStub = sinon.stub(AsiEndpoint, "isSelfHosted").returns(false)
