...
export class AsiAccountService {
	private static instance: AsiAccountService
	...
	/**
	 * Returns the singleton instance of AsiAccountService
	 * @returns Singleton instance of AsiAccountService
	 */
	public static getInstance(): AsiAccountService {
		if (!AsiAccountService.instance) {
			AsiAccountService.instance = new AsiAccountService()
		}
		return AsiAccountService.instance
	}

	/**
	 * Returns the base URL for the Asi API
	...
	/**
	 * Helper function to make authenticated requests to the Asi API
	...
	private async authenticatedRequest<T>(endpoint: string, config: AxiosRequestConfig = {}): Promise<T> {
		...
		if (!AsiAccountAuthToken) {
			throw new Error("No Asi account auth token found")
		}
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(Asi_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async fetchFeaturebaseToken(): Promise<FeaturebaseTokenResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<FeaturebaseTokenResponse>(Asi_API_ENDPOINT.FEATUREBASE_TOKEN)
			return data
		} catch (error) {
	...
	async switchAccount(organizationId?: string): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(Asi_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
