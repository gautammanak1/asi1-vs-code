...
export class OcaAuthService {
	...
	async getAuthToken(): Promise<string | null> {
		...
		try {
			shouldRefresh =
				await this.requireProvider().shouldRefreshAccessToken(apiKey);
		} catch {
	...
	async createAuthRequest(): Promise<ProtoString> {
		...
		const ocaMode =
			this.requireController().stateManager.getGlobalSettingsKey("ocaMode") ||
			"internal";
		const idcsUrl =
			ocaMode === "external"
				? this._config.external.idcs_url
				: this._config.internal.idcs_url;
		if (!idcsUrl) {
	...
	private async kickstartInteractiveLoginAsFallback(
		_err?: unknown,
	): Promise<void> {
	...
	async subscribeToAuthStatusUpdate(
		...
		if (!this._ocaAuthState) {
			this._ocaAuthState =
				await this.requireProvider().getExistingAuthState(ctrl);
	...
	async sendAuthStatusUpdate(): Promise<void> {
		...
		const promises = Array.from(this._activeAuthStatusUpdateSubscriptions).map(
			async (entry) => {
