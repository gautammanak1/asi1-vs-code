import { OcaAuthState, OcaUserInfo } from "@shared/proto/Asi/oca_account";
...
export class OcaAuthProvider {
	 ...
	 */
	async shouldRefreshAccessToken(
	...
	public async getExistingAuthState(
		controller: Controller,
	): Promise<OcaAuthState | null> {
	...
	async retrieveOcaAuthState(
		...
		const userRefreshToken =
			controller.stateManager.getSecretKey("ocaRefreshToken");
		if (!userRefreshToken) {
		...
		try {
			...
			if (refreshToken) {
				...
				throw new OcaRefreshError(
					"No refresh token received during OCA token refresh.",
				);
			}
			...
			const desc =
				data?.error_description || (isAxios ? (err as any).message : undefined);
			const invalidGrant =
				(status === 400 && code === "invalid_grant") || status === 401;

			Logger.error("OCA refresh failed", { status, code, desc });

			throw new OcaRefreshError(
				desc || "OCA refresh failed",
				status,
				code,
				invalidGrant,
				data,
	...
	clearAuth(controller: Controller): void {
		...
		// Clear legacy OCA secrets that may persist from older Asi versions.
