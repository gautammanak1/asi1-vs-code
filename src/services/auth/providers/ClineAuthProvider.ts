...
export interface AsiAuthApiTokenExchangeResponse {
	success: boolean
	data: AsiAuthResponseData
}

export interface AsiAuthApiTokenRefreshResponse {
	success: boolean
	data: AsiAuthResponseData
}
...
export class AsiAuthProvider {
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: AsiAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("Asi:AsiAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: AsiAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves Asi auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<AsiAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveAsiAuthInfo(controller: Controller): Promise<AsiAuthInfo | null> {
		try {
			// Get the stored auth data from secure storage
			const storedAuthDataString = controller.stateManager.getSecretKey("Asi:AsiAccountId")
			...
			let storedAuthData: AsiAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						controller.stateManager.setSecret("AsiAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("Asi:AsiAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use Asi, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<AsiAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: AsiAuthInfo): Promise<AsiAuthInfo> {
		try {
			const endpoint = new URL(Asi_API_ENDPOINT.REFRESH_TOKEN, this.config.apiBaseUrl)
			...
			const data: AsiAuthApiTokenExchangeResponse = await response.json()
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<AsiAuthInfo | null> {
		try {
			...
			const tokenUrl = new URL(Asi_API_ENDPOINT.TOKEN_EXCHANGE, this.config.apiBaseUrl)
			...
			const responseType: AsiAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("Asi:AsiAccountId", JSON.stringify(AsiAuthInfo))
	...
	private async fetchRemoteUserInfo(tokenData: AsiAuthApiTokenExchangeResponse["data"]): Promise<AsiAccountUserInfo> {
		try {
			const userResponse = await axios.get(`${AsiEnv.config().apiBaseUrl}/api/v1/users/me`, {
				headers: {
			...
			return {
				id: tokenData.userInfo.AsiUserId || "",
