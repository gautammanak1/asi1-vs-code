...
import { BannerActionType, type BannerCardData } from "@shared/asi/banner"
...
export class BannerService {
	...
	public getWelcomeBanners(): BannerCardData[] | undefined {
		const isLocal = process.env.IS_DEV === "true" || process.env.Asi_ENVIRONMENT === "local"
		const flagEnabled = isLocal || featureFlagsService.getBooleanFlagEnabled(FeatureFlag.REMOTE_WELCOME_BANNERS)

		if (!flagEnabled) {
			return undefined
		}
		const bypassDismissals = process.env.IS_DEV === "true" || process.env.Asi_ENVIRONMENT === "local"
	...
	public async sendBannerEvent(bannerId: string, eventType: "dismiss"): Promise<void> {
		try {
			const url = new URL("/banners/v2/messages", AsiEnv.config().apiBaseUrl).toString()
	...
	private buildFetchUrl(): string {
		const url = new URL("/banners/v2/messages", AsiEnv.config().apiBaseUrl)
