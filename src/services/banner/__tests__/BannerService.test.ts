...
describe("BannerService", () => {
	...
	beforeEach(() => {
		...
		// Mock AsiEnv.config() to return a valid config
	...
	describe("IDE Type Detection", () => {
		...
		it('should return "jetbrains" when ide is "Asi for JetBrains" (case-insensitive)', async () => {
			stubHostInfo({ ide: "Asi for JetBrains" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
		...
		it("should prefer ide field over platform field for detection", async () => {
			stubHostInfo({ ide: "Asi for JetBrains", platform: "Visual Studio Code 1.103.0" })
			mockFetch.resolves(createSuccessResponse(emptyResponse))
