...
export class AsiError extends Error {
	...
	/**
	 * Parses a stringified error into a AsiError instance.
	 */
	static parse(errorStr?: string, modelId?: string): AsiError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return AsiError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a AsiError instance.
	 * Always returns a AsiError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): AsiError {
		try {
			// If already a AsiError, return it directly to prevent infinite recursion
			if (error instanceof AsiError) {
				return error
			}
			return new AsiError(JSON.parse(error), modelId, providerId)
		} catch {
			return new AsiError(error, modelId, providerId)
		}
	}

	public isErrorType(type: AsiErrorType): boolean {
		return AsiError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: AsiError): AsiErrorType | undefined {
		...
		if (message) {
			...
			if (message?.includes(Asi_ACCOUNT_AUTH_ERROR_MESSAGE) || authErrorRegex.some((regex) => regex.test(message))) {
				return AsiErrorType.Auth
			}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = AsiErrorType.Network
	}
}
