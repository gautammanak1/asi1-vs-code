...
class NoOpErrorProvider implements IErrorProvider {
	async captureException(error: Error | AsiError, properties?: Record<string, unknown>): Promise<void> {
		Logger.error("[NoOpErrorProvider] captureException called", { error: error.message || String(error), properties })
	}

	public logException(error: Error | AsiError, _properties?: Record<string, unknown>): void {
