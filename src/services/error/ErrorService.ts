...
export class ErrorService {
	...
	captureException(error: Error | AsiError, properties?: Record<string, unknown>) {
		return this.provider.captureException(error, properties)
	}

	public logException(error: Error | AsiError, properties?: Record<string, unknown>): void {
	...
	public toAsiError(rawError: unknown, modelId?: string, providerId?: string): AsiError {
		const transformed = AsiError.transform(rawError, modelId, providerId)
