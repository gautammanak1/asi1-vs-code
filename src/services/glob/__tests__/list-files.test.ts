...
describe("listFiles", () => {
	const tmpDir = path.join(os.tmpdir(), `Asi-list-files-test-${Math.random().toString(36).slice(2)}`)
...
describe("listFiles gitignore handling", () => {
	...
	const baseDir = path.join(os.tmpdir(), `Asi-gitignore-test-${Math.random().toString(36).slice(2)}`)
