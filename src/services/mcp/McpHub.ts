...
export class McpHub {
	...
	isConnecting = false
	/**
	 * Flag to skip file watcher processing when we're updating Asi-specific settings
	 ...
	 */
	private isUpdatingAsiSettings = false
	 ...
	 * Excludes Asi-specific settings since they don't affect the MCP server transport connection.
	 *
	 * ## Asi-specific settings (don't require restart):
	 ...
	 *
	 * ## Adding new Asi-specific settings:
	 ...
	 * 2. Add it to `isUpdatingAsiSettings` flag usage in the update function
	...
	private configsRequireRestart(oldConfig: McpServerConfig, newConfig: McpServerConfig): boolean {
		// Exclude Asi-specific settings from comparison (add new ones here)
		const {
	...
	private setupFileWatcher(name: string, config: Extract<McpServerConfig, { type: "stdio" }>) {
		...
		if (filePath) {
			// we use chokidar instead of onDidSaveTextDocument because it doesn't require the file to be open in the editor. The settings config is better suited for onDidSave since that will be manually updated by the user or Asi (and we want to detect save events, not every file change)
	...
	async toggleToolAutoApproveRPC(serverName: string, toolNames: string[], shouldAllow: boolean): Promise<McpServer[]> {
		...
		this.isUpdatingAsiSettings = true
		try {
			const settingsPath = await getMcpSettingsFilePathHelper(await this.getSettingsDirectoryPath())
			const content = await fs.readFile(settingsPath, "utf-8")
			const config = JSON.parse(content)
			...
			setTimeout(() => {
				this.isUpdatingAsiSettings = false
			}, 300)
		}
	}
