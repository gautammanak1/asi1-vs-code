...
export type TelemetryProviderConfig =
	 ...
	 * @param bypassUserSettings - When true, telemetry is sent regardless of the user's Asi telemetry opt-in/opt-out settings.
	 * This is used for:
	 * 	- User-controlled collectors configured via environment variables (e.g., Asi_OTEL_TELEMETRY_ENABLED).
...
export class TelemetryProviderFactory {
	...
	public static getDefaultConfigs(): TelemetryProviderConfig[] {
		...
		if (!AsiEndpoint.isSelfHosted() && isPostHogConfigValid(posthogConfig)) {
			configs.push({ type: "posthog", ...posthogConfig })
		}

		// Skip build-time OTEL in selfHosted mode - enterprise customers should not send telemetry to Asi's collector
		...
		if (!AsiEndpoint.isSelfHosted() && otelConfig) {
			configs.push({
				type: "opentelemetry",
				config: otelConfig,
		...
		if (runtimeOtelConfig) {
			configs.push({
				...
				// If the user has `Asi_OTEL_TELEMETRY_ENABLED` in his environment, enable
				// OTEL regardless of his Asi telemetry settings
