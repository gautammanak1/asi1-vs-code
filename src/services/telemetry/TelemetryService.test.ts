...
import { AsiEndpoint } from "@/config";
...
describe("Telemetry system is abstracted and can easily switch between providers", () => {
	...
	describe("Telemetry Service", () => {
		it("should derive remote workspace metadata from host version", async () => {
			...
			const createProvidersStub = sinon.stub(
				TelemetryProviderFactory,
				"createProviders",
			);
			...
			assert.strictEqual(
				remoteLogSpy.firstCall.args[1]?.is_remote_workspace,
				true,
			);
			assert.strictEqual(
				localLogSpy.firstCall.args[1]?.is_remote_workspace,
				false,
			);
		...
		it("should include correct metadata with telemetry events", async () => {
			...
			assert.strictEqual(
				eventName,
				"task.created",
				"Event name should be task.created",
			);
			...
			assert.ok(
				identifyUserSpy.calledOnce,
				"IdentifyUser should be called once",
			);
			...
			assert.deepStrictEqual(
				metadata,
				MOCK_METADATA,
				"Identify user should include only the expected metadata properties",
			);
			...
			assert.ok(
				recordCounterSpy.called,
				"recordCounter should be called for tool usage",
			);
			const recordCounterArgs = recordCounterSpy.firstCall.args;
			const recordCounterAttributes = recordCounterArgs[2] as Record<
				string,
				unknown
			>;
			...
			assert.ok(
				logSpy.calledWith("direct_event", { custom: "data" }),
				"Direct provider log should not add metadata",
			);
		...
		it("should support multi-provider telemetry for dual tracking", async () => {
			...
			assert.strictEqual(
				eventName1,
				"task.created",
				"First provider should receive correct event name",
			);
			assert.strictEqual(
				eventName2,
				"task.created",
				"Second provider should receive correct event name",
			);
			...
			assert.deepStrictEqual(
				properties1,
				expectedProperties,
				"First provider should receive correct properties",
			);
			assert.deepStrictEqual(
				properties2,
				expectedProperties,
				"Second provider should receive correct properties",
			);
			...
			assert.ok(
				identifyUserSpy1.calledOnce,
				"First provider should receive user identification",
			);
			assert.ok(
				identifyUserSpy2.calledOnce,
				"Second provider should receive user identification",
			);
	...
	describe("PostHog Provider", () => {
		it("should create PostHog provider and track events", async () => {
			...
			const posthogProvider =
				providers.find((p) => !(p instanceof NoOpTelemetryProvider)) ||
				providers[0];

			const posthogTelemetryService = new TelemetryService(
				[posthogProvider],
				MOCK_METADATA,
			);
			...
			posthogTelemetryService.captureModelSelected(
				"claude-3",
				"anthropic",
				"task-123",
			);
	...
	describe("No-Op Provider", () => {
		it("should create No-Op provider and handle all operations safely", async () => {
			...
			noOpTelemetryService.captureToolUsage(
				"task-789",
				"write_to_file",
				"gpt-4",
				"openai",
		...
		it("should handle unsupported provider types by returning No-Op provider", async () => {
			...
			assert.strictEqual(
				unsupportedProvider.isEnabled(),
				false,
				"Unsupported provider should return NoOp provider",
			);
			assert.deepStrictEqual(
	...
	describe("Factory Configuration", () => {
		it("should return default configurations", () => {
			...
			const isPostHogConfigValidStub = sinon
				.stub(posthogConfigModule, "isPostHogConfigValid")
				.returns(true);
			const isSelfHostedStub = sinon
				.stub(AsiEndpoint, "isSelfHosted")
				.returns(false);
		...
		it("should NOT include PostHog config when in selfHosted mode", () => {
			// Stub AsiEndpoint.isSelfHosted() to return true (selfHosted mode)
			const isSelfHostedStub = sinon
				.stub(AsiEndpoint, "isSelfHosted")
				.returns(true);
			...
			assert.strictEqual(
				hasPosthog,
				false,
				"Should NOT include PostHog configuration in selfHosted mode",
			);
		...
		it("should include PostHog config when NOT in selfHosted mode and config is valid", () => {
			// Stub AsiEndpoint.isSelfHosted() to return false (normal mode)
			const isSelfHostedStub = sinon
				.stub(AsiEndpoint, "isSelfHosted")
				.returns(false);
			const isPostHogConfigValidStub = sinon
			...
			assert.strictEqual(
				hasPosthog,
				true,
				"Should include PostHog configuration when not in selfHosted mode",
			);
		...
		it("should NOT include build-time OTEL config when in selfHosted mode", () => {
			...
			const getValidOtelConfigStub = sinon
				.stub(otelConfigModule, "getValidOpenTelemetryConfig")
				.returns({
					enabled: true,
					metricsExporter: "otlp",
				});
			// Disable runtime OTEL to isolate test
			const getRuntimeOtelConfigStub = sinon
				.stub(otelConfigModule, "getValidRuntimeOpenTelemetryConfig")
				.returns(null);
			...
			assert.strictEqual(
				hasOtel,
				false,
				"Should NOT include build-time OTEL configuration in selfHosted mode",
			);
		...
		it("should include build-time OTEL config when NOT in selfHosted mode", () => {
			...
			assert.strictEqual(
				hasOtel,
				true,
				"Should include build-time OTEL configuration when not in selfHosted mode",
			);
		...
		it("should STILL include runtime env OTEL config even in selfHosted mode", () => {
			...
			assert.strictEqual(
				hasOtel,
				true,
				"Should include runtime env OTEL configuration even in selfHosted mode",
			);
	...
	describe("CLI Subagents Telemetry", () => {
		it("should capture subagent toggle events correctly", async () => {
			...
			assert.strictEqual(
				...
				"Event should be subagent_enabled when enabled",
			);
			assert.strictEqual(
				properties1.enabled,
				true,
				"Properties should include enabled: true",
			);
			assert.ok(properties1.timestamp, "Properties should include timestamp");
			assert.strictEqual(
				typeof properties1.timestamp,
				"string",
			...
			assert.strictEqual(
				...
				"Event should be subagent_disabled when disabled",
			);
			assert.strictEqual(
				properties2.enabled,
				false,
				"Properties should include enabled: false",
			);
		...
		it("should capture subagent execution events correctly", async () => {
			...
			assert.ok(
				logSpy.calledOnce,
				"Log should be called once for successful execution",
			);
			const [eventName1, properties1] = logSpy.firstCall.args;
			assert.ok(properties1, "Properties should be defined");
			assert.strictEqual(
				...
				"Event should be subagent_completed when successful",
			);
			assert.strictEqual(
				properties1.ulid,
				"task-123",
				"Properties should include task ULID",
			);
			assert.strictEqual(
				properties1.durationMs,
				1500,
				"Properties should include duration",
			);
			assert.strictEqual(
				properties1.outputLines,
				25,
				"Properties should include output line count",
			);
			assert.strictEqual(
				properties1.success,
				true,
				"Properties should include success status",
			);
			assert.ok(properties1.timestamp, "Properties should include timestamp");
			...
			assert.ok(
				logSpy.calledOnce,
				"Log should be called once for failed execution",
			);
			const [eventName2, properties2] = logSpy.firstCall.args;
			assert.ok(properties2, "Properties should be defined");
			assert.strictEqual(
				...
				"Event should be subagent_started when failed",
			);
			assert.strictEqual(
				properties2.ulid,
				"task-456",
			...
			assert.strictEqual(
				properties2.durationMs,
				3200,
				"Properties should include duration",
			);
			assert.strictEqual(
				properties2.outputLines,
				150,
				"Properties should include output line count",
			);
			assert.strictEqual(
		...
		it("should respect subagents telemetry category settings", async () => {
			...
			assert.ok(
				logSpy.calledOnce,
				"Event should be captured when category is enabled",
			);
			...
			assert.ok(
				logSpy.calledOnce,
				"Execution event should be captured when category is enabled",
			);
	...
	describe("Skills Telemetry", () => {
		it("should capture skill used events correctly", async () => {
			...
			assert.strictEqual(
				eventName,
				"task.skill_used",
				"Event name should be task.skill_used",
			);
			...
			assert.strictEqual(
				...
				"my-skill",
				"Properties should include skillName",
			);
			assert.strictEqual(
				properties.skillSource,
				"global",
				"Properties should include skillSource",
			);
			assert.strictEqual(
				properties.skillsAvailableGlobal,
				2,
				"Properties should include global skill count",
			);
			assert.strictEqual(
				properties.skillsAvailableProject,
				3,
				"Properties should include project skill count",
			);
			assert.strictEqual(
				properties.provider,
				"Asi",
			...
			assert.strictEqual(
				...
				"Properties should include modelId",
			);
