...
import { AsiAccountUserInfo } from "@/services/auth/AuthService"
...
export type TelemetryMetadata = {
	/**
	 * The extension or Asi-core version. JetBrains and CLI have different
	 * versioning than the VSCode Extension, but on those platforms this will be the _Asi-core version_
	 * which uses the same as the versioning as the VSCode extension.
	 */
	extension_version: string
	/**
	 * The type of Asi distribution, e.g VSCode Extension, JetBrains Plugin or CLI. This
...
/**
 * TelemetryService handles telemetry event tracking for the Asi extension
...
export class TelemetryService {
	...
	public static readonly METRICS = {
		TASK: {
			TURNS_TOTAL: "Asi.turns.total",
			TURNS_PER_TASK: "Asi.turns.per_task",
			TOKENS_INPUT_TOTAL: "Asi.tokens.input.total",
			TOKENS_INPUT_PER_RESPONSE: "Asi.tokens.input.per_response",
			TOKENS_OUTPUT_TOTAL: "Asi.tokens.output.total",
			TOKENS_OUTPUT_PER_RESPONSE: "Asi.tokens.output.per_response",
			COST_TOTAL: "Asi.cost.total",
			COST_PER_EVENT: "Asi.cost.per_event",
		},
		CACHE: {
			WRITE_TOTAL: "Asi.cache.write.tokens.total",
			WRITE_PER_EVENT: "Asi.cache.write.tokens.per_event",
			READ_TOTAL: "Asi.cache.read.tokens.total",
			READ_PER_EVENT: "Asi.cache.read.tokens.per_event",
			HITS_TOTAL: "Asi.cache.hits.total",
		},
		TOOLS: {
			CALLS_TOTAL: "Asi.tool.calls.total",
			CALLS_PER_TASK: "Asi.tool.calls.per_task",
		},
		ERRORS: {
			TOTAL: "Asi.errors.total",
			PER_TASK: "Asi.errors.per_task",
		},
		API: {
			TTFT_SECONDS: "Asi.api.ttft.seconds",
			DURATION_SECONDS: "Asi.api.duration.seconds",
			THROUGHPUT_TOKENS_PER_SECOND: "Asi.api.throughput.tokens_per_second",
		},
		HOOKS: {
			EXECUTIONS_TOTAL: "Asi.hooks.executions.total",
			DURATION_SECONDS: "Asi.hooks.duration.seconds",
			FAILURES_TOTAL: "Asi.hooks.failures.total",
			CANCELLATIONS_TOTAL: "Asi.hooks.cancellations.total",
			CONTEXT_MODIFICATIONS_TOTAL: "Asi.hooks.context_modifications.total",
			CACHE_ACCESSES_TOTAL: "Asi.hooks.cache.accesses.total",
		},
		AI_OUTPUT: {
			ACCEPTED_LINES_ADDED: "Asi.ai_output.accepted.lines_added.total",
			ACCEPTED_LINES_DELETED: "Asi.ai_output.accepted.lines_deleted.total",
			ACCEPTED_LINES_CHANGED: "Asi.ai_output.accepted.lines_changed.total",
			ACCEPTED_FILES_CREATED: "Asi.ai_output.accepted.files_created.total",
			ACCEPTED_FILES_DELETED: "Asi.ai_output.accepted.files_deleted.total",
			ACCEPTED_FILES_MOVED: "Asi.ai_output.accepted.files_moved.total",
			REJECTED_LINES_ADDED: "Asi.ai_output.rejected.lines_added.total",
			REJECTED_LINES_DELETED: "Asi.ai_output.rejected.lines_deleted.total",
			REJECTED_LINES_CHANGED: "Asi.ai_output.rejected.lines_changed.total",
			REJECTED_FILES_CREATED: "Asi.ai_output.rejected.files_created.total",
			REJECTED_FILES_DELETED: "Asi.ai_output.rejected.files_deleted.total",
			REJECTED_FILES_MOVED: "Asi.ai_output.rejected.files_moved.total",
		},
		GRPC: {
			RESPONSE_SIZE_BYTES: "Asi.grpc.response.size_bytes",
		},
	}
	...
	private static readonly EVENTS = {
		...
		TASK: {
			...
			// Tracks when individual Asi rules are toggled on/off
			...
			// Tracks when Asi web tools setting is toggled on/off
			Asi_WEB_TOOLS_TOGGLED: "task.Asi_web_tools_toggled",
	...
	public async updateTelemetryState(didUserOptIn: boolean): Promise<void> {
		...
		if (hostSetting.isEnabled === Setting.DISABLED) {
			// Only show warning if user has opted in to Asi telemetry but host telemetry is disabled
			if (didUserOptIn) {
				void HostProvider.window
					.showMessage({
						type: ShowMessageType.WARNING,
						message:
							"Anonymous Asi error and usage reporting is enabled, but IDE telemetry is disabled. To enable error and usage reporting for this extension, enable telemetry in IDE settings.",
						options: {
							items: ["Open Settings"],
						},
					})
	 ...
	 */
	public identifyAccount(userInfo: AsiAccountUserInfo) {
		const propertiesWithMetadata: TelemetryProperties = {
			...this.telemetryMetadata,
		}
	...
	/**
	 * Records when Asi calls the task completion_result tool signifying that Asi is done with the task
	 ...
	 * @param provider The API provider identifier (e.g. "anthropic", "openai", "Asi")
	...
	/**
	 * Records when individual Asi rules are toggled on/off
	 ...
	 */
	public captureAsiRuleToggled(ulid: string, ruleFileName: string, enabled: boolean, isGlobal: boolean) {
	...
	/**
	 * Records when Asi web tools are enabled/disabled by the user
	 * @param ulid Unique identifier for the task
	 * @param enabled Whether Asi web tools are enabled (true) or disabled (false)
	 */
	public captureAsiWebToolsToggle(ulid: string, enabled: boolean) {
		this.capture({
			event: TelemetryService.EVENTS.TASK.Asi_WEB_TOOLS_TOGGLED,
			properties: {
				ulid,
				enabled,
			},
