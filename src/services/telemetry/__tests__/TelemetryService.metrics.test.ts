...
class FakeProvider implements ITelemetryProvider {
	...
	public gauges = new Map<
		string,
		Map<
			string,
			{ value: number; attributes: TelemetryProperties; description?: string }
		>
	>();
...
describe("TelemetryService metrics", () => {
	...
	it("captureTokenUsage emits cache and cost metrics when options provided", () => {
		...
		const cacheWriteCounter = provider.counters.find(
			(entry) => entry.name === TelemetryService.METRICS.CACHE.WRITE_TOTAL,
		);
		...
		const cacheReadCounter = provider.counters.find(
			(entry) => entry.name === TelemetryService.METRICS.CACHE.READ_TOTAL,
		);
		assert.ok(cacheReadCounter);
		assert.strictEqual(cacheReadCounter?.value, 30);

		const costCounter = provider.counters.find(
			(entry) => entry.name === TelemetryService.METRICS.TASK.COST_TOTAL,
		);
		...
		const cacheWriteHist = provider.histograms.find(
			(entry) => entry.name === TelemetryService.METRICS.CACHE.WRITE_PER_EVENT,
		);
		assert.ok(cacheWriteHist);
		assert.strictEqual(cacheWriteHist?.value, 50);

		const cacheReadHist = provider.histograms.find(
			(entry) => entry.name === TelemetryService.METRICS.CACHE.READ_PER_EVENT,
		);
		assert.ok(cacheReadHist);
		assert.strictEqual(cacheReadHist?.value, 30);

		const costHist = provider.histograms.find(
			(entry) => entry.name === TelemetryService.METRICS.TASK.COST_PER_EVENT,
		);
		assert.ok(costHist);
	...
	it("captureTokenUsage includes options in event properties", () => {
		...
		const tokenEvent = provider.logs.find(
			(entry) => entry.event === "task.tokens",
		);
	...
	it("metrics include is_remote_workspace in standard attributes", () => {
		...
		assert.strictEqual(
			provider.counters[0].attributes.is_remote_workspace,
			true,
		);
		assert.strictEqual(
			provider.histograms[0].attributes.is_remote_workspace,
			true,
		);
	});
	...
	it("captureConversationTurnEvent emits counters with cache and cost", () => {
		...
		service.captureConversationTurnEvent(
			"task-2",
			"openai",
			"gpt-4",
			"assistant",
		...
		const costEntry = provider.counters.find(
			(entry) => entry.name === "Asi.cost.total",
		);
		assert.ok(costEntry);
		...
		const turnEntry = provider.histograms.find(
			(entry) => entry.name === TelemetryService.METRICS.TASK.TURNS_PER_TASK,
		);
		assert.ok(turnEntry);
	...
	it("captureWorkspaceInitialized emits gauge and retires previous series", () => {
		...
		const initialSeries = provider.gauges.get("Asi.workspace.active_roots");
		...
		const updatedSeries = provider.gauges.get("Asi.workspace.active_roots");
	...
	it("captureTaskCompleted records completion payload with TTFT and duration histograms", () => {
		...
		const completionEvent = provider.logs.find(
		...
		assert.strictEqual(
			completionEvent?.properties?.apiFormat,
			ApiFormat.OPENAI_RESPONSES,
		);
		assert.strictEqual(
			completionEvent?.properties?.apiFormatName,
			"OPENAI_RESPONSES",
		);
		...
		const ttftMetric = provider.histograms.find(
			(entry) => entry.name === TelemetryService.METRICS.API.TTFT_SECONDS,
		);
		...
		const durationMetric = provider.histograms.find(
			(entry) => entry.name === TelemetryService.METRICS.API.DURATION_SECONDS,
		);
	...
	it("captureGrpcResponseSize records histogram with correct name, value, and attributes", () => {
		...
		service.captureGrpcResponseSize(
			123456,
			"Asi.StateService",
			"subscribeToState",
		);
		...
		assert.strictEqual(entry.attributes.service, "Asi.StateService");
