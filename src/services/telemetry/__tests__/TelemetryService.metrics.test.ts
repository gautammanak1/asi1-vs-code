...
describe("TelemetryService metrics", () => {
	...
	it("captureConversationTurnEvent emits counters with cache and cost", () => {
		...
		const costEntry = provider.counters.find((entry) => entry.name === "Asi.cost.total")
		assert.ok(costEntry)
	...
	it("captureGrpcResponseSize records histogram with correct name, value, and attributes", () => {
		...
		service.captureGrpcResponseSize(123456, "Asi.StateService", "subscribeToState")
	...
	it("captureGrpcResponseSize includes request_id when provided", () => {
		...
		service.captureGrpcResponseSize(5000, "Asi.StateService", "subscribeToState", "req-42")
	...
	it("captureGrpcResponseSize includes standard metadata attributes", () => {
		...
		service.captureGrpcResponseSize(1000, "Asi.StateService", "subscribeToState")
