...
import type { AsiAccountUserInfo } from "@/services/auth/AuthService"
...
function makeUserInfo(
	overrides: Partial<AsiAccountUserInfo> & { orgOverrides?: Record<string, unknown> } = {},
): AsiAccountUserInfo {
...
describe("OpenTelemetryTelemetryProvider.identifyUser", () => {
	...
	it("should clear stale org attributes when switching from org to no-org", () => {
		...
		const userNoOrg: AsiAccountUserInfo = {
			id: "user-1",
			displayName: "Test User",
			email: "test@example.com",
			createdAt: new Date().toISOString(),
