...
export class PostHogClientProvider {
	 ...
	 * For exceptions, we only capture those from the Asi extension.
	 * this is specifically to avoid capturing errors from anything other than Asi
	 */
	static eventFilter(event: EventMessage | null) {
		...
		// Check if any exception is from Asi
		for (let i = 0; i < exceptionList.length; i++) {
			const stacktrace = exceptionList[i].stacktrace
			// Fast check: error message contains "Asi"
			if (stacktrace?.value?.toLowerCase().includes("Asi")) {
				return event
			}
			...
			if (frames?.length) {
				for (let j = 0; j < frames.length; j++) {
					...
					// The VSIX path may include publisher or product name (e.g. gautammanak2, Asi CLI).
					if (fileName?.includes("gautammanak2") || fileName?.includes("saoudrizwan") || fileName?.includes("Asi")) {
						return event
					}
				}
			}
