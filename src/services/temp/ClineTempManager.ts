/**
 * AsiTempManager - Manages temporary files for Asi with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "Asi" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for Asi's temporary files.
 */
class AsiTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "Asi" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/Asi
		// Windows: C:\Users\{user}\AppData\Local\Temp\Asi
		// Linux: /tmp/Asi
		const baseTempDir = os.tmpdir()
		const AsiTempDir = path.join(baseTempDir, "Asi")

		try {
			fs.mkdirSync(AsiTempDir, { recursive: true })
			this.tempDir = AsiTempDir
		} catch {
	...
	/**
	 * Clean up old Asi temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Asi temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(`Asi temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`)
			}
		} catch (error) {
			Logger.error("Error during Asi temp cleanup", error)
		}
...
export const AsiTempManager = new AsiTempManagerImpl()
