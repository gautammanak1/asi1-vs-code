...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".Asi_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "Asi Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "Asi@example.com"], { cwd: workspacePath })
