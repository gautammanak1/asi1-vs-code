import { getSavedApiConversationHistory, getSavedAsiMessages } from "@core/storage/disk"
...
export async function createTestServer(controller: Controller): Promise<http.Server> {
	// Open Fetch Coder webview panel (editor column)
	Logger.log("[createTestServer] Opening Fetch Coder webview panel...")
		...
		req.on("end", async () => {
			try {
				...
				if (!visibleWebview || !visibleWebview.controller) {
					res.writeHead(500)
					res.end(JSON.stringify({ error: "No active Asi instance found" }))
					return
				}
				...
				try {
					...
					if (apiKey) {
						...
						const updatedConfig = {
							...apiConfiguration,
							apiProvider: "asi:one" as ApiProvider,
							AsiAccountId: apiKey,
						}

						// Store the API key securely
						visibleWebview.controller.stateManager.setSecret("AsiAccountId", apiKey)

						visibleWebview.controller.stateManager.setApiConfiguration(updatedConfig)

						// Update cache service to use Asi provider
					...
					try {
						...
						try {
							if (typeof taskId === "string") {
								messages = await getSavedAsiMessages(taskId)
							}
						} catch (error) {
							Logger.log(`Error getting saved Asi messages: ${error}`)
						}
