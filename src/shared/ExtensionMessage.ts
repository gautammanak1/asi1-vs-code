...
import { AsiFeatureSetting } from "./ClineFeatureSetting"
import { BannerCardData } from "./asi/banner"
import { AsiRulesToggles } from "./asi-rules"
import { FocusChainSettings } from "./FocusChainSettings"
import { HistoryItem } from "./HistoryItem"
import { McpDisplayMode } from "./McpDisplayMode"
import { AsiMessageModelInfo } from "./messages"
import { OnboardingModelGroup } from "./proto/Asi/state"
...
export const COMMAND_CANCEL_TOKEN = "__Asi_command_cancel__"
export interface ExtensionState {
	...
	version: string
	distinctId: string
	globalAsiRulesToggles: AsiRulesToggles
	localAsiRulesToggles: AsiRulesToggles
	localWorkflowToggles: AsiRulesToggles
	globalWorkflowToggles: AsiRulesToggles
	localCursorRulesToggles: AsiRulesToggles
	localWindsurfRulesToggles: AsiRulesToggles
	remoteRulesToggles?: AsiRulesToggles
	remoteWorkflowToggles?: AsiRulesToggles
	localAgentsRulesToggles: AsiRulesToggles
	mcpResponsesCollapsed?: boolean
	strictPlanModeEnabled?: boolean
	yoloModeToggled?: boolean
	useAutoCondense?: boolean
	subagentsEnabled?: boolean
	AsiWebToolsEnabled?: AsiFeatureSetting
	worktreesEnabled?: AsiFeatureSetting
	...
	multiRootSetting: AsiFeatureSetting
...
export interface AsiSayBrowserAction {
	action: BrowserAction
	coordinate?: string
	text?: string
}

export interface AsiSayGenerateExplanation {
	title: string
	fromRef: string
	toRef: string
...
export interface AsiSaySubagentStatus {
...
export interface AsiAskUseMcpServer {
	serverName: string
...
export interface AsiAskUseSubagents {
	prompts: string[]
}

export interface AsiPlanModeResponse {
	response: string
	options?: string[]
	selected?: string
}

export interface AsiAskQuestion {
	question: string
	options?: string[]
	selected?: string
}

export interface AsiAskNewTask {
	context: string
}

export interface AsiApiReqInfo {
	request?: string
	tokensIn?: number
	tokensOut?: number
	cacheWrites?: number
...
export interface AsiSubagentUsageInfo {
	source: "subagents"
	tokensIn: number
	tokensOut: number
	cacheWrites: number
...
export type AsiApiReqCancelReason = "streaming_failed" | "user_cancelled" | "retries_exhausted"
