import { AsiMessage } from "./ExtensionMessage"

/**
 * Hook metadata extracted from hook message text.
 * Mirrors the AsiSayHook interface but represents parsed data.
 */
...
function getSay(msg: AsiMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: AsiMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: AsiMessage): HookMetadata | null {
 ...
 */
function dedupeToolOrCommandMessagesByTimestamp(messages: AsiMessage[]): AsiMessage[] {
...
function combineHookWithOutputs(
	hookMessage: AsiMessage,
	startIndex: number,
	messages: AsiMessage[],
): { combined: AsiMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: AsiMessage[]): AsiMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, AsiMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: AsiMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: AsiMessage[], originalMessages: AsiMessage[]): Map<number, AsiMessage[]> {
	const map = new Map<number, AsiMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: AsiMessage[], preToolUseMap: Map<number, AsiMessage[]>): AsiMessage[] {
	const result: AsiMessage[] = []
 ...
 *
 * @param messages Array of AsiMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: AsiMessage[]): AsiMessage[] {
