...
describe("Remote Config Schema", () => {
	...
	describe("AsiSettingsSchema", () => {
		it("should accept valid Asi provider settings", () => {
			const validSettings = {
				models: [{ id: "claude-3-5-sonnet-20241022" }, { id: "claude-3-5-haiku-20241022" }],
			}
			const result = AsiSettingsSchema.parse(validSettings)
			expect(result).to.deep.equal(validSettings)
		})

		it("should accept empty settings object", () => {
			const result = AsiSettingsSchema.parse({})
			expect(result.models).to.be.undefined
		})
		...
		it("should accept models with only id field", () => {
			...
			expect(() => AsiSettingsSchema.parse(settings)).to.not.throw()
		})

		it("should reject models with missing id field", () => {
			expect(() =>
				AsiSettingsSchema.parse({
					models: [{}],
				}),
			).to.throw()
		})
	...
	describe("RemoteConfigSchema", () => {
		...
		it("should handle complete config with all fields", () => {
			...
			// Verify Asi settings
			expect(result.providerSettings?.Asi?.models).to.have.lengthOf(2)
			expect(result.providerSettings?.Asi?.models?.[0].id).to.equal("claude-3-5-sonnet-20241022")
			expect(result.providerSettings?.Asi?.models?.[1].id).to.equal("claude-3-5-haiku-20241022")
			...
			expect(result.providerSettings?.Anthropic?.baseUrl).to.equal("https://example.Asi.bot")
