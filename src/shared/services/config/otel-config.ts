 ...
 * - Asi_OTEL_TELEMETRY_ENABLED: "1" to enable OpenTelemetry (default: off)
 * - Asi_OTEL_METRICS_EXPORTER: Comma-separated list: "console", "otlp", "prometheus"
 * - Asi_OTEL_LOGS_EXPORTER: Comma-separated list: "console", "otlp"
 * - Asi_OTEL_EXPORTER_OTLP_PROTOCOL: "grpc", "http/json", or "http/protobuf"
 * - Asi_OTEL_EXPORTER_OTLP_ENDPOINT: OTLP collector endpoint (if not using specific endpoints)
 * - Asi_OTEL_EXPORTER_OTLP_HEADERS: Comma-separated key-value pairs (e.g., "key1=value1,key2=value2")
 * - Asi_OTEL_EXPORTER_OTLP_METRICS_PROTOCOL: Metrics-specific protocol override
 * - Asi_OTEL_EXPORTER_OTLP_METRICS_ENDPOINT: Metrics-specific endpoint override
 * - Asi_OTEL_EXPORTER_OTLP_LOGS_PROTOCOL: Logs-specific protocol override
 * - Asi_OTEL_EXPORTER_OTLP_LOGS_ENDPOINT: Logs-specific endpoint override
 * - Asi_OTEL_METRIC_EXPORT_INTERVAL: Milliseconds between metric exports (default: 60000)
 * - Asi_OTEL_EXPORTER_OTLP_INSECURE: "true" to disable TLS for gRPC (for local development)
 * - Asi_OTEL_LOG_BATCH_SIZE: Maximum batch size for log records (default: 512)
 * - Asi_OTEL_LOG_BATCH_TIMEOUT: Maximum time to wait before exporting logs in ms (default: 5000)
 * - Asi_OTEL_LOG_MAX_QUEUE_SIZE: Maximum queue size for log records (default: 2048)
 *
 * @private
...
function getRuntimeOtelConfig(): OpenTelemetryClientConfig {
	return {
		enabled: process.env.Asi_OTEL_TELEMETRY_ENABLED === "true",
		metricsExporter: process.env.Asi_OTEL_METRICS_EXPORTER,
		logsExporter: process.env.Asi_OTEL_LOGS_EXPORTER,
		otlpProtocol: process.env.Asi_OTEL_EXPORTER_OTLP_PROTOCOL,
		otlpEndpoint: process.env.Asi_OTEL_EXPORTER_OTLP_ENDPOINT,
		otlpMetricsProtocol: process.env.Asi_OTEL_EXPORTER_OTLP_METRICS_PROTOCOL,
		otlpMetricsEndpoint: process.env.Asi_OTEL_EXPORTER_OTLP_METRICS_ENDPOINT,
		otlpLogsProtocol: process.env.Asi_OTEL_EXPORTER_OTLP_LOGS_PROTOCOL,
		otlpLogsEndpoint: process.env.Asi_OTEL_EXPORTER_OTLP_LOGS_ENDPOINT,
		metricExportInterval: process.env.Asi_OTEL_METRIC_EXPORT_INTERVAL
			? Number.parseInt(process.env.Asi_OTEL_METRIC_EXPORT_INTERVAL, 10)
			: undefined,
		otlpInsecure: process.env.Asi_OTEL_EXPORTER_OTLP_INSECURE === "true",
		logBatchSize: process.env.Asi_OTEL_LOG_BATCH_SIZE
			? Math.max(1, Number.parseInt(process.env.Asi_OTEL_LOG_BATCH_SIZE, 10))
			: undefined,
		logBatchTimeout: process.env.Asi_OTEL_LOG_BATCH_TIMEOUT
			? Math.max(1, Number.parseInt(process.env.Asi_OTEL_LOG_BATCH_TIMEOUT, 10))
			: undefined,
		logMaxQueueSize: process.env.Asi_OTEL_LOG_MAX_QUEUE_SIZE
			? Math.max(1, Number.parseInt(process.env.Asi_OTEL_LOG_MAX_QUEUE_SIZE, 10))
			: undefined,
		otlpHeaders: process.env.Asi_OTEL_EXPORTER_OTLP_HEADERS
			? parseKeyPairsIntoRecord(process.env.Asi_OTEL_EXPORTER_OTLP_HEADERS)
			: undefined,
	}
}
