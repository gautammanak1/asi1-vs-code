...
export function getBlobStoreSettingsFromEnv(): BlobStoreSettings {
	return {
		adapterType: process?.env?.Asi_STORAGE_ADAPTER || "unknown",
		bucket: process?.env?.Asi_STORAGE_BUCKET || "Asi",
		accessKeyId: process?.env?.Asi_STORAGE_ACCESS_KEY_ID || "",
		secretAccessKey: process?.env?.Asi_STORAGE_SECRET_ACCESS_KEY || "",
		region: process?.env?.Asi_STORAGE_REGION,
		endpoint: process?.env?.Asi_STORAGE_ENDPOINT,
		accountId: process?.env?.Asi_STORAGE_ACCOUNT_ID,

		intervalMs: parseIntEnv(process.env.Asi_STORAGE_SYNC_INTERVAL_MS, 30000),
		maxRetries: parseIntEnv(process.env.Asi_STORAGE_SYNC_MAX_RETRIES, 5),
		batchSize: parseIntEnv(process.env.Asi_STORAGE_SYNC_BATCH_SIZE, 10),
		maxQueueSize: parseIntEnv(process.env.Asi_STORAGE_SYNC_MAX_QUEUE_SIZE, 1000),
		maxFailedAgeMs: parseIntEnv(process.env.Asi_STORAGE_SYNC_MAX_FAILED_AGE_MS, SEVEN_DAYS_MS),
		backfillEnabled: process.env.Asi_STORAGE_SYNC_BACKFILL_ENABLED === "true",
	}
}
