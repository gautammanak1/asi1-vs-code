...
import { AsiStorage } from "./ClineStorage"
...
/**
 * S3/R2 blob storage implementation of AsiStorage.
 * Uses AWS S3 or Cloudflare R2 as the backend storage.
 */
export class AsiBlobStorage extends AsiStorage {
	override name = "AsiBlobStorage"

	private static store: AsiBlobStorage | null = null
	static get instance(): AsiBlobStorage {
		if (!AsiBlobStorage.store) {
			AsiBlobStorage.store = new AsiBlobStorage()
		}
		return AsiBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!AsiBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[AsiBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[AsiBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = AsiBlobStorage.instance
