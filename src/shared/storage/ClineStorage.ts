...
export interface AsiStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: AsiStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and AsiSyncStorage both satisfy this interface.
 */
export interface AsiMemento {
 ...
 */
export abstract class AsiStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "AsiStorage"
	/**
 ...
 * Unlike AsiStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: AsiStorageChangeEvent) => void

export abstract class AsiSyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of AsiStorage using a Map.
 */
export class AsiInMemoryStorage extends AsiStorage {
	/**
