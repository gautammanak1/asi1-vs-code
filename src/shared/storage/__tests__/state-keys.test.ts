...
describe("State Keys Type Safety", () => {
	...
	describe("Type Exports", () => {
		...
		it("should export usable key types", () => {
			...
			const localKey: LocalStateKey = "localAsiRulesToggles"
		...
		it("should have LocalState keys map to AsiRulesToggles", () => {
			const localState: Partial<LocalState> = {
				localAsiRulesToggles: {},
				localCursorRulesToggles: { "some-rule": true },
			}
			expect(localState.localAsiRulesToggles).to.deep.equal({})
		})
	})
	...
	describe("Edge Cases", () => {
		...
		it("should handle keys with special characters", () => {
			// The Asi:AsiAccountId key has a colon
			expect(SecretKeys).to.include("Asi:AsiAccountId")
			expect(isSecretKey("Asi:AsiAccountId")).to.be.true
		})
