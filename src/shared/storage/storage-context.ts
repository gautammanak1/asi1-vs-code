...
import { AsiFileStorage } from "./ClineFileStorage"
import { AsiMemento } from "./ClineStorage"
...
export interface StorageContext {
	...
	readonly globalState: AsiMemento
	 ...
	 *
	 * This split exists because CLI needs to intercept the AsiMemento interface to global state,
	 * but state resets need to write through to the backing store.
	 */
	readonly globalStateBackingStore: AsiFileStorage

	/** Secrets — API keys and other sensitive values. File uses restricted permissions (0o600). */
	readonly secrets: AsiFileStorage<string>

	/** Workspace-scoped state — per-project toggles, rules, etc. */
	readonly workspaceState: AsiFileStorage

	/** The resolved path to the data directory (~/.Asi/data) */
...
export interface StorageContextOptions {
	/**
	 * Override the Asi home directory. Defaults to Asi_DIR env var or ~/.Asi.
	 */
	AsiDir?: string
 ...
 *
 * File layout:
 *   ~/.Asi/data/globalState.json    — global state
 *   ~/.Asi/data/secrets.json        — secrets (mode 0o600)
 *   ~/.Asi/data/workspaces/<hash>/workspaceState.json — per-workspace state
 *
...
export function createStorageContext(opts: StorageContextOptions = {}): StorageContext {
	const AsiDir = opts.AsiDir || process.env.Asi_DIR || path.join(os.homedir(), ".Asi")
	const dataDir = path.join(AsiDir, SETTINGS_SUBFOLDER)
	...
	const globalState = new AsiFileStorage(path.join(dataDir, "globalState.json"), "GlobalState")

	return {
		globalState,
		globalStateBackingStore: globalState,
		secrets: new AsiFileStorage<string>(path.join(dataDir, "secrets.json"), "Secrets", {
			fileMode: 0o600, // Owner read/write only — protects API keys
		}),
		workspaceState: new AsiFileStorage(path.join(workspaceDir, "workspaceState.json"), "WorkspaceState"),
		dataDir,
