...
log("Running standalone Asi", ExtensionRegistryInfo.version)
log(`Asi_ENVIRONMENT: ${process.env.Asi_ENVIRONMENT}`)

// WE WILL HAVE TO MIGRATE THIS FROM DATA TO v1 LATER
const SETTINGS_SUBFOLDER = "data"

export function initializeContext(AsiDir?: string) {
	const Asi_DIR = AsiDir || process.env.Asi_DIR || `${os.homedir()}/.Asi`
	const DATA_DIR = path.join(Asi_DIR, SETTINGS_SUBFOLDER)
	...
	const extensionContext: ExtensionContext = {
		...
		storagePath: WORKSPACE_STORAGE_DIR, // Deprecated, not used in Asi.
		globalStorageUri: URI.file(DATA_DIR),
		globalStoragePath: DATA_DIR, // Deprecated, not used in Asi.

		// Logs are global per extension, not per workspace.
		logUri: URI.file(DATA_DIR),
		logPath: DATA_DIR, // Deprecated, not used in Asi.

		extensionUri: URI.file(EXTENSION_DIR),
		extensionPath: EXTENSION_DIR, // Deprecated, not used in Asi.
