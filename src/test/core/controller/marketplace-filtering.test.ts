...
import { AsiEndpoint, AsiEnv } from "@/config"
...
describe("Controller Marketplace Filtering", () => {
	...
	// Initialize AsiEndpoint before tests run (required for AsiEnv.config() to work)
	before(async () => {
		if (!AsiEndpoint.isInitialized()) {
			await AsiEndpoint.initialize("/test/extension")
		}
	})
	...
	describe("API interaction", () => {
		it("should call marketplace API with correct parameters", async () => {
			...
			callArgs[0].should.equal(`${AsiEnv.config().mcpBaseUrl}/marketplace`)
		})
