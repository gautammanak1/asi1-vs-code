...
This directory contains the end-to-end tests for the Asi VS Code extension using Playwright. These tests simulate user interactions with the extension in a real VS Code environment.
...
- **`fixtures/server/`** - Mock API server for testing Asi's backend interactions
...
- **`sidebar`** - Playwright Frame object for the Asi extension's sidebar
...
await addSelectedCodeToAsiWebview(page)
```
...
2. **Playwright will open:**
   - A VS Code window with Asi extension loaded
...
- **VS Code Configuration:**
  ...
  - Extension development mode with Asi loaded
  - Temporary user data and extensions directories

- **Mock API Server:**
  - Runs on `http://localhost:7777`
  - Provides mock responses for Asi API calls
...
- `Asi_E2E_TESTS_VERBOSE=true` - Enable verbose logging
