...
e2e.describe("Diff Editor", () => {
	E2E_WORKSPACE_TYPES.forEach(({ title, workspaceType }) => {
		e2e.extend({
			...
			await inputbox.fill("[diff.test.ts] Hello, Asi!")
			await expect(inputbox).toHaveValue("[diff.test.ts] Hello, Asi!")
			...
			await expect(sidebar.getByText("Hello, Asi!")).toBeVisible() // History with the previous sent message
			...
			await sidebar.waitForSelector('span:has-text("Asi wants to edit this file:")')

			// Asi Diff Editor should open with the file name and diff
			await expect(page.getByText("test.ts: Original ↔ Asi's")).toBeVisible()
