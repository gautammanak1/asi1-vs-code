...
const replace_in_file = `I successfully replaced "john" with "Asi" in the test.ts file. The change has been completed and the file now contains:

\`\`\`typescript
export const name = "Asi"
\`\`\`
...
<attempt_completion>
<result>
I have successfully replaced the name "john" with "Asi" in the test.ts file. The file now exports:
...
The user wants me to replace the name "john" with "Asi" in the test.ts file. I can see the file content provided:

\`\`\`typescript
export const name = "john"
\`\`\`

I need to change "john" to "Asi". This is a simple targeted edit, so I should use the replace_in_file tool rather than write_to_file since I'm only changing one small part of the file.

I need to:
1. Use replace_in_file to change "john" to "Asi" in the test.ts file
2. The SEARCH block should match the exact content: \`export const name = "john"\`
3. The REPLACE block should be: \`export const name = "Asi"\`
</thinking>

I'll replace "john" with "Asi" in the test.ts file.
...
export const E2E_MOCK_API_RESPONSES = {
	DEFAULT: "Hello! I'm a mock Asi API response.",
