...
export class AsiDataMock {
	...
	public switchToUserType(type: "personal" | "enterprise"): UserResponse {
		const token = AsiDataMock.getDefaultToken(type)
		if (!token) {
	...
	private _createUserResponse(userData: (typeof AsiDataMock.USERS)[0]): UserResponse {
	...
	public getUserByToken(token?: string): UserResponse {
		...
		const actualToken = token || AsiDataMock.getDefaultToken("personal") || "test-personal-token"
	...
	private _getUserByToken(token: string): UserResponse {
		const match = AsiDataMock.findUserByToken(token)
