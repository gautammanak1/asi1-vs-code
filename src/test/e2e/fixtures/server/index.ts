...
import { AsiDataMock } from "./data"

const E2E_API_SERVER_PORT = 7777

export const MOCK_Asi_API_SERVER_URL = `http://localhost:${E2E_API_SERVER_PORT}`

const useVerboseLogging = process.env.Asi_E2E_TESTS_VERBOSE === "true"
function log(...args: unknown[]) {
	if (useVerboseLogging) {
		console.log("[AsiApiServerMock]", ...args)
	}
}

export class AsiApiServerMock {
	static globalSharedServer: AsiApiServerMock | null = null
	...
	public readonly API_USER = new AsiDataMock("personal")
	...
	public static async startGlobalServer(): Promise<AsiApiServerMock> {
		log("=== SERVER FIXTURE CALLED ===")
		if (AsiApiServerMock.globalSharedServer) {
			log("Using existing global server")
			return AsiApiServerMock.globalSharedServer
		}

		log("Starting global server...")
		const server = createServer((req: IncomingMessage, res: ServerResponse) => {
			// Parse URL and method
			const parsedUrl = new URL(req.url || "/", MOCK_Asi_API_SERVER_URL)
			...
			if (isAuthRequired && authToken) {
				...
				const user = AsiApiServerMock.globalSharedServer!.API_USER.getUserByToken(authToken)
				if (!user) {
					return sendApiError("Invalid token", 401)
				}
				AsiApiServerMock.globalSharedServer!.setCurrentUser(user)
			}
			...
			const handleRequest = async () => {
				...
				const routeMatch = AsiApiServerMock.matchRoute(path, method)
				...
				const controller = AsiApiServerMock.globalSharedServer!
				...
				if (baseRoute === "/api/v1") {
					...
					if (endpoint === "/auth/token" && method === "POST") {
						...
						// Return format matching AsiAuthProvider expectations
						return sendApiResponse({
					...
					if (endpoint === "/chat/completions" && method === "POST") {
						...
						if (body.includes("[diff.test.ts] Hello, Asi!")) {
							...
							choices: [
								{
									...
									message: {
										role: "assistant",
										content: "Hello! I'm a mock Asi API response.",
									},
									finish_reason: "stop",
								},
							],
		...
		const controller = new AsiApiServerMock(server)
		AsiApiServerMock.globalSharedServer = controller

		// Track connections for proper cleanup
		server.on("connection", (socket) => {
			AsiApiServerMock.globalSockets.add(socket)
			socket.on("close", () => {
				AsiApiServerMock.globalSockets.delete(socket)
			})
		})
		...
		await new Promise<void>((resolve, reject) => {
			server.listen(E2E_API_SERVER_PORT, (error?: Error) => {
				if (error) {
					...
					log(`AsiApiServerMock listening on port ${E2E_API_SERVER_PORT}`)
					resolve()
				}
			})
		})
	...
	public static async stopGlobalServer(): Promise<void> {
		...
		const server = AsiApiServerMock.globalSharedServer.server

		// Clean shutdown - destroy all socket connections first
		AsiApiServerMock.globalSockets.forEach((socket) => socket.destroy())
		AsiApiServerMock.globalSockets.clear()
		...
		AsiApiServerMock.globalSharedServer = null
	}
}
