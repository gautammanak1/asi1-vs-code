...
/**
 * NOTE: Use the `e2e` test fixture for all E2E tests to test the Asi extension.
 *
 * Extended Playwright test configuration for Asi E2E testing.
 *
 * This test configuration provides a comprehensive setup for end-to-end testing of the Asi VS Code extension,
 ...
 *
 * Fixtures provided:
 * - `server`: Shared AsiApiServerMock instance for API mocking (reused across all tests)
 ...
 * - `page`: Playwright Page object representing the main VS Code window with Asi sidebar opened
 * - `sidebar`: Playwright Frame object representing the Asi extension's sidebar iframe
 *
 * @returns Extended test object with all fixtures available for E2E test scenarios:
 * - **server**: Automatically starts and manages a AsiApiServerMock instance
 ...
 * - **page**: Configures the main VS Code window with notifications disabled and Asi sidebar open
 * - **sidebar**: Provides access to the Asi extension's sidebar frame
 *
 * @example
 * ```typescript
 ...
 * - Installs the Asi extension in development mode
...
export const e2e = test
	.extend<{ server: AsiApiServerMock | null }>({
		server: async ({}, use) => {
