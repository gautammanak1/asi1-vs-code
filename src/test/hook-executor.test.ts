...
`
	...
	beforeEach(async () => {
		...
		// Create .Asirules/hooks subdirectory structure
		tempDir = path.join(baseTempDir, ".Asirules", "hooks")
		await fs.mkdir(tempDir, { recursive: true })
		testHandler = createTestHandler()
		mockMessages = []

		// Mock StateManager to return baseTempDir as workspace root
		// This allows HookFactory to find hooks in baseTempDir/.Asirules/hooks/
	...
	describe("Error Handling", () => {
		...
		it("should update message state on hook failure", async function () {
			...
			const mockHandler = {
				...testHandler,
				getAsiMessages: () => messages,
				addToAsiMessages: async (msg: AsiMessage) => {
					messages.push(msg)
				},
				updateAsiMessage: async (index: number, updates: Partial<AsiMessage>) => {
					if (messages[index]) {
