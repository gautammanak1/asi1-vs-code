...
describe("Hook Management Integration", () => {
	...
	beforeEach(async () => {
		...
		tempDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "hook-integration-test-"),
		);
	...
	describe("Complete Hook Lifecycle", () => {
		it("should support full lifecycle: create -> verify disabled -> enable -> verify enabled -> delete -> verify gone", async function () {
			...
			const createResponse = await createHook(
				mockController,
				createRequest,
				globalHooksDir,
			);
			...
			const enableResponse = await toggleHook(
				mockController,
				enableRequest,
				globalHooksDir,
			);
			...
			const disableResponse = await toggleHook(
				mockController,
				disableRequest,
				globalHooksDir,
			);
			...
			if (isWindows) {
				...
				disableResponse.hooksToggles!.globalHooks[0].enabled.should.equal(
					false,
				);
			...
			const deleteResponse = await deleteHook(
				mockController,
				deleteRequest,
				globalHooksDir,
			);
		...
		it("should handle multiple global hooks with independent states", async function () {
			...
			const hooks = await refreshHooks(
				mockController,
				undefined,
				globalHooksDir,
			);
			...
			const hooksAfterToggle = await refreshHooks(
				mockController,
				undefined,
				globalHooksDir,
			);
			const taskStart = hooksAfterToggle.globalHooks.find(
				(h) => h.name === "TaskStart",
			);
			const taskResume = hooksAfterToggle.globalHooks.find(
				(h) => h.name === "TaskResume",
			);
			const userPrompt = hooksAfterToggle.globalHooks.find(
				(h) => h.name === "UserPromptSubmit",
			);
			const taskComplete = hooksAfterToggle.globalHooks.find(
				(h) => h.name === "TaskComplete",
			);
			...
			// Verify all are gone
			const finalHooks = await refreshHooks(
				mockController,
				undefined,
				globalHooksDir,
			);
	...
	describe("Cache Invalidation", () => {
		it("should properly invalidate cache across all operations", async function () {
			...
			const hookPath = path.join(
				globalHooksDir,
				isWindows ? "TaskStart.ps1" : "TaskStart",
			);
			if (!isWindows) {
