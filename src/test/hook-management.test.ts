...
describe("Hook Management", () => {
	...
	beforeEach(async () => {
		...
		globalHooksDir = path.join(tempDir, "global", "Documents", "Asi", "Hooks");
		workspaceHooksDir = path.join(tempDir, "workspace", ".Asirules", "hooks");
	...
	describe("createHook", () => {
		...
		it("should create Notification hook with valid template", async function () {
			...
			response
				.hooksToggles!.globalHooks.some((h) => h.name === "Notification")
				.should.equal(true);
		});
	});
	...
	describe("toggleHook", () => {
		...
		it("should return updated hooks state", async function () {
			...
			const response = await toggleHook(
				mockController,
				request,
				globalHooksDir,
			);
	...
	describe("refreshHooks", () => {
		it("should discover hooks in global directory", async function () {
			...
			if (isWindows) {
				await fs.writeFile(
					path.join(globalHooksDir, "TaskStart.ps1"),
					"Write-Output '{}'",
					{ mode: 0o644 },
				);
				await fs.writeFile(
					path.join(globalHooksDir, "TaskResume.ps1"),
					"Write-Output '{}'",
					{ mode: 0o644 },
				);
			} else {
				await fs.writeFile(
					path.join(globalHooksDir, "TaskStart"),
					"#!/usr/bin/env node",
					{ mode: 0o755 },
				);
				await fs.writeFile(
					path.join(globalHooksDir, "TaskResume"),
					"#!/usr/bin/env node",
					{ mode: 0o644 },
				);
			}

			const result = await refreshHooks(
				mockController,
				undefined,
				globalHooksDir,
			);
		...
		it("should discover hooks in workspace directories", async function () {
			...
			await fs.writeFile(
				path.join(workspaceHooksDir, hookFileName("UserPromptSubmit")),
				hookTemplate,
				{
					mode: 0o755,
				},
		...
		it("should correctly identify executable vs non-executable hooks", async function () {
			...
			await fs.writeFile(
				path.join(globalHooksDir, "TaskCancel"),
				"#!/usr/bin/env node",
				{ mode: 0o644 },
			);
			...
			const taskCancel = result.globalHooks.find(
				(h) => h.name === "TaskCancel",
			);
		...
		it("should include absolute paths in hook info", async function () {
			...
			if (isWindows) {
				await fs.writeFile(
					path.join(globalHooksDir, "TaskComplete.ps1"),
					"Write-Output '{}'",
					{ mode: 0o644 },
				);
			} else {
				await fs.writeFile(
					path.join(globalHooksDir, "TaskComplete"),
					"#!/usr/bin/env node",
					{ mode: 0o755 },
				);
			}
			...
			result.globalHooks[0].absolutePath.should.equal(
				path.join(globalHooksDir, hookFileName("TaskComplete")),
			);
		});
		...
		it("should resolve .ps1-only hooks on Windows (extensionless is unsupported)", async function () {
			...
			await withPlatform("win32", async () => {
				...
				const taskStart = result.globalHooks.find(
					(h) => h.name === "TaskStart",
				);
				should.exist(taskStart);
				taskStart!.absolutePath.should.equal(
					path.join(globalHooksDir, "TaskStart.ps1"),
				);
				taskStart!.enabled.should.equal(true);
			});
		...
		it("should ignore extensionless hook on Windows and use .ps1 only", async function () {
			...
			await withPlatform("win32", async () => {
				...
				const taskResume = result.globalHooks.find(
					(h) => h.name === "TaskResume",
				);
				should.exist(taskResume);
				taskResume!.absolutePath.should.equal(
					path.join(globalHooksDir, "TaskResume.ps1"),
				);
			});
		});
	...
	describe("Edge Cases", () => {
		it("should handle missing .Asirules directory gracefully", async function () {
			this.timeout(5000);
		...
		it("should toggle and delete .ps1-only hooks on Windows", async function () {
			...
			await withPlatform("win32", async () => {
				...
				const toggled = toggleResponse.hooksToggles!.globalHooks.find(
					(h) => h.name === "TaskCancel",
				);
