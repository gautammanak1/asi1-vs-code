...
describe("getAvailableSlashCommands", () => {
	...
	describe("Base Slash Commands", () => {
		...
		it("should not include the deprecated subagent slash command", async () => {
			...
			const deprecatedCommand = response.commands.find(
				(cmd) => cmd.name === "subagent",
			);
	...
	describe("Local Workflow Toggles", () => {
		...
		it("should extract filename from full path", async () => {
			mockStateManager.getWorkspaceStateKey
				.withArgs("workflowToggles")
				.returns({
					"/Users/test/project/.Asirules/workflows/deep-analysis.md": true,
				});
		...
		it("should handle Windows-style paths", async () => {
			mockStateManager.getWorkspaceStateKey
				.withArgs("workflowToggles")
				.returns({
					"C:\\Users\\test\\project\\.Asirules\\workflows\\windows-workflow.md": true,
				});
