...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "Asi-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .Asirules/workflows directory specifically", async () => {
		// Create a test directory structure
		const AsirulesDirTest = path.join(tmpDir, "Asirules-test")
		const AsirulesDirPath = path.join(AsirulesDirTest, ".Asirules")

		// Create .Asirules directory and root files
		await fs.mkdir(AsirulesDirPath, { recursive: true })
		await fs.writeFile(path.join(AsirulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(AsirulesDirPath, "settings.js"), "// settings")

		// Create .Asirules/other directory and files
		const otherDirPath = path.join(AsirulesDirPath, "other")
		...
		// Create .Asirules/workflows directory and files
		const workflowsDirPath = path.join(AsirulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(AsirulesDirPath)
		...
		const filteredFiles = await readDirectory(AsirulesDirPath, [[".Asirules", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(AsirulesDirPath, "config.json"),
			path.resolve(AsirulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(AsirulesDirPath, [
			[".Asirules", "workflows"],
			[".Asirules", "other"],
		])
		...
		const rootOnlyFiles = [path.resolve(AsirulesDirPath, "config.json"), path.resolve(AsirulesDirPath, "settings.js")]

		multiExcludeFiles.sort().should.deepEqual(rootOnlyFiles.sort())
	})

	it("should exclude .Asirules/hooks directory specifically", async () => {
		// Create a test directory structure
		const AsirulesDirTest = path.join(tmpDir, "Asirules-hooks-test")
		const AsirulesDirPath = path.join(AsirulesDirTest, ".Asirules")
		...
		// Create .Asirules/hooks directory and files
		const hooksDirPath = path.join(AsirulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(AsirulesDirPath, [[".Asirules", "hooks"]])
