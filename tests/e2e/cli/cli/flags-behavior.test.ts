// ---------------------------------------------------------------------------
// CLI flag behavioral tests
//
// These tests verify the runtime behavior of each CLI flag — not just that
// the flag appears in --help output (that's covered in tests/flags.test.ts),
// but that the flag actually changes what Asi does.
//
// Tests marked ⚠️ in the spec reflect known gaps where the flag is accepted
// but currently has no observable effect. They are still written so the
// behavior can be asserted once the implementation catches up.
// ---------------------------------------------------------------------------

import { test } from "@microsoft/tui-test"
import { Asi_BIN, TERMINAL_WIDE } from "../helpers/constants.js"
import { AsiEnv } from "../helpers/env.js"
import { expectVisible } from "../utils.js"

// ---------------------------------------------------------------------------
// Asi --act
// Starts Asi in Act mode regardless of globalState
// ---------------------------------------------------------------------------
test.describe("Asi --act", () => {
	test.use({
		program: { file: Asi_BIN, args: ["--tui", "--act"] },
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts in Act mode", async ({ terminal }) => {
		await expectVisible(terminal, "Act")
	})
})

// ---------------------------------------------------------------------------
// Asi --plan
// Starts Asi in Plan mode regardless of globalState
// ---------------------------------------------------------------------------
test.describe("Asi --plan", () => {
	test.use({
		program: { file: Asi_BIN, args: ["--tui", "--plan"] },
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts in Plan mode", async ({ terminal }) => {
		await expectVisible(terminal, "Plan")
	})
})

// ---------------------------------------------------------------------------
// Asi --timeout <n>  ⚠️
// Current behavior: starts interactive mode and ignores timeout value
// ---------------------------------------------------------------------------
test.describe("Asi --timeout (interactive mode, flag ignored) ⚠️", () => {
	test.use({
		program: { file: Asi_BIN, args: ["--tui", "--timeout", "30"] },
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts interactive mode (timeout value currently ignored)", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})

// ---------------------------------------------------------------------------
// Asi --model <model-id>  ⚠️
// Current behavior: starts interactive mode and ignores model value
// ---------------------------------------------------------------------------
test.describe("Asi --model (interactive mode, flag ignored) ⚠️", () => {
	test.use({
		program: {
			file: Asi_BIN,
			args: ["--tui", "--model", "claude-3-5-haiku-20241022"],
		},
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts interactive mode (model value currently ignored)", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})

// ---------------------------------------------------------------------------
// Asi --verbose  ⚠️
// Current behavior: starts interactive mode and ignores verbose value
// ---------------------------------------------------------------------------
test.describe("Asi --verbose (interactive mode, flag ignored) ⚠️", () => {
	test.use({
		program: { file: Asi_BIN, args: ["--tui", "--verbose"] },
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts interactive mode (verbose value currently ignored)", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})

// ---------------------------------------------------------------------------
// Asi -c / Asi --cwd <dir>  ⚠️
// Starts Asi in interactive mode with the cwd present in the client footer
// ---------------------------------------------------------------------------
test.describe("Asi --cwd <dir>", () => {
	test.use({
		program: { file: Asi_BIN, args: ["--tui", "--cwd", "/tmp"] },
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts interactive mode with --cwd flag", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})

test.describe("Asi -c <dir> (short alias)", () => {
	test.use({
		program: { file: Asi_BIN, args: ["--tui", "-c", "/tmp"] },
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts interactive mode with -c flag", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})

// ---------------------------------------------------------------------------
// Asi --config <dir>
// Starts Asi in interactive mode using settings from the custom config dir
// ---------------------------------------------------------------------------
test.describe("Asi --config (claude-sonnet-4.6)", () => {
	test.use({
		program: {
			file: Asi_BIN,
			args: ["--tui", "--config", "configs/claude-sonnet-4.6"],
		},
		...TERMINAL_WIDE,
		env: AsiEnv("claude-sonnet-4.6"),
	})

	test("starts interactive mode with custom config directory", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})

// ---------------------------------------------------------------------------
// Asi --thinking  ⚠️
// Starts Asi in interactive mode with thinking turned on regardless of globalState
// (if thinking not supported, this flag is a no-op)
// ---------------------------------------------------------------------------
test.describe("Asi --thinking ⚠️", () => {
	test.use({
		program: { file: Asi_BIN, args: ["--tui", "--thinking"] },
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts interactive mode with --thinking flag", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})

// ---------------------------------------------------------------------------
// Asi --reasoning-effort <level>  ⚠️
// Starts Asi in interactive mode with reasoning turned on regardless of globalState
// (if reasoning not supported, this flag is a no-op)
// ---------------------------------------------------------------------------
test.describe("Asi --reasoning-effort ⚠️", () => {
	test.use({
		program: { file: Asi_BIN, args: ["--tui", "--reasoning-effort", "high"] },
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts interactive mode with --reasoning-effort flag", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})

// ---------------------------------------------------------------------------
// Asi --max-consecutive-mistakes <n>
// ---------------------------------------------------------------------------
test.describe("Asi --max-consecutive-mistakes", () => {
	test.use({
		program: {
			file: Asi_BIN,
			args: ["--tui", "--max-consecutive-mistakes", "5"],
		},
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts interactive mode with --max-consecutive-mistakes flag", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})

// ---------------------------------------------------------------------------
// Asi --double-check-completion
// ---------------------------------------------------------------------------
test.describe("Asi --double-check-completion", () => {
	test.use({
		program: { file: Asi_BIN, args: ["--tui", "--double-check-completion"] },
		...TERMINAL_WIDE,
		env: AsiEnv("default"),
	})

	test("starts interactive mode with --double-check-completion flag", async ({ terminal }) => {
		await expectVisible(terminal, /what can i do|plan|act/i)
	})
})
