...
const AccountView = ({ onDone }: AccountViewProps) => {
	...
	const { environment, remoteConfigSettings, navigateToSettings } =
		useExtensionState();
...
const AsiAccountSummary = ({
	...
	const isLockedByRemoteConfig =
		Object.keys(remoteConfigSettings || {}).length > 0;

	const [dropdownValue, setDropdownValue] = useState<string>(
		activeOrganization?.organizationId || uid,
	);
			...
			<div className="flex flex-col w-full gap-1 mb-6">
				<div className="flex items-center flex-wrap gap-y-4">
					...
					<div className="flex flex-col">
						{displayName && (
							<h2 className="text-foreground m-0 text-lg font-medium">
								{displayName}
							</h2>
						)}
						...
						<div className="flex gap-2 items-center mt-1">
							<Tooltip>
								...
								<TooltipContent hidden={!isLockedByRemoteConfig}>
									This cannot be changed while your organization has remote
