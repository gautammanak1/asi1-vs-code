 ...
 */
export function convertProtoUsageTransaction(
	protoTransaction: ProtoUsageTransaction,
): AsiAccountUsageTransaction {
	return {
		...protoTransaction,
 ...
 */
export function convertProtoUsageTransactions(
	protoTransactions: ProtoUsageTransaction[],
): AsiAccountUsageTransaction[] {
...
export const isAdminOrOwner = (activeOrg: UserOrganization): boolean => {
	return (
		activeOrg.roles.findIndex((role) => role === "admin" || role === "owner") >
		-1
	);
};
