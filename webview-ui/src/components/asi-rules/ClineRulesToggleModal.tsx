...
const AsiRulesToggleModal: React.FC = () => {
	const {
		...
		hooksEnabled,
		setGlobalAsiRulesToggles,
		setLocalAsiRulesToggles,
	...
	const globalRules = Object.entries(globalAsiRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b));

	// Format local rules for display with proper typing
	const localRules = Object.entries(localAsiRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b));
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && (
						<TooltipContent>Manage Asi Rules & Workflows</TooltipContent>
					)}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={
								isVisible
									? "Hide Asi Rules & Workflows"
									: "Show Asi Rules & Workflows"
							}
				...
				>
					...
					<div className="flex-shrink-0 px-3 pt-2">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								<p>
									Rules allow you to provide Asi with system-level guidance.
									Think of them as a persistent way to include context and
									preferences for your projects or globally for every
									conversation.{" "}
									<VSCodeLink
										className="text-xs"
										href="https://docs.fetch.ai/features/asi-rules"
								...
								<p>
									Workflows allow you to define a series of steps to guide Asi
									through a repetitive set of tasks, such as deploying a service
									or submitting a PR. To invoke a workflow, type{" "}
									...
									<VSCodeLink
										...
										href="https://docs.fetch.ai/features/slash-commands/workflows"
									>
										Docs
									</VSCodeLink>
								</p>
								...
								<p>
									Skills are reusable instruction sets that Asi can activate
									on-demand. When a task matches a skill's description, Asi uses
									the <span className="font-bold">use_skill</span> tool to load
									the full instructions.
								</p>
							) : (
								<p>
									Hooks allow you to execute custom scripts at specific points
									in Asi's execution lifecycle, enabling automation and
							...
							<>
								<div className="text-xs text-description mb-4">
									<p>
										...
										<VSCodeLink
											className="text-xs"
											href="https://docs.fetch.ai/features/hooks"
											style={{ display: "inline", fontSize: "inherit" }}
										>
											Docs
										</VSCodeLink>
								...
								{isWindows && (
									<div className="flex items-center gap-2 px-3 py-3 mb-4 bg-vscode-inputValidation-warningBackground border-l-[3px] border-vscode-inputValidation-warningBorder">
										...
										<span className="text-base">
											Hook toggling is not yet supported on Windows in this
											foundation PR. Hooks can be created, edited, and deleted,
											and execute whenever the hook file exists. Coming next:
								...
								{workspaceHooks.map((workspace, index) => (
									...
									>
										<div className="text-sm font-normal mb-2">
											{workspace.workspaceName}/.Asirules/hooks/
										</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
...
export default AsiRulesToggleModal;
