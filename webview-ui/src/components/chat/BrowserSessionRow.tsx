...
const BrowserSessionRow = memo((props: BrowserSessionRowProps) => {
	const { messages, isLast, onHeightChange, lastModifiedMessage, onSetQuote } =
		props;
	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string;
				messages: AsiMessage[]; // messages up to and including the result
			};
			nextAction?: {
				messages: AsiMessage[]; // messages leading to next result
			};
		}[] = [];

		let currentStateMessages: AsiMessage[] = [];
		let nextActionMessages: AsiMessage[] = [];
	...
	const _shouldShowSettings = useMemo(() => {
		...
		return (
			lastMessage?.ask === "browser_action_launch" ||
			lastMessage?.say === "browser_action_launch"
		);
	}, [messages]);
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved
						? "Asi is using the browser:"
						: "Asi wants to use the browser:"}
				</span>
			</div>
			<div
				style={{
