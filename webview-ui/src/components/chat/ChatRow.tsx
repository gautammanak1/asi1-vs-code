...
interface ChatRowProps {
	...
	lastModifiedMessage?: AsiMessage
	isLast: boolean
	...
	({
		...
		const [cost, apiReqCancelReason, apiReqStreamingFailedMessage] = useMemo(() => {
			if (message.text != null && message.say === "api_req_started") {
				const info: AsiApiReqInfo = JSON.parse(message.text)
			...
			switch (type) {
				...
				case "mistake_limit_reached":
					return [
						...
						<span className="text-error font-bold">Asi is having trouble...</span>,
					]
				case "command":
					return [
						<TerminalIcon className="text-foreground size-2" />,
						<span className="font-bold text-foreground">Asi wants to execute this command:</span>,
					]
				case "use_mcp_server":
					const mcpServerUse = JSON.parse(message.text || "{}") as AsiAskUseMcpServer
					return [
						...
						<span className="ph-no-capture font-bold text-foreground break-words">
							Asi wants to {mcpServerUse.type === "use_mcp_tool" ? "use a tool" : "access a resource"} on the{" "}
				...
				case "followup":
					return [
						...
						<span className="font-bold text-foreground">Asi has a question:</span>,
					]
				default:
					return [null, null]
			}
		...
		const tool = useMemo(() => {
			if (message.ask === "tool" || message.say === "tool") {
				return JSON.parse(message.text || "{}") as AsiSayTool
			}
			return null
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span style={{ fontWeight: "bold" }}>Asi wants to delete this file:</span>
							</div>
							<CodeAccordian
								// isLoading={message.partial}
								code={tool.content}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span className="font-bold">Asi wants to create a new file:</span>
							</div>
							{backgroundEditEnabled && tool.path && tool.content ? (
						...
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									Asi wants to search this directory for <code className="break-all">{tool.regex}</code>:
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">Asi is condensing the conversation:</span>
							</div>
		...
		if (message.ask === "use_mcp_server" || message.say === "use_mcp_server") {
			const useMcpServer = JSON.parse(message.text || "{}") as AsiAskUseMcpServer
			...
			case "say":
				switch (message.say) {
					...
					case "Asiignore_error":
						return <ErrorRow errorType="Asiignore_error" message={message} />
					...
					case "generate_explanation": {
						let explanationInfo: AsiSayGenerateExplanation = {
							title: "code changes",
							fromRef: "",
							toRef: "",
							status: "generating",
						...
						return (
							<div className="flex flex-col bg-warning/20 p-2 rounded-xs border border-error">
								...
								<div className="text-foreground opacity-80">
									Asi may have trouble viewing the command's output. Please update VSCode (
			...
			case "ask":
				switch (message.ask) {
					...
					case "followup":
						...
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as AsiAskQuestion
					...
					case "new_task":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Asi wants to start a new task:</span>
								</div>
					...
					case "condense":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Asi wants to condense your conversation:</span>
								</div>
								<NewTaskPreview context={message.text || ""} />
							</div>
						)
					case "report_bug":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Asi wants to create a Github issue:</span>
								</div>
					...
					case "plan_mode_respond": {
						...
						let selected: string | undefined
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as AsiPlanModeResponse
