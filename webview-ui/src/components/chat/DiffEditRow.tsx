...
const FileBlock = memo<{
	...
	({ file, isStreaming, startLineNumber }) => {
		...
		const actionStyle =
			ACTION_STYLES[file.action as keyof typeof ACTION_STYLES] ??
...
const DiffStats = memo<{ additions: number; deletions: number }>(
	({ additions, deletions }) => (
...
const DiffLine = memo<{
	...
	const code =
		isAddition || isDeletion ? line.slice(hasSpacePrefix ? 2 : 1) : line;
...
function parseAllSearchReplaceBlocks(patch: string, path: string): Patch[] {
	...
	for (let i = 0; i < searchPositions.length; i++) {
		...
		const end =
			i < searchPositions.length - 1 ? searchPositions[i + 1] : patch.length;
 ...
 */
function parseSearchReplaceFormat(
	patch: string,
	path: string,
): Patch | undefined {
	...
	const searchContent = afterSearch
