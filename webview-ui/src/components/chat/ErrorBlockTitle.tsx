...
export const ErrorBlockTitle = ({
	...
	const title = (() => {
		...
		if (apiReqCancelReason === "user_cancelled") {
			...
			const parsedAsiError = AsiError.parse(apiRequestFailedMessage)
			const titleText = parsedAsiError?.isErrorType(AsiErrorType.Balance) ? "Credit Limit Reached" : "API Request Failed"
