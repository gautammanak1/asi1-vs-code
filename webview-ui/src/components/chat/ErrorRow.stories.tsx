...
// Mock data factories
const createMockMessage = (overrides: Partial<AsiMessage> = {}): AsiMessage => ({
	ts: Date.now(),
	type: "say",
	say: "error",
...
const meta: Meta<typeof ErrorRow> = {
	...
	parameters: {
		docs: {
			description: {
				component:
					"Displays different types of error messages in the chat interface, including API errors, credit limit errors, diff errors, and Asiignore errors. Handles special error parsing for Asi provider errors and provides appropriate user actions.",
			},
		},
	},
...
export const Default: Story = {
	...
	argTypes: {
		errorType: {
			control: { type: "select" },
			options: ["error", "mistake_limit_reached", "diff_error", "Asiignore_error"],
...
// Asi-specific errors
export const AsiBalanceError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
...
export const AsiRateLimitError: Story = {
	args: {
		message: createMockMessage(),
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
...
export const AuthErrorSignedIn: Story = {
	...
	decorators: [
		createStoryDecorator({
			AsiUser: { id: "user123", email: "user@example.com" },
...
export const InteractiveSignIn: Story = {
	args: {
		...
		errorType: "error",
		apiRequestFailedMessage: JSON.stringify({
			message: "Please sign in to access Asi services.",
			code: "ERR_BAD_REQUEST",
	...
	play: async ({ canvasElement }) => {
		...
		const signInButton = canvas.getByRole("button", { name: /sign in to Asi/i })
