...
const collectToolsInRange = (
	...
	startIdx: number,
	endIdx: number,
	stopCondition?: (msg: AsiMessage) => boolean,
...
const findCurrentApiReq = (messages: AsiMessage[]): { index: number; hasCost: boolean } | null => {
...
const findPrevCompletedApiReq = (messages: AsiMessage[], beforeIdx: number): number => {
...
export const RequestStartRow: React.FC<RequestStartRowProps> = ({
	...
	const hasCompletionResult = AsiMessages.some(
	...
	const willBeAbsorbed = useMemo(() => {
		return isApiReqAbsorbable(message.ts, AsiMessages)
	}, [message.ts, AsiMessages])
	...
	const currentActivities = useMemo(() => {
		const currentApiReq = findCurrentApiReq(AsiMessages)
		if (!currentApiReq) {
			return []
		}
		...
		if (!currentApiReq.hasCost) {
			...
			return collectToolsInRange(AsiMessages, currentApiReq.index + 1, AsiMessages.length)
		}
	...
	const hasCompletedTools = useMemo(() => {
		...
		return AsiMessages.some((msg, idx) => {
