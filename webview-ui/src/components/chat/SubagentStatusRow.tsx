...
function parseSubagentRowData(message: AsiMessage): SubagentRowData | null {
	if (!message.text) {
		return null
	}
	...
	try {
		if (message.ask === "use_subagents" || message.say === "use_subagents") {
			const parsed = JSON.parse(message.text) as AsiAskUseSubagents
		...
		const parsed = JSON.parse(message.text) as AsiSaySubagentStatus
...
export default function SubagentStatusRow({ message, isLast, lastModifiedMessage }: SubagentStatusRowProps) {
	...
	const title = singular ? "Asi wants to use a subagent:" : "Asi wants to use subagents:"
		...
		<div className="mb-2">
			...
			<div className="space-y-2">
				{data.items.map((entry, index) => {
					...
					const statsText = `${formatCount(entry.toolCalls)} tools called`
