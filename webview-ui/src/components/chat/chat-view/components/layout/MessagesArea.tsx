...
export const MessagesArea: React.FC<MessagesAreaProps> = ({
	...
	const { AsiMessages } = useExtensionState()
	const lastRawMessage = useMemo(() => AsiMessages.at(-1), [AsiMessages])
	...
	const scrolledPastUserMessageIndex = useMemo(() => {
		...
		return AsiMessages.findIndex((msg) => msg.ts === scrolledPastUserMessage.ts)
	}, [AsiMessages, scrolledPastUserMessage])
	...
	const displayedGroupedMessages = useMemo<(AsiMessage | AsiMessage[])[]>(() => {
		if (!showThinkingLoaderRow) {
			return groupedMessages
		}
		const waitingRow: AsiMessage = {
