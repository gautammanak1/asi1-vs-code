...
const getActivityText = (tool: AsiSayTool): string | null => {
	const cleanedPath = cleanPathPrefix(tool.path || "");
	const formatSearchRegex = (
		regex: string,
		path: string,
		filePattern?: string,
	): string => {
...
const getCurrentActivities = (allMessages: AsiMessage[]): AsiMessage[] => {
 ...
 */
export const ToolGroupRenderer = memo(
	({ messages, allMessages, isLastGroup }: ToolGroupRendererProps) => {
		...
		const filteredMessages = useMemo(
			() => getToolsNotInCurrentActivities(messages, allMessages),
		...
		const completedTools = useMemo(
			() => buildToolsWithReasoning(filteredMessages),
			[filteredMessages],
		);
		...
		const allTools = useMemo(() => {
			...
			const activePaths = new Set(
				activeTools.map((item) => item.parsedTool.path).filter(Boolean),
			);

			// Filter out completed items that are also being actively read
			const dedupedCompleted = completedTools.filter(
				(item) => !activePaths.has(item.parsedTool.path),
			);
		...
		return (
			<div className={cn("px-4 py-2 ml-1 text-description")}>
				{/* Header */}
				<div className="text-[13px] text-description font-semibold mb-1">
					{summary}:
				</div>
 ...
 */
export function buildToolsWithReasoning(
	messages: AsiMessage[],
): ToolWithReasoning[] {
 ...
 */
function parseToolSafe(text: string | undefined): AsiSayTool {
	try {
		return JSON.parse(text || "{}") as AsiSayTool;
	} catch {
		return {} as AsiSayTool;
	}
}
 ...
 */
function getToolDisplayInfo(tool: AsiSayTool) {
	...
	switch (tool.tool) {
		case "readFile": {
			...
			return {
				...
				path: filePath,
				label: "read",
				displayText: lineNote
					? `${cleanPathPrefix(filePath)} · ${lineNote}`
					: undefined,
			};
		}
		case "listFilesTopLevel":
		...
		case "searchFiles":
			return {
				...
				displayText: formatSearchDisplay(
					tool.regex || "",
					filePath,
					tool.filePattern,
				),
...
function formatSearchDisplay(
	...
	const termDisplay =
		terms.length > 3 ? `${terms.length} patterns` : `"${terms.join(" | ")}"`;
 ...
 */
export function getToolGroupSummaryFromParsedTools(
	tools: AsiSayTool[],
): string {
	...
	if (counts.search > 0) {
		parts.push(
			`performed ${counts.search} search${counts.search > 1 ? "es" : ""}`,
		);
	}
