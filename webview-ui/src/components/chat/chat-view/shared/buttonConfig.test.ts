...
describe("getButtonConfig", () => {
	...
	it("returns partial config for streaming messages", () => {
		const streamingMessage: AsiMessage = {
			type: "say",
			say: "api_req_started",
			partial: true,
			ts: Date.now(),
	...
	describe("Command Execution States", () => {
		it("returns command config for command ask", () => {
			const commandMessage: AsiMessage = {
				type: "ask",
				ask: "command",
				ts: Date.now(),
			}
		...
		it("returns command_output config for command_output ask", () => {
			const commandOutputMessage: AsiMessage = {
				type: "ask",
				ask: "command_output",
				ts: Date.now(),
			}
	...
	it("returns api_req_active config for api_req_started say message", () => {
		const apiReqMessage: AsiMessage = {
			type: "say",
			say: "api_req_started",
			ts: Date.now(),
		}
