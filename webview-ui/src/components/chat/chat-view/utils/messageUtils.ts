...
import type { AsiMessage, AsiSayBrowserAction, AsiSayTool } from "@shared/ExtensionMessage"
 ...
 */
export function isLowStakesTool(message: AsiMessage): boolean {
	if (message.say !== "tool" && message.ask !== "tool") {
		return false
	}
	try {
		const tool = JSON.parse(message.text || "{}") as AsiSayTool
 ...
 */
export function isToolGroup(item: AsiMessage | AsiMessage[]): item is AsiMessage[] & { _isToolGroup: true } {
 ...
 */
export function processMessages(messages: AsiMessage[]): AsiMessage[] {
	...
	return messages.filter((message, index, arr) => {
		switch (message.ask) {
			case "completion_result":
				// don't show a chat row for a completion_result ask without text. This specific type of message only occurs if Asi wants to execute a command as part of its completion result, in which case we interject the completion_result tool with the execute_command tool.
				if (message.text === "") {
					return false
				}
				break
		...
		switch (message.say) {
			...
			case "text":
				// Sometimes Asi returns an empty text message, we don't want to render these. (We also use a say text for user messages, so in case they just sent images we still render that)
 ...
 */
export function isBrowserSessionMessage(message: AsiMessage): boolean {
	if (message.type === "ask") {
 ...
 */
export function groupMessages(visibleMessages: AsiMessage[]): (AsiMessage | AsiMessage[])[] {
	const result: (AsiMessage | AsiMessage[])[] = []
	let currentGroup: AsiMessage[] = []
		...
		if (message.ask === "browser_action_launch" || message.say === "browser_action_launch") {
			...
			if (isBrowserSessionMessage(message)) {
				...
				if (message.say === "browser_action") {
					const browserAction = JSON.parse(message.text || "{}") as AsiSayBrowserAction
 ...
 */
export function getTaskMessage(messages: AsiMessage[]): AsiMessage | undefined {
 ...
 */
function isDisplayedCheckpoint(checkpointIndex: number, allMessages: AsiMessage[]): boolean {
	...
	for (let i = checkpointIndex - 1; i >= 0; i--) {
		...
		if (msg.say === "tool" || msg.ask === "tool") {
			try {
				const tool = JSON.parse(msg.text || "{}") as AsiSayTool
 ...
 */
export function findNextSegmentCost(checkpointTs: number, allMessages: AsiMessage[]): number | undefined {
	const checkpointIndex = allMessages.findIndex((m) => m.ts === checkpointTs && m.say === "checkpoint_created")
	if (checkpointIndex === -1) {
		return undefined
	}
 ...
 */
export function isTextMessagePendingToolCall(textTs: number, allMessages: AsiMessage[]): boolean {
 ...
 */
export function isToolGroupInFlight(toolGroupMessages: AsiMessage[], allMessages: AsiMessage[]): boolean {
	if (toolGroupMessages.length === 0) {
		return false
	}

	// Step 1: Find the MOST RECENT api_req_started overall (search backwards)
	let mostRecentApiReq: AsiMessage | null = null
 ...
 */
export function getToolsNotInCurrentActivities(toolGroupMessages: AsiMessage[], allMessages: AsiMessage[]): AsiMessage[] {
 ...
 *
 * Note: this operates on a flat `AsiMessage[]` (e.g. `modifiedMessages`) rather than
 ...
 */
export function isApiReqAbsorbable(apiReqTs: number, allMessages: AsiMessage[]): boolean {
	const apiReqIndex = allMessages.findIndex((m) => m.ts === apiReqTs && m.say === "api_req_started")
	if (apiReqIndex === -1) {
		return false
	}
 ...
 */
function isApiReqFollowedOnlyByLowStakesTools(index: number, messages: (AsiMessage | AsiMessage[])[]): boolean {
	let hasLowStakesTool = false
	let hasReasoning = false
 ...
 */
export function groupLowStakesTools(groupedMessages: (AsiMessage | AsiMessage[])[]): (AsiMessage | AsiMessage[])[] {
	const result: (AsiMessage | AsiMessage[])[] = []
	let toolGroup: AsiMessage[] = []
	let pendingReasoning: AsiMessage[] = []
	let pendingApiReq: AsiMessage[] = []
	let hasTools = false
	const pendingTools: AsiMessage[] = []
	...
	const commitToolGroup = () => {
		if (toolGroup.length > 0 && hasTools) {
			const group = toolGroup as AsiMessage[] & { _isToolGroup: boolean }
