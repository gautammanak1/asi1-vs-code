...
export function isToolGroup(
	item: AsiMessage | AsiMessage[],
): item is AsiMessage[] & { _isToolGroup: true } {
...
function stripCompletionResultFlag(text: string): string {
	if (text.endsWith(COMPLETION_RESULT_CHANGES_FLAG)) {
		return text.slice(0, -COMPLETION_RESULT_CHANGES_FLAG.length);
	}
	return text;
}

/** Normalize assistant text for deduping against attempt_completion body. */
function normalizeAssistantBody(text: string): string {
	return text.trim().replace(/\r\n/g, "\n");
}

/**
 * When the model streams the same answer as `say: text` and then repeats it in
 * `say: completion_result` (attempt_completion), hide the redundant text row so
 * only the Result card + completion flow remain.
 */
function collectDuplicateTextBeforeCompletion(
	messages: AsiMessage[],
): Set<number> {
	const hideTextTs = new Set<number>();
	for (let i = 1; i < messages.length; i++) {
		...
		const completionBody = normalizeAssistantBody(
			stripCompletionResultFlag(msg.text),
		);
		if (completionBody.length === 0) {
			continue;
		}
		for (let j = i - 1; j >= 0; j--) {
			const prev = messages[j];
			if (prev.type === "say" && prev.say === "text" && prev.text?.trim()) {
				const textBody = normalizeAssistantBody(prev.text);
				if (textBody === completionBody) {
					hideTextTs.add(prev.ts);
				}
				break;
			}
		}
...
function isApiReqFollowedOnlyByLowStakesTools(
	index: number,
	messages: (AsiMessage | AsiMessage[])[],
): boolean {
	let hasLowStakesTool = false;
	let hasReasoning = false;
