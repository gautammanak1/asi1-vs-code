...
const ToDoListHeader = memo<{
	...
	const displayText = isCompleted
		? COMPLETED_MESSAGE
		: currentTodo?.text || TODO_LIST_LABEL;
		...
		>
			...
			<div className="flex items-center gap-2 z-10 py-2 px-2.5">
				...
				<div className="flex items-center text-foreground shrink-0">
					{isExpanded ? (
						<ChevronDownIcon className="ml-0.25" size="16" />
					) : (
...
const parseCurrentTodoInfo = (text: string): TodoInfo | null => {
	...
	const result: TodoInfo = {
		currentTodo,
		currentIndex:
			firstIncompleteIndex >= 0 ? firstIncompleteIndex + 1 : totalCount,
		completedCount,
		totalCount,
		...
		const handleEditClick = useCallback(
			(e: React.MouseEvent) => {
				...
				if (currentTaskItemId) {
					FileServiceClient.openFocusChainFile(
						StringRequest.create({ value: currentTaskItemId }),
					);
				}
			},
			[currentTaskItemId],
