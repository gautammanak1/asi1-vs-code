...
export const truncateDataUris = (text: string): string => {
	return text.replace(/data:[^;,]+(?:;[^,]+)?,[^\s<>"']+/g, (match) =>
		truncateSingleDataUri(match),
	);
};
...
export const normalizeRelativeUrl = (
	relativeUrl: string,
	baseUrl: string,
): string => {
...
export const checkIfImageUrl = async (url: string): Promise<boolean> => {
	...
	if (url.startsWith("https")) {
		try {
			...
			const servicePromise = WebServiceClient.checkIsImageUrl(
 ...
 */
export const extractUrlsFromText = (
	text: string,
	maxUrls: number = 50,
): UrlMatch[] => {
