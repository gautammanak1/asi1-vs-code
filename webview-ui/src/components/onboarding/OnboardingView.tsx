...
const OnboardingView = ({ onboardingModels }: { onboardingModels: OnboardingModelGroup }) => {
	...
	const models = useMemo(() => getAsiUIOnboardingGroups(onboardingModels), [onboardingModels])
	...
	const stepDisplayInfo = useMemo(() => {
		...
		const userStep = stepNumber !== 0 && stepNumber !== 2 && userType ? STEP_CONFIG[userType] : undefined
		const descriptionFromUserStep =
			userStep && "description" in userStep ? userStep.description : undefined
		const description = step?.description ?? descriptionFromUserStep ?? null
	...
	return (
		<div className="fixed inset-0 p-0 flex flex-col w-full">
			<div className="h-full px-5 xs:mx-10 overflow-auto flex flex-col gap-4 items-center justify-center">
				<AsiLogoWhite className="size-16 flex-shrink-0" />
