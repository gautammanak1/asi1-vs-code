...
export const AsiAuthContext = createContext<AsiAuthContextType | undefined>(
	undefined,
);

export const AsiAuthProvider: React.FC<{ children: React.ReactNode }> = ({
	children,
}) => {
	...
	const getUserOrganizations = useCallback(async () => {
		try {
			const response = await AccountServiceClient.getUserOrganizations(
				EmptyRequest.create(),
			);
	...
	useEffect(() => {
		const cancelSubscription = AccountServiceClient.subscribeToAuthStatusUpdate(
			EmptyRequest.create(),
			{
