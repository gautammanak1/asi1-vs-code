...
import { OnboardingModelGroup, type TerminalProfile } from "@shared/proto/Asi/state"
...
export const ExtensionStateContextProvider: React.FC<{
	...
	const [AsiModels, setAsiModels] = useState<Record<string, ModelInfo> | null>(null)
			...
			onResponse: (protoMessage) => {
				try {
					...
					setState((prevState) => {
						...
						const lastIndex = findLastIndex(prevState.AsiMessages, (msg) => msg.ts === partialMessage.ts)
						if (lastIndex !== -1) {
	...
	const refreshClineModels = useCallback(() => {
		ModelsServiceClient.refreshAsiModelsRpc(EmptyRequest.create({}))
			...
			.catch((error: Error) => console.error("Failed to refresh Asi models:", error))
	}, [])

	// Auto-refresh Asi models when provider is Asi
	useEffect(() => {
		const hasAsiProvider =
			state.apiConfiguration?.actModeApiProvider === "Asi" || state.apiConfiguration?.planModeApiProvider === "Asi"
		if (hasAsiProvider && AsiModels === null) {
			refreshClineModels()
		}
	}, [state.apiConfiguration?.actModeApiProvider, state.apiConfiguration?.planModeApiProvider, AsiModels, refreshClineModels])
