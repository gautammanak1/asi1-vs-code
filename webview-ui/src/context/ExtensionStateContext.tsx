...
} from "@shared/proto/Asi/state";
import { convertProtoToAsiMessage } from "@shared/proto-conversions/cline-message";
...
export interface ExtensionStateContextType extends ExtensionState {
	...
	AsiModels: Record<string, ModelInfo> | null;
	...
	setGlobalAsiRulesToggles: (toggles: Record<string, boolean>) => void;
	setLocalAsiRulesToggles: (toggles: Record<string, boolean>) => void;
...
export const ExtensionStateContextProvider: React.FC<{
	...
	const [settingsTargetSection, setSettingsTargetSection] = useState<
		string | undefined
	>(undefined);
	...
	const hideWorktrees = useCallback(
	...
	const [onboardingModels, setOnboardingModels] = useState<
		OnboardingModelGroup | undefined
	>(undefined);

	const [AsiModels, setAsiModels] = useState<Record<string, ModelInfo> | null>(
		null,
	);
	...
	const [vercelAiGatewayModels, setVercelAiGatewayModels] = useState<
		Record<string, ModelInfo>
	>({});
	...
	const [availableTerminalProfiles, setAvailableTerminalProfiles] = useState<
		TerminalProfile[]
	>([]);
	...
	const [basetenModelsState, setBasetenModels] = useState<
		Record<string, ModelInfo>
	>({
		...basetenModels,
	...
	const settingsButtonClickedSubscriptionRef = useRef<(() => void) | null>(
		null,
	);
	...
	useEffect(() => {
						...
						try {
							...
							setState((prevState) => {
								...
								if (
									...
									stateData.AsiMessages = stateData.AsiMessages?.length
										? stateData.AsiMessages
		...
		mcpServersSubscriptionRef.current = McpServiceClient.subscribeToMcpServers(
			EmptyRequest.create(),
			{
				onResponse: (response) => {
		...
		openRouterModelsUnsubscribeRef.current =
			ModelsServiceClient.subscribeToOpenRouterModels(EmptyRequest.create({}), {
		...
		liteLlmModelsUnsubscribeRef.current =
			ModelsServiceClient.subscribeToLiteLlmModels(EmptyRequest.create({}), {
				onResponse: (response: OpenRouterCompatibleModelInfo) => {
					const models = fromProtobufModels(response.models);
	...
	const refreshClineModels = useCallback(() => {
		ModelsServiceClient.refreshClineModelsRpc(EmptyRequest.create({}))
			.then((response: OpenRouterCompatibleModelInfo) => {
				const models = fromProtobufModels(response.models);
				setAsiModels((prev) =>
					Object.keys(models).length > 0 ? models : (prev ?? null),
				);
			})
			.catch((error: Error) =>
	...
	const contextValue: ExtensionStateContextType = {
		...
		setShowMcp,
		closeMcpView,
		setGlobalAsiRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				globalAsiRulesToggles: toggles,
			})),
		setLocalAsiRulesToggles: (toggles) =>
			setState((prevState) => ({
				...prevState,
				localAsiRulesToggles: toggles,
			})),
